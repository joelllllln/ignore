...
(() => {
  ...
  // ── LIGHTSABER DRAW (v17.14, owner call: "switch the finger-draw sound to a light saber voom voooom").
  // The old draw voice was a dry zap re-fired many times a second — spam. Now the finger IS a saber:
  // ONE persistent voice — two detuned saws through a resonant lowpass, with a slow LFO shimmer on the
  // detune — ignites on the first draw tick and hums while you sweep. Speed drives the VOOM: a fast
  // sweep flings the filter open, lifts the pitch and swells the volume (fast attack), then everything
  // sags back on its own slow release — the classic doppler swing. Lift (or rest) the finger for
  // ~160ms and the blade powers down. No per-hit one-shots at all.
  const Saber = { o1: null, o2: null, fil: null, g: null, lfo: null, killer: 0, lx: 0, ly: 0, lt: 0 };
  function Audio_saber(x, y) {
    if (!opt("sound")) return; const a = Sfx.ac(); if (!a) return;
    const now = performance.now();
    let speed = 0;
    if (Saber.lt && now - Saber.lt < 200) speed = Math.hypot(x - Saber.lx, y - Saber.ly) / Math.max(8, now - Saber.lt) * 1000;   // blade speed, world px/s
    Saber.lx = x; Saber.ly = y; Saber.lt = now;
    if (!Saber.g) {   // ignite
      const t0 = a.currentTime;
      const g = a.createGain(); g.gain.setValueAtTime(0.0001, t0); g.gain.setTargetAtTime(0.045, t0, 0.03);
      const fil = a.createBiquadFilter(); fil.type = "lowpass"; fil.Q.value = 4; fil.frequency.value = 260;
      const o1 = a.createOscillator(); o1.type = "sawtooth"; o1.frequency.value = 82;
      const o2 = a.createOscillator(); o2.type = "sawtooth"; o2.frequency.value = 82; o2.detune.value = 12;
      const lfo = a.createOscillator(); lfo.frequency.value = 5.2; const lg = a.createGain(); lg.gain.value = 7;
      lfo.connect(lg); lg.connect(o2.detune);   // the electric shimmer
      o1.connect(fil); o2.connect(fil); fil.connect(g); g.connect(Sfx.out(a));
      o1.start(); o2.start(); lfo.start();
      Object.assign(Saber, { o1, o2, fil, g, lfo });
    }
    const t = a.currentTime, k = Math.min(1, speed / 1400);
    Saber.fil.frequency.setTargetAtTime(260 + k * 2400, t, 0.045);   // voom: filter flings open with the sweep…
    Saber.o1.frequency.setTargetAtTime(82 + k * 34, t, 0.06);        // …pitch lifts a little…
    Saber.o2.frequency.setTargetAtTime(82 + k * 34, t, 0.06);
    Saber.g.gain.setTargetAtTime(0.045 + k * 0.055, t, k > 0.25 ? 0.03 : 0.12);   // …and volume swells fast, sags slow
    ...
    Saber.killer = setTimeout(() => {   // blade off — quick ramp, then release the nodes
      ...
      const { o1, o2, lfo } = Saber;
      setTimeout(() => { try { o1.stop(); o2.stop(); lfo.stop(); } catch (e) {} }, 350);
      Saber.o1 = Saber.o2 = Saber.fil = Saber.g = Saber.lfo = null; Saber.lt = 0;
    }, 160);
  }
