...
(() => {
  ...
  let envFor = 0, envW = 0, envH = 0, envAnchors = [], envDust = [], envMotes = [], envDash = 12;
  function ensureEnv() {
    ...
    const nx = Math.max(5, Math.round(W / 155)), ny = Math.max(4, Math.round(H / 175));   // v18.5 density pass (was /240, /260)
    ...
    for (let i = 0; i < 10; i++) envMotes.push({ edge: i % 4, off: er(), spd: 0.008 + er() * 0.02, ph: er() * TAU, m: 14 + er() * 26 });
