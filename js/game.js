...
(() => {
  ...
  let comboMul = 1, comboT = 0, comboFxT = 0;
  ...
  function hitDot(d, dmg, src) {
    ...
    if (d.hp <= 0) {
      ...
      if (src === "draw") {
        ...
        comboMul = Math.min(5, cm + 0.35); comboT = 1.6;
      }
  ...
  function render() {
    ...
    if (state === "play" && comboMul > 1.02 && !planetMeta(S.galaxy).conquered) {
      ...
      ctx.fillText("☝ COMBO ×" + comboMul.toFixed(1) + (comboMul >= 4.99 ? " — MAX" : ""), SW / 2, 100);
      ctx.globalAlpha = draining ? 0.25 : 0.6;
      ctx.fillStyle = "rgba(255,255,255,0.18)"; ctx.fillRect(SW / 2 - 62, 107, 124, 5);
      ctx.fillStyle = "#fff"; ctx.fillRect(SW / 2 - 62, 107, 124 * cm01, 5);
      ctx.restore();
    }
