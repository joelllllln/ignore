...
(() => {
  ...
  const UPS = [
    ...
    { id: "value",     tab: "eco", name: "Value",      base: 130, mul: 1.46, desc: () => "×" + derived.valueMul.toFixed(2) + " /dot  ·  ✦ boost in " + mileNext(ecoLv("value")) + "" },
  ...
  // Value's linear gain is shaved by MILE_SHARE and handed back in a lump every MILE_LEG legacy
  ...
  const valueFromLv = r => { const L = legacyLv(r);
    return 1 + VAL_PER * (1 - MILE_SHARE) * L + VAL_PER * MILE_SHARE * MILE_LEG * Math.floor(L / MILE_LEG); };
  const spawnFromLv = r => 0.9 + 1.15 * legacyLv(r);
  ...
  function recompute() {
    ...
    derived.capacity = ecoCost() * 220 * Math.pow(1.60, legacyLv(L.capacity)) * (derived.incomeMul || 1);   // cash ceiling rides the FRONTIER economy (v17.4 — visiting an old world must not crash the ceiling below your wallet and freeze banking) × the SAME income multiplier as the payouts (engine incl.) so it never lags your income
    ...
    derived.luck = Math.min(0.6, 0.003 * legacyLv(L.luck) + pk().luck);    // +0.3% chance of a rare 9× SPECIAL dot per Luck level (buffed from 0.1% — was a trap stat vs Value) + Ascension Fortune perk
  ...
  function syncHUD() {
    ...
    for (const id in listRows) {
      ...
      if (row.kind === "unit") {
        ...
        // JUICY"). Value's lump lands every MILE_LEG legacy levels; the row fills toward it so you can
        ...
        if (row.mile && u.id === "value" && !maxed) {
          ...
          row.el.classList.toggle("mile-near", away <= 3 && away > 1);
        ...
        } else if (row.mile) { row.el.classList.remove("mile-near", "mile-ready"); row.el.style.setProperty("--mile", 0); }
      }
    }
  ...
  function renderList() {
    ...
    if (activeTab === "def" || activeTab === "drone") {
      ...
      for (const u of UPS) { if (u.tab !== activeTab) continue;
        const el = document.createElement("div"); el.className = "up";
        el.innerHTML = `<span class="u-mile"></span><span class="u-dot" style="background:${col}"></span><div class="u-mid"><div class="u-name">${u.name}<span class="lv"></span><span class="u-milechip">✦ MILESTONE</span></div><div class="u-desc"></div></div><button class="u-info" title="Info">i</button><button class="u-buy"></button>`;
        wrap.appendChild(el);
  ...
  function buyUpgrade(u) {
    ...
    if (u.id === "value" && Math.floor(legacyLv(lvl0) / MILE_LEG) < Math.floor(legacyLv(S.lv[u.id]) / MILE_LEG)) {
      flashAdd(0.45); shakeAdd(3.2); vibe([40, 30, 70]); Audio_win();
      ring(W / 2, H / 2, 30, Math.max(W, H) * 0.34, 0.5); burst(W / 2, H / 2, 22, 170, 2.1);
      floatTxt(W / 2, H / 2 - 30, "\u2726 VALUE MILESTONE");
      floatTxt(W / 2, H / 2 - 6, "\u00d7" + derived.valueMul.toFixed(2) + " per dot");
    } else Audio_buy();
    syncHUD(); save();
  }
  ...
  if (typeof window !== "undefined") window.__SIM = {
    ...
    valueMul: valueFromLv, spawnFromLv, legacyLv, ECO_STEP, ECO_FINE_FROM,   // v18.51: the GAME's curves — the sim carried its own copy of 1 + 0.13*lv and would have silently kept the old scale
