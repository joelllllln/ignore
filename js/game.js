...
(() => {
  ...
  //   DISC.pr = the PLAY radius: the disc actually on screen at normal zoom. Anything that must stay
  //             where the player can see it (the boss patrol, Dot Rain, the mine complex) uses this.
  let DISC = { cx: 0, cy: 0, r: 200, pr: 160 };
  ...
  function fitDisc() {
    ...
    DISC.r = Math.max(60, Math.hypot(SW, bandH) / 2 * 1.06 / z);  // limb just past every corner of the band
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    ...
    if (fromTop) {   // Dot Rain falls INTO the bowl — spread over the disc, not piled on one line
      ...
      const p2 = discPt(a, Math.min(discR() * 0.94, playR() * (1.06 + Math.random() * 0.06)));   // v18.27: the belt just past the edge of view — they walk in out of the terrain, they never pop into open ground
      ex = p2.x; ey = p2.y;
    }
  ...
  function ensureEnv() {
    ...
    const n = Math.max(9, Math.round(24 * B.dens));
    const angles = [];
