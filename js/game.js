...
(() => {
  ...
  let WORLD = { cx: 0, cy: 0, hw: 300, hh: 300, vw: 260, vh: 260, cr: 60 };
  ...
  const MAP_PAD = 1.04;   // the border sits just past the edge of the view — with no terrain on it any more, the border IS the edge you see, so it stays where you can see it
  ...
  function fitWorld() {
    ...
    WORLD.cr = Math.min(WORLD.hw, WORLD.hh) * 0.16;               // rounded corners — a world, not a poster frame
  ...
  function buildMine(g) {
    ...
    Audio_buy(); flashAdd(0.25); vibe([30, 20, 60]);   // rim rebuilds so the headframe appears immediately
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    ...
    if (fromTop) {   // Dot Rain falls INTO the bowl — spread over the disc, not piled on one line
      ...
      // fauna comes in over the EDGE OF THE MAP, evenly all the way around it, and lands just
      // inside the edge of the view so you always see it arrive
      const a = Math.random(), bp = perim(a);
      const p2 = beltPt(a, visDepth(bp) + rnd(0.01, 0.07) * Math.min(viewHW(), viewHH()));
      ex = p2.x; ey = p2.y;
    }
  ...
  function addBorder(dOf, steps) {   // trace the map's border into the CURRENT path, offset inward by dOf(t)
    const n = steps || 220;
    for (let i = 0; i <= n; i++) {
      const t = i / n, p = perim(t), d = dOf ? dOf(t) : 0;
  ...
  function bandWatch() {
    ...
    if (Math.abs(top - bandLast) > 2) { bandLast = top; fitWorld(); }
  }
  function drawEnv() {
    bandWatch();
    const t = performance.now() / 1000, cx = wCX(), cy = wCY(), hw = wHW(), hh = wHH();
    ctx.save();
    // THE GROUND — solid, so the backdrop starfield can't shine up through the field
    ctx.beginPath(); addBorder(null); ctx.fillStyle = "#050505"; ctx.fill();
    ctx.clip();
    { const g = ctx.createRadialGradient(cx, cy, Math.min(hw, hh) * 0.2, cx, cy, Math.hypot(hw, hh));
    ...
    // THE BORDER — the edge of the map, and one quiet line inside it so the edge has some depth
    ctx.globalAlpha = 0.09; ctx.lineWidth = 1;
    ctx.beginPath(); addBorder(() => Math.min(hw, hh) * 0.045); ctx.stroke();
    ctx.globalAlpha = 0.24; ctx.lineWidth = 2.2;
    ctx.beginPath(); addBorder(() => 1.4); ctx.stroke();
  ...
  function setScreen(s) {
    ...
    if (s === "play") { syncAutoBtn(); fitWorld(); }   // the dock/HUD only have real geometry once play is on screen — fit the map to the band they leave
  ...
  function resize() {
    ...
    fitWorld();                                  // re-fit the map to the new play band (also sets camMin)
  ...
  if (typeof ResizeObserver !== "undefined") {
    let rfT = 0;
    const ro = new ResizeObserver(() => { const n = performance.now(); if (n - rfT < 120) return; rfT = n; fitWorld(); });
