...
(() => {
  ...
  // v18.29 THE MAP IS A RECTANGLE AGAIN (owner: "make it a rectangle … the edges of the world are
  // black rather than the planet's actual environment … things are still spawning not around the
  // edges of the map"). A disc whose limb sat off-screen put the whole landscape out of sight: the
  // edges of what you actually look at were plain black ground, and fauna crossed a line with
  // nothing on it. The world is a rounded RECTANGLE, sized just past the play band, and its BORDER
  // carries the terrain — the belt reaches far enough inward that its ridge and the formations
  // standing on it are inside the view, so every edge of the screen shows this planet's environment
  // and fauna walks out of it.
  ...
  //   WORLD.bd     — how deep the terrain belt reaches in from the border
  //   WORLD.vd     — how deep the border is from the edge of the view: anything deeper is ON SCREEN
  // Everything about the landscape is expressed as (t, depth) — position along the border and depth
  // inward — so it composes identically on a tall phone, a landscape phone or a tablet.
  let WORLD = { cx: 0, cy: 0, hw: 300, hh: 300, vw: 260, vh: 260, cr: 60, bd: 70, vd: 34 };
  ...
  const MAP_PAD = 1.15;   // the border sits this far past the edge of the view
  const BELT = 0.34;      // terrain belt depth as a fraction of the map's short half-extent
  const wCX = () => WORLD.cx, wCY = () => WORLD.cy, wHW = () => WORLD.hw, wHH = () => WORLD.hh;
  const viewHW = () => WORLD.vw, viewHH = () => WORLD.vh, beltD = () => WORLD.bd;
  function fitWorld() {
    ...
    const bandBot = Math.max(bandTop + 40, Math.min(dockTop - 6, SH));   // a 120px minimum here used to OVERRIDE the real dock position on a landscape phone, so the world was fitted to a band that did not exist and drew straight through the dock
    ...
    WORLD.vw = Math.max(40, SW / 2 / z); WORLD.vh = Math.max(40, bandH / 2 / z);   // the screen rectangle, in world units
    WORLD.hw = WORLD.vw * MAP_PAD; WORLD.hh = WORLD.vh * MAP_PAD;
    WORLD.cx = W / 2; WORLD.cy = H / 2;                           // the map IS the world centre, so the army stands in the middle of it
    const short = Math.min(WORLD.hw, WORLD.hh);
    WORLD.cr = short * 0.20;                                      // rounded corners — a world, not a poster frame
    WORLD.bd = short * BELT;
    WORLD.vd = (1 - 1 / MAP_PAD) * Math.max(WORLD.hw, WORLD.hh);  // depth at which the belt comes into view
    camMin = Math.min(SW / (2 * WORLD.hw * 1.14), bandH / (2 * WORLD.hh * 1.14));   // zoom-out stop: the whole map + a margin of outer waste
  }
  ...
  // WALK THE BORDER: t in [0,1) → a point on it plus the INWARD normal there. This one coordinate
  // carries the whole landscape — where a formation stands, where the ridge runs, where fauna comes
  // over the edge — so none of it has to know what shape of screen it is on.
  function perim(t) {
    const hw = wHW(), hh = wHH(), r = Math.min(WORLD.cr, hw, hh), cx = wCX(), cy = wCY();
  ...
  const perimLen = () => { const r = Math.min(WORLD.cr, wHW(), wHH()); return 4 * (wHW() - r) + 4 * (wHH() - r) + 2 * Math.PI * r; };
  // how deep the border sits BEYOND the edge of the view at a point on it — different on the long and
  // short axes (on a landscape phone, enormously so), so the belt has to be measured per edge or the
  // terrain reaches into view on two sides and nowhere near it on the other two
  const visDepth = p => Math.abs(p.nx) * (WORLD.hw - WORLD.vw) + Math.abs(p.ny) * (WORLD.hh - WORLD.vh);
  const beltAt = p => Math.max(WORLD.bd, visDepth(p) * 1.9);
  // a point `d` deep inside the border at border position t (negative d = out in the wastes)
  const beltPt = (t, d) => { const p = perim(t); return { x: p.x + p.nx * d, y: p.y + p.ny * d, nx: p.nx, ny: p.ny }; };
  // rotation that makes a motif's local -y point INWARD — every formation leans off the border
  const inRot = p => Math.atan2(p.nx, -p.ny);
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    ...
    if (fromTop) {   // Dot Rain falls INTO the bowl — spread over the disc, not piled on one line
      ...
      // v18.29: fauna comes out of the TERRAIN along the border — the spawn point snaps to a real
      // formation (with jitter) and sits at a depth just inside the edge of the view, so you watch
      // them emerge from the environment at the edge of the map instead of crossing a bare line
      const an = envAnchors.length ? envAnchors[Math.floor(Math.random() * envAnchors.length)] : null;
      const a = (an ? an.t + rnd(-0.012, 0.012) : Math.random());
      const bp = perim(a), p2 = beltPt(a, visDepth(bp) + rnd(0.02, 0.30) * beltAt(bp));
      ex = p2.x; ey = p2.y;
    }
  ...
  function ensureEnv() {
    if (envFor === S.galaxy && envW === W && envH === H && envR === WORLD.hw + WORLD.hh && envAnchors.length) return;
    envFor = S.galaxy; envW = W; envH = H; envR = WORLD.hw + WORLD.hh;
    envAnchors = []; envBack = []; envOuter = []; envGround = []; envDust = []; envMotes = [];
    const B = biomeOf(S.galaxy);
    const short = Math.min(wHW(), wHH()), L = perimLen(), BD = beltD(), VD = WORLD.vd;
    const U = short / 320;   // ONE unit of scale: everything below is a fraction of the map, so the art is the same composition on a 320px phone and a desktop
    ...
    // Positions are border coordinates t ∈ [0,1) — the layout is what makes two worlds with similar
    // formations still read as different places. Counts scale with the SIZE of the border, so a
    // bigger map is a bigger landscape rather than the same handful of features stretched thinner.
    const n = clamp(Math.round(B.dens * L / 76), 12, 90);
    const ts = [];
    if (B.layout === "even") { for (let i = 0; i < n; i++) ts.push((i + 0.3 + er() * 0.4) / n); }
    else if (B.layout === "clumps") { const cl = 3 + Math.floor(er() * 3), m = Math.ceil(n / cl);
      for (let c = 0; c < cl; c++) { const c0 = (c + er() * 0.35) / cl, w = 0.08 + er() * 0.07;
        for (let i = 0; i < m; i++) ts.push(c0 + (er() - 0.5) * w); } }
    else if (B.layout === "arcs") { const t0 = er(), span = 0.27 + er() * 0.11;
      for (let i = 0; i < n; i++) ts.push(er() < 0.72 ? t0 + er() * span : er()); }
    else if (B.layout === "twin") { const t0 = er();
      for (let i = 0; i < n; i++) ts.push((i % 2 ? t0 : t0 + 0.5) + (er() - 0.5) * 0.25); }
    else { const m = Math.max(5, Math.round(n * 0.45)); for (let i = 0; i < m; i++) ts.push((i + er() * 0.6) / m); }
    const big = B.layout === "sparse";
    const cand = ts.map(t => {
      const lm = er() < 0.20;
      // a normal formation stands about a third of the way in from the border, a landmark half of it,
      // a signature most of it — the skyline you recognise a world by
      const s = ((lm ? (big ? 3.4 : 3.0) : (big ? 2.6 : 2.0)) + er() * 0.9) * U;
      return { t: ((t % 1) + 1) % 1, s, v: er(), p: er() * TAU, lm, f: pick() };
    }).sort((x, y) => x.t - y.t);
    // SIGNATURE FORMATIONS first — one to three colossal structures, spaced around the border, each
    // holding its own ground so nothing else grows through it
    const sigs = [];
    for (let i = 0; i < B.sig; i++)
      sigs.push({ t: ((i / B.sig + er() * 0.12) % 1), d: 0, s: (4.6 + er() * 1.2) * U,
        v: er(), p: er() * TAU, lm: true, sig: true, f: B.mix[0] });
    const gap = (a, b) => { const dd = Math.abs(a - b); return Math.min(dd, 1 - dd) * L; };   // border distance, wrapping
    const clearOf = c => sigs.every(g => gap(c.t, g.t) > (halfW(g.s) + halfW(c.s)) * 1.15);
    // NO PILE-UPS: walk the border in order and drop anything that would grow through its neighbour
    const kept = [];
    for (const c of cand) {
      const prev = kept[kept.length - 1];
      if (prev && (c.t - prev.t) * L < (halfW(prev.s) + halfW(c.s)) * 1.15) continue;
      if (!clearOf(c)) continue;
      kept.push(c);
    }
    if (kept.length > 2) {   // and close the loop: the last one must clear the first one too
      const f = kept[0], l = kept[kept.length - 1];
      if ((1 - l.t + f.t) * L < (halfW(f.s) + halfW(l.s)) * 1.15) kept.pop();
    }
    // the belt has DEPTH: formations stand anywhere from the border itself to the ridge line, and
    // everything past VD is inside the view — so the edges of the screen carry real terrain
    for (const c of kept) { c.d = beltAt(perim(c.t)) * (0.22 + er() * 0.76); envAnchors.push(c); }
    for (const g of sigs) { g.d = beltAt(perim(g.t)) * (0.55 + er() * 0.3); envAnchors.push(g); }
    // BACK ROW — a fainter, smaller rank pressed right against the border, for depth
    for (let i = 0; i < Math.round(n * 0.9); i++) { const t = er();
      envBack.push({ t, d: beltAt(perim(t)) * (0.02 + er() * 0.14), s: (1.0 + er() * 0.7) * U,
        v: er(), p: er() * TAU, f: pick() }); }
    // OUTER WASTES — the same landscape continuing past the border, thinning as it recedes
    for (let i = 0, on = clamp(Math.round(L / 58), 20, 130); i < on; i++) {
      const q = er();
      envOuter.push({ t: er(), d: -short * (0.03 + q * 0.55), s: (1.7 - q * 0.8) * U,
        v: er(), p: er() * TAU, al: 0.04 + 0.26 * (1 - q) * (1 - q), f: pick() });
    }
    // THE SURFACE — this world's own ground texture, scattered across the open field inside the ridge
    { const fw = Math.max(20, wHW() - beltAt({ nx: 1, ny: 0 }) * 0.9), fh = Math.max(20, wHH() - beltAt({ nx: 0, ny: 1 }) * 0.9);
      const gn = clamp(Math.round(B.dens * (4 * fw * fh) / ((64 * U) * (64 * U))), 24, 260);   // one surface feature per ~64U square of open ground
      for (let i = 0; i < gn; i++) { const q = er();
        envGround.push({ x: (er() * 2 - 1) * fw, y: (er() * 2 - 1) * fh,
          s: (0.34 + q * q * 0.72) * U, v: er(), p: er() * TAU, rot: er() * TAU }); } }
    // THE RIDGE — one closed ground-line running the whole border, the inner edge of the terrain.
    // Built from coherent waves and spikes, never per-vertex noise (that mills it into a gear).
    { const steps = 320, ph = [er() * TAU, er() * TAU, er() * TAU], spikes = [];
      if (B.ridge === "shard") for (let i = 0; i < 15; i++) spikes.push({ a: er() * TAU, w: 0.07 + er() * 0.10, h: 0.10 + er() * 0.17 });
      const sm = x => x * x * (3 - 2 * x);
      envRidge = new Array(steps + 1);
      for (let i = 0; i <= steps; i++) {
        const a = i / steps * TAU; let k;   // k = how far in from the border, as a fraction of the belt
        if (B.ridge === "jag") k = 0.80 + 0.05 * Math.sin(a * 11 + ph[0]) + 0.03 * Math.sin(a * 23 + ph[1]);
        else if (B.ridge === "dune") k = 0.78 + 0.11 * Math.sin(a * 3 + ph[0]) + 0.045 * Math.sin(a * 7 + ph[1]);
        else if (B.ridge === "shard") { k = 0.70;
          for (const sp of spikes) { const dd = Math.abs(((a - sp.a + Math.PI * 3) % TAU) - Math.PI); if (dd < sp.w) k += sp.h * sm(1 - dd / sp.w); } }
        else if (B.ridge === "flat") k = 0.84 + 0.025 * Math.sin(a * 5 + ph[0]) + 0.018 * Math.sin(a * 11 + ph[1]);
        else k = 0.66 + 0.24 * sm(clamp(0.5 + 0.9 * Math.sin(a * 4.2 + ph[0]), 0, 1));   // broken: plateaus split by gulfs
        envRidge[i] = beltAt(perim(i / steps)) * clamp(k, 0.35, 1.02);
      } }
    // dust & drifting motes live in the terrain belt
    for (let i = 0; i < Math.round(L / 34); i++)
      envDust.push({ t: er(), d: beltAt({ nx: 1, ny: 1 }) * er() * 0.9, r: (0.6 + er() * 1.5) * Math.max(1, U * 0.9), ln: er() < 0.35 ? (2 + er() * 5) * Math.max(1, U) : 0, an: er() * TAU });
    for (let i = 0; i < 16; i++) { const t = er(); envMotes.push({ off: t, spd: 0.0012 + er() * 0.0028, dk: 0.15 + er() * 0.6, ph: er() * TAU }); }
  }
  const ENV_DRAW = [null,
  ...
  // A motif grows along its local -y, and inRot(p) turns that into "away from the border", so every
  // formation leans in off the edge of the map.
  ...
  // trace the map's border into the CURRENT path, offset inward by dOf(t) (negative = outward)
  function addBorder(dOf, steps) {
    const n = steps || 300;
    for (let i = 0; i <= n; i++) {
      const t = i / n, p = perim(t), d = dOf ? dOf(t, i) : 0;
      const x = p.x + p.nx * d, y = p.y + p.ny * d;
      i ? ctx.lineTo(x, y) : ctx.moveTo(x, y);
    }
    ctx.closePath();
  }
  const ridgeD = (t, i) => envRidge.length ? envRidge[Math.round(t * (envRidge.length - 1)) % envRidge.length] : 0;
  // OUTER WASTES (v18.27, owner: "the edges should be the environment/terrain of the planet not random
  // shit") — beyond the border the SAME landscape keeps going: this world's ridge echoing outward as
  // receding contours, its own formations standing in the half-light, dimming into the dark. No orbit
  // rings, no bearing ticks, no starfield — it is all ground. Drawn with an even-odd fill so it can
  // never paint over the playfield.
  function drawOuter(t) {
    const cx = wCX(), cy = wCY(), M = Math.max(W, H) * 1.8, short = Math.min(wHW(), wHH());
    ctx.save();
    ctx.beginPath(); ctx.rect(cx - M, cy - M, M * 2, M * 2); addBorder(null, 200);
    ctx.fillStyle = "#000"; ctx.fill("evenodd");
    ctx.clip("evenodd");                       // everything below is confined to the wastes
    { const g = ctx.createLinearGradient(0, cy - wHH() - short * 0.8, 0, cy - wHH());   // a little light still on the land just outside
      g.addColorStop(0, "rgba(255,255,255,0)"); g.addColorStop(1, "rgba(255,255,255,0.016)");
    ...
    for (let ring = 0; ring < 4; ring++) {     // the ridge, receding outward as fainter contours of the same terrain
      const off = short * (0.05 + ring * 0.10);
      ctx.globalAlpha = 0.14 * Math.pow(0.62, ring);
      ctx.beginPath(); addBorder((tt, i) => -off + (ridgeD(tt) - beltD() * 0.8) * 0.4, 220); ctx.stroke();
    }
    ctx.lineWidth = 1.6;
    const fnOf = i => ENV_DRAW[clamp(i | 0, 1, 18)] || ENV_DRAW[1];
    for (const o of envOuter) {                // the same formations, standing out in the wastes
      const p2 = beltPt(o.t, o.d);
      ctx.save(); ctx.translate(p2.x, p2.y); ctx.rotate(inRot(p2));
  ...
  function bandWatch() {
    ...
    if (Math.abs(top - bandLast) > 2) { bandLast = top; fitWorld(); envFor = 0; }
  }
  function drawEnv() {
    bandWatch();
    ensureEnv();
    const B = biomeOf(S.galaxy), t = performance.now() / 1000;
    const cx = wCX(), cy = wCY(), BD = beltD();
    const fnOf = i => ENV_DRAW[clamp(i | 0, 1, 18)] || ENV_DRAW[1];
    drawOuter(t);
    ctx.save();
    // THE GROUND ITSELF — the backdrop starfield lives in screen space and would otherwise shine up
    // through the surface. Lay solid ground inside the border, with a soft vignette into the belt.
    ctx.beginPath(); addBorder(null, 200); ctx.fillStyle = "#050505"; ctx.fill();
    ctx.clip();                                // v18.27: NOTHING the landscape draws may leak past the border
    { const g = ctx.createRadialGradient(cx, cy, Math.min(wHW(), wHH()) * 0.2, cx, cy, Math.hypot(wHW(), wHH()));
      g.addColorStop(0, "rgba(255,255,255,0.022)"); g.addColorStop(0.8, "rgba(255,255,255,0.010)"); g.addColorStop(1, "rgba(0,0,0,0.22)");
      ctx.fillStyle = g; ctx.fillRect(cx - wHW(), cy - wHH(), wHW() * 2, wHH() * 2); }
    ctx.strokeStyle = "#fff"; ctx.fillStyle = "#fff";
    // THE SURFACE — this world's own ground, under everything
    { const gd = GROUND_DRAW[B.gnd] || GROUND_DRAW.plates;
      ctx.lineWidth = 1.2;
      for (const q of envGround) {
        ctx.save(); ctx.translate(cx + q.x, cy + q.y); ctx.rotate(q.rot);
        ctx.globalAlpha = 0.13; gd(ctx, q.s, q.v, q.p, t); ctx.restore(); } }
    // THE RIDGE — the terrain silhouette running just inside every edge of the screen, with a softer
    // echo behind it for depth. This is the line the owner sees as "the edge of the world".
    ctx.globalAlpha = 0.30; ctx.lineWidth = 1.7;
    ctx.beginPath(); addBorder(ridgeD); ctx.stroke();
    ctx.globalAlpha = 0.11; ctx.lineWidth = 1.2;
    ctx.beginPath(); addBorder(tt => ridgeD(tt) * 0.55, 220); ctx.stroke();
    // dust & scratches through the terrain belt
    ctx.globalAlpha = 0.14; ctx.lineWidth = 1;
    for (const d of envDust) {
      const p2 = beltPt(d.t, Math.min(d.d, beltAt(perim(d.t)) * 0.9));
      if (d.ln) { ctx.beginPath(); ctx.moveTo(p2.x - Math.cos(d.an) * d.ln, p2.y - Math.sin(d.an) * d.ln); ctx.lineTo(p2.x + Math.cos(d.an) * d.ln, p2.y + Math.sin(d.an) * d.ln); ctx.stroke(); }
      else { ctx.beginPath(); ctx.arc(p2.x, p2.y, d.r, 0, TAU); ctx.fill(); }
    }
    // BACK ROW — the fainter rank pressed against the border, for depth
    ctx.lineWidth = 1.5;
    for (const a of envBack) {
      const p2 = beltPt(a.t, a.d);
      ctx.save(); ctx.translate(p2.x, p2.y); ctx.rotate(inRot(p2));
    ...
    for (const a of envAnchors) {
      ...
      ctx.globalAlpha = a.sig ? 0.72 : a.lm ? 0.56 : 0.42;
    ...
    for (const m of envMotes) {
      const mt = (m.off + t * m.spd) % 1, p2 = beltPt(mt, beltAt(perim(mt)) * m.dk);
      ctx.globalAlpha = 0.05 + 0.09 * (0.5 + 0.5 * Math.sin(t * 1.7 + m.ph));
      ctx.beginPath(); ctx.arc(p2.x, p2.y, 1.5, 0, TAU); ctx.fill();
    }
    // THE BORDER — the world's own boundary, drawn last so it reads as the edge of the ground
    ctx.globalAlpha = 0.20; ctx.lineWidth = 2.4;
    ctx.beginPath(); addBorder(tt => 1.2, 200); ctx.stroke();
  ...
  function setScreen(s) {
    ...
    if (s === "play") { syncAutoBtn(); fitWorld(); envFor = 0; }   // v18.26: the dock/HUD only have real geometry once play is on screen — fit the world disc to the band they leave
  ...
  function resize() {
    ...
    fitWorld(); envFor = 0;                      // re-fit the map to the new play band and rebuild its terrain (also sets camMin)
  ...
  if (typeof ResizeObserver !== "undefined") {
    let rfT = 0;
    const ro = new ResizeObserver(() => { const n = performance.now(); if (n - rfT < 120) return; rfT = n; fitWorld(); envFor = 0; });
