...
(() => {
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    ...
    const men = (pmSpawn && pmSpawn.conquered) ? 2.0 : 3.5 * Math.pow(clamp(curEarned / Math.max(1, conquerTarget(S.galaxy)), 0, 1), 0.7);
  ...
  function render() {
    ...
    if (state === "play" && !S.travel && planetMeta(S.galaxy).conquered) {
      ...
      ctx.fillText(v.mine ? "⛏ core mine digging " + fmtMineRate(mineRate(S.galaxy)) : "⛏ mine site ready — build it from the STAR MAP (" + curSym(S.galaxy) + " " + fmt(mineCost(S.galaxy)) + ")", SW / 2, cy + 43);
