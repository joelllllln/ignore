...
(() => {
  ...
  function drawEnv() {
    ...
    // ⛏ ◈ CORE MINE HEADFRAME (v18.5) — a BUILT ascension farm is visible ON the world: derrick tower,
    // spinning winding wheel, shaft into the rim, and ◈ glints rising out of it on the mine's heartbeat
    if (mineBuilt(S.galaxy)) {
      const mx = W * 0.32, my = 118;   // upper-left quarter, fully clear of the planet banner at any zoom — a landmark standing at the field's edge
      ctx.save(); ctx.translate(mx, my); ctx.scale(1.3, 1.3); ctx.globalAlpha = 0.65; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(-16, 0); ctx.lineTo(-5, -30); ctx.lineTo(5, -30); ctx.lineTo(16, 0); ctx.stroke();   // A-frame tower
      ctx.beginPath(); ctx.moveTo(-10, -14); ctx.lineTo(10, -14); ctx.stroke();                                        // crossbeam
      const wr = 7, rot = t * 1.4;                                                                                     // winding wheel, always turning
      ctx.beginPath(); ctx.arc(0, -34, wr, 0, TAU); ctx.stroke();
      for (let i = 0; i < 3; i++) { const a2 = rot + i * TAU / 3; ctx.beginPath(); ctx.moveTo(Math.cos(a2) * wr, -34 + Math.sin(a2) * wr); ctx.lineTo(-Math.cos(a2) * wr, -34 - Math.sin(a2) * wr); ctx.stroke(); }
      ctx.beginPath(); ctx.moveTo(-9, 0); ctx.lineTo(-9, 7); ctx.moveTo(9, 0); ctx.lineTo(9, 7); ctx.moveTo(-13, 7); ctx.lineTo(13, 7); ctx.stroke();   // shaft mouth
      for (let i = 0; i < 2; i++) { const kk = (t * 0.35 + i * 0.5) % 1;                                              // ◈ glints rising from the shaft
        ctx.globalAlpha = 0.55 * (1 - kk); const gy = 4 - kk * 26, gs = 2.4;
        ctx.beginPath(); ctx.moveTo(0, gy - gs); ctx.lineTo(gs, gy); ctx.lineTo(0, gy + gs); ctx.lineTo(-gs, gy); ctx.closePath(); ctx.stroke(); }
      ctx.restore();
    }
    ctx.restore();
  }
