...
(() => {
  ...
  function update(dt) {
    ...
    earnT += dt; if (earnT >= 1) { cps = cps * 0.6 + (earnAcc / earnT) * 0.4; earnAcc = 0; earnT = 0;
      S.cpsS = Math.max((S.cpsS || 0) * 0.9 + cps * 0.1, cps * 0.5); }   // v17.19: the unit-price anchor — slow (τ≈10s, jackpot spikes wash out) but floored at half the live rate (a reload or stale save can't buy an army at yesterday's prices)
