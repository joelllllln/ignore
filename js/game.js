...
(() => {
  ...
  function update(dt) {
    ...
    { const bgSum = empireIdleRate(); if (bgSum > 0) { const add = bgSum * dt; S.cash = Math.max(S.cash, Math.min(derived.capacity, S.cash + add)); S.totalRun += add; META.totalEver += add; earnAcc += add;   // v17.21 audit: empire income now counts in cps — the boss bounty floor AND the unit-price anchor see your TRUE throughput (it was invisible, undercounting empire-heavy saves; onearmy-sim already modeled it this way)
    ...
    const stats = "<div class='gi-unlock'>" + curSym(g) + " <b>" + curName(g) + "</b>" +
      ...
      (pv && pv.conquered && !pv.mine
        ? "<br>" + (S.cash >= mineCost(g)
            ? "<button id='gi-mine'>⛏ BUILD ◈ CORE MINE · " + curSym(g) + " " + fmt(mineCost(g)) + "  (" + fmtMineRate(mineRate(g)) + ")</button>"
            : "<button id='gi-mine' class='gi-lock' disabled>⛏ ◈ mine · need " + curSym(g) + " " + fmt(mineCost(g)) + "</button>")
        : "") +
      "<br>" + prog + "</div>";
