...
(() => {
  ...
  let lastAttnT = performance.now();   // v17.27: last REAL user input (pointer/key) — the attention gate for conquer-bar pace
  ...
  // v17.19 INCOME-ANCHORED ARMY (owner call: "on P2 the new mortars cost ~600 while I earn 50k/s —
  // this needs a serious look"). ROOT CAUSE: pre-v17 every planet was a fresh start, so a unit price
  // keyed to the planet's currency scale (eco) matched a fresh wallet. ONE ARMY made your income
  // carry the whole investment multiplier from world to world — eco-anchored units became one-tap
  // trinkets the moment you land. A unit now costs REAL TIME AT YOUR OWN THROUGHPUT: each class is
  // priced in seconds-of-your-live-income (deeper classes ask more seconds; each copy ×1.5 as
  // before), floored by the old eco formula so the virgin P1 start is untouched. The anchor is a
  // SLOW-smoothed income tracker persisted in the save (S.cpsS) — wheel jackpots can't spike prices
  // and a page reload can't reset them.
  const UNIT_SECS = { turret: 10, mortar: 18, plasma: 30, laser: 45, railgun: 70, nova: 100, drone: 10, swarm: 18, collector: 30, magnet: 45, tractor: 70, singularity: 100, wormhole: 130 };
  // v17.24 (owner call: "drones are too expensive on P1, why is it 20k"): the income anchor exists
  // because income CARRIES across planets — but on virgin P1 nothing has carried yet, and the eco
  // pricing was always tuned for exactly that stage (drone 125, turret 125…). The anchor now only
  // engages once you've REACHED P2 (peakGalaxy ≥ 2); P1 of every run — first or post-ascension —
  // plays the classic tuned arc.
  // Anchors price against FARMED income — live income ÷ the prestige Engine (v17.25): within-run
  // farming can never turn prices into chips, while each Ascension legitimately melts them (the whoosh).
  const farmCps = () => (S.cpsS || 0) / (pk().income || 1);
  const unitBuyCost = type => Math.ceil(Math.max(
    ecoCost() * (UNIT_FACTOR[type] || 40) * BUY_MUL,
    (S.peakGalaxy || 1) >= 2 ? farmCps() * (UNIT_SECS[type] || 30) : 0
  ) * Math.pow(1.5, countType(type)) * pk().cost * TEST_MUL());   // frontier eco floor OR seconds-of-income, whichever is REAL money to you now; × Ascension cost-reduction perk
  ...
  // v17.26 (owner call: "arriving at a new planet should be a LOT more painful — no upgrade spam,
  // more like wait ~30 seconds per upgrade"): anchor 0.03 → 0.1s — a carried mid-arc level now costs
  // ~30s of farmed income at arrival, escalating ×1.42 per level into minutes. The landing is a real
  // rebuild, not a shopping spree; the Engine still melts it on ascension replays.
  const upCost = u => Math.ceil(Math.max(ecoCost() * 2, (S.peakGalaxy || 1) >= 2 ? farmCps() * 0.1 : 0) * BUY_MUL * Math.pow(u.mul, ecoLv(u.id)) * pk().cost * (UP_DISC[u.id] || 1) * TEST_MUL());
  ...
  const TRAVEL_SECS = 420;
  const travelCost = g => { g = g || S.galaxy;
    return Math.round(Math.min(eco(g) * TRAVEL_COST_K, Math.max(eco(g) * 2e5, farmCps() * TRAVEL_SECS)) * TEST_MUL()); };
  ...
  const IDLE_FRAC = 0.02;       // v17.27 (owner call, Cookie-Clicker rule): money always flows at FULL rate, but the CONQUER BAR only crawls without you — this is the one idle pace, applied to the empire trickle, to OFFLINE earnings, and to an unattended open tab. Pure-AFK conquest ≈ 50× the designed active hours (days early, MONTHS at the frontier); any real play instantly runs at full speed on top.
  const idleBarRate = () => IDLE_FRAC * ACTIVE_REF * eco(S.galaxy) * (S.conquest || 1);   // the bar's max fill per second when you're not actually here
  ...
  let dots = [], orbs = [], beams = [], shells = [], drones = [], spawnAcc = 0, cps = 0, cpsBuf = [], earnAcc = 0, earnT = 0, curEarned = 0, bossAcc = 0;
  ...
  function ascend() {
    ...
    curEarned = 0; earnAcc = 0; earnT = 0; cps = 0; cpsBuf = []; spawnAcc = 0; bossAcc = 0; autoAcc = 0;
    ...
    try {
      ...
      if (d) {
        if (d.S) { S = Object.assign(fresh(), d.S); S.lv = Object.assign(fresh().lv, d.S.lv || {}); if (!S.units || !S.units.length) S.units = [newUnit("turret")]; S.units.forEach(u => { u.cd = u.cd || 0; }); if (!S.classNodes || typeof S.classNodes !== "object") S.classNodes = {}; ALL_TYPES.forEach(t => { if (!S.classNodes[t]) S.classNodes[t] = {}; }); if (!Array.isArray(S.collectors) || !S.collectors.length) { const n = 1 + (d.S.lv && d.S.lv.drones || 0); S.collectors = []; for (let i = 0; i < n; i++) S.collectors.push({ type: "drone" }); } S.units = capList(S.units); S.collectors = capList(S.collectors);
          ...
          S.cash = num(S.cash, 0); S.cpsS = num(S.cpsS, 0, 1e30); S.totalRun = num(S.totalRun, 0); S.runSec = num(S.runSec, 0);   // cpsS caps at 1e30 — far beyond any legitimate economy, and the rise-only ratchet means an absurd plant would otherwise stick forever
          ...
          S.conquest = num(S.conquest, 1) || 1; for (const k in S.lv) S.lv[k] = Math.round(num(S.lv[k], 0, 1e6)); }
  ...
  function applyAway(e) {
    ...
    if (offTotal > 0) {
      ...
      if (!pmv.conquered) { pmv.earned = Math.min(conquerTarget(S.galaxy), (pmv.earned || 0) + Math.min(offTotal, idleBarRate() * e)); curEarned = pmv.earned; }   // v17.27: offline money banks in FULL, but the conquer bar only takes the idle trickle — you can't finish a campaign in your sleep (line used to credit the full offTotal, contradicting the IDLE_FRAC design)
  ...
  function update(dt) {
    ...
    recompute();
    const ceStart = curEarned;   // v17.27: snapshot the conquer bar — if you're not actually here, this frame's fill gets clamped to the idle trickle at the end of the tick
    ...
    // v17.27 ATTENTION GATE (owner call — the Cookie-Clicker rule): no input for 90s = the field still
    // earns FULL money, but the conquer bar accepts at most the idle trickle. Any tap/key instantly
    // restores full pace. (Auto-buy is idle automation and deliberately does NOT count as attention.)
    if (performance.now() - lastAttnT > 90000 && curEarned > ceStart) {
      const pmA = planetMeta(S.galaxy);
      if (!pmA.conquered) curEarned = Math.min(curEarned, ceStart + idleBarRate() * dt);
    }
    ...
    earnT += dt; if (earnT >= 1) { cps = cps * 0.6 + (earnAcc / earnT) * 0.4; earnAcc = 0; earnT = 0;
      // v17.20 (audit): the price anchor is the MINIMUM cps of the last 10s, RATCHETED (rise-only per
      // run). Min-of-window = boss/wheel jackpot spikes can never inflate prices (a spike can't be
      // the minimum); rise-only = parking on a weak old planet to deflate your income can never
      // DEFLATE prices either (the v17.19 blend could be farmed down ~90% in 30s). Resets with the
      // run on ascend; reloads keep the saved anchor and the ratchet means they can only raise it.
      cpsBuf.push(cps); if (cpsBuf.length > 10) cpsBuf.shift();
      const anc = Math.min.apply(null, cpsBuf);
      if (!isFinite(S.cpsS) || S.cpsS < 0) S.cpsS = 0;   // heal any junk that slipped past load sanitation — Infinity would otherwise win every ratchet compare forever
      if (anc > (S.cpsS || 0)) S.cpsS = anc; }
  ...
  function syncHUD() {
    ...
    if (S.travel) { enroute = true; const remain = Math.max(0, S.travel.dur - S.travel.t), fin = skipToFinishCost();
      ...
      if (performance.now() - lastAttnT > 90000) label += "  ·  ⏸ IDLE — TAP TO PUSH";   // v17.27: the bar is at idle trickle; money still flows in full
    }
    const bt = $("btn-travel");
  ...
  function Audio_saber(x, y) {
    ...
    if (!Saber.g) {   // ignite
      ...
      const fil = a.createBiquadFilter(); fil.type = "lowpass"; fil.Q.value = 4.5; fil.frequency.value = 120;
      const o1 = a.createOscillator(); o1.type = "sawtooth"; o1.frequency.value = 46;
      const o2 = a.createOscillator(); o2.type = "sawtooth"; o2.frequency.value = 46; o2.detune.value = 9;
      const sub = a.createOscillator(); sub.type = "sine"; sub.frequency.value = 23;
    ...
    if (speed != null) {   // params update once per stroke window — between windows the hum just holds
      ...
      Saber.fil.frequency.setTargetAtTime(120 + k * 720 + flip * 360, t, flip > 0.4 ? 0.02 : 0.05);   // voom stays properly DARK — capped ~1.2kHz even flat-out (v17.18: darker + quieter, owner call)
  ...
  function renderAscend() {
    ...
    const srcHtml = '<div class="asc-src"><div class="asc-src-h">WHERE THE \u25c8 COME FROM</div>'
      + '<p class="muted asc-how">You earn \u25c8 cores <b>only at the moment you ascend</b> \u2014 nothing drips in over time. Each planet <b>conquered this run</b> banks a fixed bounty that grows ~\u00d71.3 per planet deeper (P1 +' + coreVal(1) + ' \u00b7 P5 +' + coreVal(5) + ' \u00b7 P10 +' + coreVal(10) + ' \u00b7 P18 +' + coreVal(18) + '). The planet you\'re still fighting adds up to <b>half</b> its bounty, scaled by its conquer bar.</p>'
  ...
  document.addEventListener("pointerdown", () => { lastAttnT = performance.now(); }, { capture: true, passive: true });   // v17.27: any real touch/click = attention (auto-buy deliberately excluded)
  document.addEventListener("keydown", () => { lastAttnT = performance.now(); }, { capture: true });
