...
(() => {
  ...
  function ascend() {
    ...
    if (!confirm("ASCEND?\n\nBank +" + pend + " \u25c8 cores" + (conqueredCount() < 3 ? " (deeper worlds pay exponentially more \u2014 P1 " + coreVal(1) + ", P5 " + coreVal(5) + ", P10 " + coreVal(10) + ")" : "") + ".\nPlanets, units, trees and cash RESET.\nCores and your Engine levels stay.")) return;
  ...
  function ascProductionHtml() {
    ...
    html += '<div class="av-tiles">'
      ...
      + ascTile("◈ PENDING", "+" + fmt(pend), pendSrc())
  ...
  function renderAscend() {
    ...
    const conqN2 = conqueredCount(), locked = false;   // v18.44: no floor — ascend whenever you like
    ...
    // v18.44: the ONLY thing that can grey this out now is having nothing to bank.
    let html = ascProductionHtml() + '<button class="big asc-go" id="ascend-go"' + (pend < 1 ? " disabled" : "") + '>'
      + (pend < 1 ? "\u25c8 NOTHING TO BANK YET \u2014 TAKE A WORLD" : "\u25c8 ASCEND NOW \u2014 BANK +" + fmt(pend)) + '</button>'
      + coach
      + srcHtml
