...
(() => {
  ...
  const unitBuyCost = type => Math.ceil(Math.max(
    ...
    (S.peakGalaxy || 1) >= 2 ? (S.cpsS || 0) * (UNIT_SECS[type] || 30) : 0
  ...
  const travelCost = g => { g = g || S.galaxy;
    return Math.round(Math.min(eco(g) * TRAVEL_COST_K, Math.max(eco(g) * 2e5, (S.cpsS || 0) * TRAVEL_SECS)) * TEST_MUL()); };
