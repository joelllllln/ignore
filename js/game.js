...
(() => {
  ...
  function objectiveNow() {
    ...
    if (revealed("eco") && !(S.lv && (S.lv.value | 0) > 0))
      return { t: "Buy a VALUE upgrade", s: "ECONOMY tab — it multiplies every dot you kill", go: "upgrades" };
  ...
  function syncHUD() {
    ...
    { const uc = $("up-cash"); if (uc) { const t = curSym(S.galaxy) + " " + fmt(S.cash); if (uc.textContent !== t) uc.textContent = t; }
      const up = $("up-cps"); if (up) { const t2 = "+" + curSym(S.galaxy) + fmt(cps) + "/s"; if (up.textContent !== t2) up.textContent = t2; } }
    ...
    { const setVis = (el, on) => { if (el && (el.style.display === "none") === on) el.style.display = on ? "" : "none"; };
      const dt = tabBtns.drone, et = tabBtns.eco;
      setVis(dt && (dt.closest(".tslot") || dt), revealed("collect"));
      setVis(et && (et.closest(".tslot") || et), revealed("eco"));
    ...
    { const nv = $("nav"), root = (typeof document !== "undefined" && document.documentElement) || null;
      ...
      if ((activeTab === "drone" && !revealed("collect")) || (activeTab === "eco" && !revealed("eco"))) {
    ...
    { const ob = $("objective"), o = objectiveNow();
      if (ob) { const on = !!o && !conq0Hide();
        ...
        if (on) { const t = "\u25b8 " + o.t, sub = o.s + (o.go ? "  ·  tap to open" : "");
          ob.onclick = o.go ? () => navGo(o.go) : null;
          ob.classList.toggle("tappable", !!o.go);
  ...
  function renderList() {
    ...
    if (activeTab === "def" || activeTab === "drone") {
      ...
      if (hidden > 0) { const n = document.createElement("div"); n.className = "up-more";
        n.textContent = "▸ " + hidden + " more " + (activeTab === "def" ? "defender" : "collector") + " class" + (hidden > 1 ? "es" : "") + " unlock as you travel";
        wrap.appendChild(n); }
    } else {
  ...
  { const uc = $("up-close"); if (uc) uc.onclick = () => navGo("play"); }
  ...
  const TUT_STEPS = [
    ...
    { sel: '#nav .nv[data-nav="upgrades"]', t: "Everything you buy", x: "Defenders, collectors and economy all live in <b>UPGRADES</b>. Open it and the line at the top always tells you <b>what to buy next</b>." },
  ];
  const Tut = {
    i: 0,
