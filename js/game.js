...
(() => {
  ...
  const mineRateTotal = () => { let r = 0; if (S && S.vault) for (const k in S.vault) if (S.vault[k] && S.vault[k].conquered) r += mineRate(+k); return r; };
  ...
  function showGalaxyInfo(g) {
    ...
    const stats = "<div class='gi-unlock'>" + curSym(g) + " <b>" + curName(g) + "</b>" +
      (pv && pv.conquered ? " · <b>+" + fmt(pv.bgRate || 0) + "/s</b> idle · <b>◈ mine " + fmtMineRate(mineRate(g)) + "</b>" : "") +
      "<br>" + prog + "</div>";
