...
(() => {
  ...
  function syncHUD() {
    ...
    { const setVis = (el, on) => { if (el && (el.style.display === "none") === on) el.style.display = on ? "" : "none"; };
      ...
      setVis($("abilities"), revealed("abil") && !settleShown);   // v18.62: one owner — see the settle block below
  ...
  // v18.71: the detail panel is a FLEX ITEM in the tree column now, not a floating overlay — see
  // .st-info in style.css. That means showing or hiding it changes the CANVAS height, and the canvas
  // only re-measures on a window resize. Re-resize on every toggle or the tree draws at the old
  // scale (nodes drift off their own hit boxes) until you rotate the phone.
  const stPanel = on => { const p = $("st-info"); if (!p || !p.classList) return;
    const was = p.classList.contains("show");
    p.classList.toggle("show", on);
    if (was !== on) STree.resize(); };
