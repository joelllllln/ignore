...
(() => {
  ...
  const UPS = [
    ...
    { id: "value",     tab: "eco", name: "Value",      base: 130, mul: 1.37, desc: () => "×" + derived.valueMul.toFixed(2) + " /dot" },
    { id: "spawnRate", tab: "eco", name: "Spawn Rate", base: 60, mul: 1.39, desc: () => { const raw = derived.spawnPerSec || 0, om = spawnOver(raw); return raw.toFixed(1) + " /s" + (om > 1.02 ? "  ·  ~" + spawnVis(raw).toFixed(0) + "/s on screen + ×" + om.toFixed(1) + " tougher" : ""); } },
  ...
  let comboMul = 1, comboT = 0, comboFxT = 0, comboGain1s = 0;
    ...
    try {
      ...
      if (d) {
        ...
        if (d.S) { S = Object.assign(fresh(), d.S); S.lv = Object.assign(fresh().lv, d.S.lv || {});
          ...
          S.conquest = num(S.conquest, 1) || 1; for (const k in S.lv) S.lv[k] = Math.round(num(S.lv[k], 0, 2000));   // level cap 2000, NOT 1e6: mul^lv overflows double past ~2500 levels (1.32^1e6 = Infinity → Infinity prices, fuzzer finding); legitimate campaigns top out ~120
  ...
  function update(dt) {
    ...
    { const pm = planetMeta(S.galaxy); if (!pm.conquered && curEarned >= conquerTarget(S.galaxy)) { pm.conquered = true; pm.bgRate = Math.max(pm.bgRate || 0, baseTarget(S.galaxy) / (IDLE_PAYBACK_H * 3600)); pm.spoils = Math.round(SETTLE_SPOILS_FRAC * conquerTarget(S.galaxy)); const cg = coreVal(S.galaxy); recompute(); floatTxt(W / 2, H / 2 - 40, "✦ PLANET CONQUERED  ·  TRAVEL UNLOCKED"); floatTxt(W / 2, H / 2 - 16, "+" + cg + " ◈ PENDING — Ascension charging (" + pendingCores() + " ready)"); floatTxt(W / 2, H / 2 + 8, "⚑ VICTORY SPOILS — settlement pays ×" + SETTLE_ON_SITE + " while " + curSym(S.galaxy) + " " + fmt(pm.spoils) + " lasts"); flashAdd(0.5); shakeAdd(4); vibe([40, 30, 90]); Audio_conquer(); syncHUD();
  ...
  function render() {
    ...
    if (state === "play" && comboMul > 1.02 && (!planetMeta(S.galaxy).conquered || wardenOn)) {
      const cm01 = (comboMul - 1) / 4, draining = comboT <= 0;
      ...
      ctx.globalAlpha = draining ? 0.38 : 0.85;
      ctx.font = "800 15px 'Space Grotesk', system-ui, sans-serif";
      ctx.fillText("☝ COMBO ×" + comboMul.toFixed(1) + (comboMul >= 4.99 ? " — MAX" : ""), SW / 2, cy);
      ctx.globalAlpha = draining ? 0.25 : 0.6;
      ctx.fillStyle = "rgba(255,255,255,0.18)"; ctx.fillRect(SW / 2 - 62, cy + 7, 124, 5);
      ctx.fillStyle = "#fff"; ctx.fillRect(SW / 2 - 62, cy + 7, 124 * cm01, 5);
      ctx.restore();
    }
