...
(() => {
  ...
  // v17.28 FULL RE-BASELINE (owner call: "AFK farming should be allowed at FULL rate everywhere —
  // the PACING must make static conquest so slow that upgrading is always the quicker path. Stop
  // patching bad pacing with gates."). The v17.2 ask was literally "level 50 spawn should be like
  // level 1 on the new planet" — but the fixed 16/12-per-planet strip left most of a farmed economy
  // intact on arrival, so a static army landed at ~1/5 of end-of-planet income and could AFK a
  // conquest in days. Now landing on a NEW FRONTIER snapshots your eco levels as the new ZERO:
  // effective Value/Spawn start at 0 on every new world and the whole in-planet climb (×15-30
  // income) is the engine. Static-AFK conquest = designed hours × that growth factor — obviously
  // slower than upgrading, organically, with zero special-casing of idle play. Levels themselves
  // still persist (they are the ledger the snapshot subtracts from); ECO_BASE is retained as the
  // LEGACY rate solely to migrate old saves into an equivalent snapshot.
  const ECO_BASE = { value: 16, spawnRate: 12 };
  const ecoSnap = () => S.ecoSnap || (S.ecoSnap = { value: 0, spawnRate: 0 });
  const ecoLv = id => { if (!(id in ECO_BASE)) return S.lv[id] || 0; return Math.max(0, (S.lv[id] || 0) - (ecoSnap()[id] || 0)); };
  ...
  // v17.30 (owner call: "the launch price rises the more I make — I can never reach it"): the quote
  // FREEZES at the moment you conquer (when launching becomes possible). Saving toward it is a fixed
  // finish line; growing income only gets you there faster. (This also matches what onearmy-sim
  // always modeled — the live game was the one re-pricing against the ratchet.) Lazy-stamped so
  // migrated saves quote once on first look.
  const launchPrice = () => { const pm = planetMeta(S.galaxy);
    if (!pm.conquered) return travelCost();
    if (!(+pm.launchQuote > 0)) pm.launchQuote = travelCost();
    return pm.launchQuote; };
  ...
  function fresh() {
    ...
    return { cash: Math.floor(eco(1) * startMul(1)), galaxy: 1, lv, ecoSnap: { value: 0, spawnRate: 0 }, classNodes, units: [newUnit("turret")], collectors: [{ type: "drone" }], totalRun: 0, peakGalaxy: 1, runSec: 0, vault: {}, travel: null, conquest: 1, victory: false, auto: defaultAuto() };
  }
    ...
    try {
      ...
      if (d) {
        if (d.S) { S = Object.assign(fresh(), d.S); S.lv = Object.assign(fresh().lv, d.S.lv || {}); if (!S.units || !S.units.length) S.units = [newUnit("turret")]; S.units.forEach(u => { u.cd = u.cd || 0; }); if (!S.classNodes || typeof S.classNodes !== "object") S.classNodes = {}; ALL_TYPES.forEach(t => { if (!S.classNodes[t]) S.classNodes[t] = {}; }); if (!Array.isArray(S.collectors) || !S.collectors.length) { const n = 1 + (d.S.lv && d.S.lv.drones || 0); S.collectors = []; for (let i = 0; i < n; i++) S.collectors.push({ type: "drone" }); } S.units = capList(S.units); S.collectors = capList(S.collectors);
          ...
          if (!S.ecoSnap || typeof S.ecoSnap !== "object") S.ecoSnap = { value: Math.min(S.lv.value || 0, 16 * (S.peakGalaxy - 1)), spawnRate: Math.min(S.lv.spawnRate || 0, 12 * (S.peakGalaxy - 1)) };   // v17.28 migration: old saves get the LEGACY per-planet strip as their snapshot — effective levels unchanged on update
          else { S.ecoSnap.value = num(S.ecoSnap.value, 0, 1e6); S.ecoSnap.spawnRate = num(S.ecoSnap.spawnRate, 0, 1e6); } }
  ...
  function update(dt) {
    ...
    { const pm = planetMeta(S.galaxy); if (!pm.conquered && curEarned >= conquerTarget(S.galaxy)) { pm.conquered = true; pm.launchQuote = travelCost(S.galaxy); pm.bgRate = Math.max(pm.bgRate || 0, baseTarget(S.galaxy) / (IDLE_PAYBACK_H * 3600)); const cg = coreVal(S.galaxy); recompute(); floatTxt(W / 2, H / 2 - 40, "✦ PLANET CONQUERED  ·  TRAVEL UNLOCKED"); floatTxt(W / 2, H / 2 - 16, "+" + cg + " ◈ PENDING — Ascension charging (" + pendingCores() + " ready)"); flashAdd(0.5); shakeAdd(4); vibe([40, 30, 90]); Audio_conquer(); syncHUD();
  ...
  function activatePlanet(g) {   // point the ONE ARMY at planet g — fleet, trees, levels and cash all arrive with you
    ...
    if (g > S.peakGalaxy) S.ecoSnap = { value: S.lv.value || 0, spawnRate: S.lv.spawnRate || 0 };   // v17.28: a NEW frontier re-baselines the whole economy — carried Value/Spawn are the new zero
