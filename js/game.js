...
(() => {
  ...
  function ascVerdict(pend) {
    const eta = wallEtaH();
    if (pend < 1) return { kind: "none", line: "Take a planet first — conquering is what fills the ◈ bank." };
    if (!isFinite(eta)) return { kind: "wait", line: "No income on this planet yet. Get the guns paying before you call it." };
    if (eta <= 0) return { kind: "go", line: "This planet is yours. Every planet you hold pays its bounty the moment you hop." };
    if (eta > ASC_HOP_H) return { kind: "go", line: "THE WALL — this bar needs ≈ " + fmtHrs(eta) + " more at your income. Hopping beats grinding it." };
    return { kind: "wait", line: "This bar lands in ≈ " + fmtHrs(eta) + " — take it first. Deeper planets bank far more ◈." };
  }
  ...
  const ASC_LOOP = [
    ...
    ["gem",    "PLANETS PAY ◈",  "deeper = far more"],
  ...
  function renderAscend() {
    ...
    const html = ascLoopHtml()
      ...
      + ascSplitHtml()
      + ascSourcesHtml()
      + '<p class="muted ax-ladder">Hop at ~3 planets on run 1, then a few deeper every time. Grinding a wall is never faster than hopping it.</p>';
