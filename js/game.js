...
(() => {
  ...
  let DISC = { cx: 0, cy: 0, r: 200 };
  const discCX = () => DISC.cx, discCY = () => DISC.cy, discR = () => DISC.r;
  function fitDisc() {
    ...
    const z = camFit || 1;
    // 0.86 leaves a real margin of orbital VOID around the limb at full zoom-out — the corners the
    // owner asked to see filled, rather than a circle jammed against the screen edges
    DISC.r = Math.max(24, Math.min(SW, bandH) / 2 * 0.80 / z);   // no meaningful floor: on a landscape phone the band is genuinely short, and any floor big enough to matter pushes the limb straight through the dock
    DISC.cx = W / 2;
    DISC.cy = H / 2 + (bandCY - SH / 2) / z;
  }
  ...
  const ZOOM_OUT = 1.0;       // v17.10 (owner call: "I can still zoom the map out past the edges on phone"): the camera now STOPS at "fit the whole world" — no more pulling back into the void beyond the border. The old 0.55 margin existed to see the spawn line with room; since v17.8 the rim terrain + spawn band + border all fit exactly at camFit, so the margin only exposed empty space
  ...
  function bossMove(d, dt) {
    ...
    const pr = discR() - 60, cx = discCX(), cy = discCY(), L = cx - pr, R = cx + pr, T = cy - pr, B = cy + pr;
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    ...
    if (fromTop) {   // Dot Rain falls INTO the bowl — spread over the disc, not piled on one line
      const ra = Math.random() * TAU, rr = discR() * Math.sqrt(Math.random()) * 0.82;
      ...
      const p2 = discPt(a, discR() - 30 - rnd(0, 22));
      ex = p2.x; ey = p2.y;
    }
  ...
  function update(dt) {
    ...
    for (const dr of drones) if (dr.wing === 0) {
      ...
      discClamp(h, 34);   // v18.26: the hive roams the disc, not the box
    }
  ...
  let envFor = 0, envW = 0, envH = 0, envR = 0, envAnchors = [], envDust = [], envMotes = [], envDash = 12, envBack = [], envRidge = [];
  // v18.26 PER-PLANET BIOMES (owner: "I need a full rework — make each planet unique with a REAL cool
  // design, not just the same shape repeated over and over again"). A world's horizon is no longer one
  // motif stamped N times around the edge. Each planet composes its own landscape from THREE things
  // that all differ planet to planet:
  //   mix    — 2-3 DIFFERENT motifs woven together (its own signature form plus borrowed ones), so a
  //            horizon reads as an ecosystem instead of a picket fence
  //   layout — how they're distributed around the rim: evenly, in clumps with empty stretches, packed
  //            into one dense arc, two opposing bands, or sparse-and-monumental
  //   ridge  — the terrain silhouette itself: jagged, rolling dunes, spikes, low flats, or broken gaps
  // plus SIGNATURE structures: one to three vast bespoke formations per world, drawn at 3.5x with
  // their own survey rings, that anchor the skyline and make each planet instantly recognisable.
  const BIOMES = [null,
    { mix: [1, 16],      layout: "clumps", dens: 1.00, ridge: "jag",    sig: 1 },   // P1  Vesta    — cratered moonscape pocked with burrows
    { mix: [2, 14],      layout: "arcs",   dens: 1.10, ridge: "shard",  sig: 2 },   // P2  Ember    — cinder cones over ash pyres
    { mix: [3, 2, 15],   layout: "twin",   dens: 1.20, ridge: "broken", sig: 1 },   // P3  Cinder   — slag fissures, vents, void wells
    { mix: [4, 7],       layout: "even",   dens: 1.00, ridge: "dune",   sig: 1 },   // P4  Hearth   — tidal pools in reed flats
    { mix: [5, 13],      layout: "clumps", dens: 1.25, ridge: "jag",    sig: 2 },   // P5  Azure    — forest stands with wisp markers
    { mix: [6, 5],       layout: "arcs",   dens: 1.15, ridge: "dune",   sig: 1 },   // P6  Verdant  — pylon line through woodland
    { mix: [7, 12],      layout: "even",   dens: 0.90, ridge: "flat",   sig: 2 },   // P7  Cobalt   — reed plain under monoliths
    { mix: [8, 9],       layout: "sparse", dens: 0.75, ridge: "dune",   sig: 1 },   // P8  Mistral  — mirage arches, storm rods
    { mix: [9, 8, 13],   layout: "twin",   dens: 1.05, ridge: "shard",  sig: 2 },   // P9  Halcyon  — rod forest and arches
    { mix: [10, 9],      layout: "clumps", dens: 1.20, ridge: "jag",    sig: 1 },   // P10 Tempest  — rift clusters, lightning rods
    { mix: [11, 10],     layout: "arcs",   dens: 1.00, ridge: "broken", sig: 2 },   // P11 Umbral   — shadow rifts, shards
    { mix: [12, 11],     layout: "sparse", dens: 0.80, ridge: "shard",  sig: 1 },   // P12 Frost    — monoliths on an ice shelf
    { mix: [13, 12, 1],  layout: "even",   dens: 1.10, ridge: "flat",   sig: 2 },   // P13 Onyx     — marker fields, broken stone
    { mix: [14, 15],     layout: "clumps", dens: 1.15, ridge: "broken", sig: 1 },   // P14 Wraith   — pyres around void wells
    { mix: [15, 14, 3],  layout: "twin",   dens: 1.05, ridge: "jag",    sig: 2 },   // P15 Pyreling — wells, pyres, fissures
    { mix: [16, 17],     layout: "arcs",   dens: 1.20, ridge: "dune",   sig: 1 },   // P16 Abyssal  — burrow fields, null gates
    { mix: [17, 16, 6],  layout: "even",   dens: 1.10, ridge: "shard",  sig: 2 },   // P17 Devourer — gates, maws, pylons
    { mix: [18, 15, 11], layout: "sparse", dens: 0.90, ridge: "broken", sig: 3 },   // P18 Null     — monumental, almost empty
  ];
  const biomeOf = g => BIOMES[clamp(g | 0, 1, 18)] || BIOMES[1];
  function ensureEnv() {
    if (envFor === S.galaxy && envW === W && envH === H && envR === DISC.r && envAnchors.length) return;
    envFor = S.galaxy; envW = W; envH = H; envR = DISC.r; envAnchors = []; envDust = []; envMotes = [];
    ...
    // how many features, and WHERE around the ring — the layout is what makes two worlds with similar
    // motifs still read as different places
    const n = Math.max(10, Math.round((TAU * R0 / 118) * B.dens));
    const angles = [];
    if (B.layout === "even") { for (let i = 0; i < n; i++) angles.push((i + 0.25 + er() * 0.5) / n * TAU); }
    else if (B.layout === "clumps") { const cl = 3 + Math.floor(er() * 3);
      for (let c = 0; c < cl; c++) { const c0 = (c + er() * 0.4) / cl * TAU, w = 0.30 + er() * 0.30, m = Math.ceil(n / cl);
        for (let i = 0; i < m; i++) angles.push(c0 + (er() - 0.5) * w); } }
    else if (B.layout === "arcs") { const a0 = er() * TAU, span = 1.5 + er() * 0.8;
      for (let i = 0; i < n; i++) angles.push(er() < 0.72 ? a0 + er() * span : er() * TAU); }   // one packed belt, a scatter elsewhere
    else if (B.layout === "twin") { const a0 = er() * TAU;
      for (let i = 0; i < n; i++) { const band = i % 2 ? a0 : a0 + Math.PI; angles.push(band + (er() - 0.5) * 1.5); } }
    else { for (let i = 0; i < n * 0.5; i++) angles.push((i + er()) / (n * 0.5) * TAU); }        // sparse: fewer, bigger
    const big = B.layout === "sparse";
    for (const a of angles) {
      const lm = er() < 0.19;
      envAnchors.push({ a, rad: R0 - 14 - er() * 20, s: (big ? 2.6 : lm ? 3.1 : 1.7) + er() * 1.2,
        v: er(), p: er() * TAU, lm, f: B.mix[Math.floor(er() * B.mix.length)] });
    }
    // SIGNATURE FORMATIONS — one to three colossal structures per world, spaced apart, each ringed by
    // its own survey circle. These are the silhouettes you remember a planet by.
    for (let i = 0; i < B.sig; i++)
      envAnchors.push({ a: (i / B.sig) * TAU + er() * 0.8, rad: R0 - 30 - er() * 12, s: 5.4 + er() * 1.6,
        v: er(), p: er() * TAU, lm: true, sig: true, f: B.mix[0] });
    // BACK ROW — a fainter, smaller rank pressed further out, for depth
    envBack = [];
    for (let i = 0; i < n * 0.8; i++)
      envBack.push({ a: er() * TAU, rad: R0 - 2 - er() * 8, s: 0.9 + er() * 0.7, v: er(), p: er() * TAU,
        f: B.mix[Math.floor(er() * B.mix.length)] });
    // THE RIDGE — one closed ground-line running the whole circumference, its shape set by the biome
    envRidge = [];
    { const steps = Math.max(72, Math.round(R0 / 3.2));
      for (let i = 0; i <= steps; i++) {
        const a = i / steps * TAU; let d = 0;
        if (B.ridge === "jag") d = 20 + er() * 14 + Math.sin(a * 9) * 4;
        else if (B.ridge === "dune") d = 22 + Math.sin(a * 3.1) * 11 + Math.sin(a * 7.3) * 4;
        else if (B.ridge === "shard") d = 15 + (er() < 0.45 ? 14 + er() * 16 : er() * 7);   // irregular spikes, not a milled sawtooth
        else if (B.ridge === "flat") d = 13 + er() * 5 + Math.sin(a * 5) * 2;
        else d = (Math.sin(a * 4.2) > 0.25) ? 10 + er() * 6 : 26 + er() * 16;   // broken: plateaus split by gulfs
        envRidge.push({ a, d });
      } }
    // dust & drifting motes live in the rim band
    for (let i = 0; i < Math.round(TAU * R0 / 26); i++)
      envDust.push({ a: er() * TAU, rad: R0 - 4 - er() * 34, r: 0.6 + er() * 1.5, ln: er() < 0.35 ? 2 + er() * 5 : 0, an: er() * TAU });
    for (let i = 0; i < 16; i++) envMotes.push({ off: er(), spd: 0.006 + er() * 0.014, m: 6 + er() * 26, ph: er() * TAU });
  }
  const ENV_DRAW = [null,
  ...
  // ORBITAL VOID (v18.26) — everything outside the ground disc. This is what fills the corners when
  // you pull the camera all the way back: not empty black, but the planet's surroundings — a soft
  // limb glow at the horizon, concentric survey rings, degree ticks around the circumference, and a
  // scatter of far stars. Drawn with an even-odd fill so it can never paint over the playfield.
  function drawVoid(t) {
    const cx = discCX(), cy = discCY(), R0 = discR();
    const M = Math.max(W, H) * 1.6;
    ctx.save();
    ctx.beginPath(); ctx.rect(cx - M, cy - M, M * 2, M * 2); ctx.arc(cx, cy, R0, 0, TAU, true);
    ctx.fillStyle = "#000"; ctx.fill("evenodd");
    ctx.clip("evenodd");                       // everything below is confined to the void
    ctx.strokeStyle = "#fff"; ctx.fillStyle = "#fff";
    { const g = ctx.createRadialGradient(cx, cy, R0, cx, cy, R0 * 1.5);   // atmosphere fading off the limb
      g.addColorStop(0, "rgba(255,255,255,0.055)"); g.addColorStop(1, "rgba(255,255,255,0)");
      ctx.fillStyle = g; ctx.fillRect(cx - M, cy - M, M * 2, M * 2); ctx.fillStyle = "#fff"; }
    ctx.globalAlpha = 0.26; ctx.lineWidth = 1.5;                          // the orbit ring hugging the limb
    ctx.beginPath(); ctx.arc(cx, cy, R0 + 9, 0, TAU); ctx.stroke();
    ctx.globalAlpha = 0.13; ctx.lineWidth = 1;
    for (const k of [1.07, 1.16, 1.30, 1.50]) { ctx.beginPath(); ctx.arc(cx, cy, R0 * k, 0, TAU); ctx.stroke(); }
    ctx.globalAlpha = 0.11; ctx.setLineDash([10, 16]); ctx.lineDashOffset = -t * 9;
    ctx.beginPath(); ctx.arc(cx, cy, R0 * 1.115, 0, TAU); ctx.stroke(); ctx.setLineDash([]);
    for (let i = 0; i < 72; i++) {                                        // bearing ticks, every 6th long
      const a = i / 72 * TAU, maj = i % 6 === 0, r1 = R0 + 13, r2 = R0 + (maj ? 34 : 22);
      ctx.globalAlpha = maj ? 0.34 : 0.16; ctx.lineWidth = maj ? 1.4 : 1;
      ctx.beginPath(); ctx.moveTo(cx + Math.cos(a) * r1, cy + Math.sin(a) * r1); ctx.lineTo(cx + Math.cos(a) * r2, cy + Math.sin(a) * r2); ctx.stroke();
    }
    // CORNER SECTORS — the diagonals are where the void is widest, so that is where the survey
    // furniture goes: a bracket opening away from the limb, its bearing, and a run of hatch marks
    ctx.font = "9px ui-monospace,monospace"; ctx.textAlign = "center";
    for (let q = 0; q < 4; q++) {
      const a = Math.PI / 4 + q * Math.PI / 2, ca = Math.cos(a), sa = Math.sin(a);
      const bx = cx + ca * (R0 * 1.36), by = cy + sa * (R0 * 1.36);
      ctx.save(); ctx.translate(bx, by); ctx.rotate(a + Math.PI / 2);
      ctx.globalAlpha = 0.30; ctx.lineWidth = 1.6;
      ctx.beginPath(); ctx.moveTo(-20, 9); ctx.lineTo(-20, 0); ctx.lineTo(20, 0); ctx.lineTo(20, 9); ctx.stroke();
      ctx.globalAlpha = 0.16; ctx.lineWidth = 1;
      for (let h = -14; h <= 14; h += 7) { ctx.beginPath(); ctx.moveTo(h, -5); ctx.lineTo(h, -1); ctx.stroke(); }
      ctx.globalAlpha = 0.34; ctx.fillText(String(Math.round(((a * 180 / Math.PI) + 90 + 360) % 360)).padStart(3, "0"), 0, 20);
      ctx.restore();
    }
    { const h = Math.imul((S.galaxy + 5) * 2654435761, 40503) >>> 0;      // seeded far stars, per world
      let q = h; const rr = () => { q = Math.imul(q ^ (q >>> 15), 2246822519) >>> 0; return (q >>> 9) / 8388608; };
      for (let i = 0; i < 190; i++) {
        const a = rr() * TAU, rad = R0 * (1.02 + rr() * 1.2);
        ctx.globalAlpha = 0.07 + rr() * 0.24;
        ctx.beginPath(); ctx.arc(cx + Math.cos(a) * rad, cy + Math.sin(a) * rad, 0.5 + rr() * 1.3, 0, TAU); ctx.fill();
      } }
    ctx.restore();
  }
  ...
  function drawEnv() {
    ...
    ctx.beginPath(); ctx.arc(cx, cy, R0, 0, TAU); ctx.fillStyle = "#040404"; ctx.fill();
    { const g = ctx.createRadialGradient(cx, cy, R0 * 0.55, cx, cy, R0);
      g.addColorStop(0, "rgba(255,255,255,0.016)"); g.addColorStop(0.82, "rgba(255,255,255,0.006)"); g.addColorStop(1, "rgba(0,0,0,0.5)");
      ctx.fillStyle = g; ctx.beginPath(); ctx.arc(cx, cy, R0, 0, TAU); ctx.fill(); }
    ctx.strokeStyle = "#fff"; ctx.fillStyle = "#fff";
    // THE HORIZON — the world's own boundary: a solid limb, a slowly turning survey circle inside it
    ctx.globalAlpha = 0.16; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(cx, cy, R0 - 1.5, 0, TAU); ctx.stroke();
    ctx.globalAlpha = 0.09; ctx.lineWidth = 1; ctx.setLineDash([envDash, envDash * 1.6]); ctx.lineDashOffset = -t * 6;
    ctx.beginPath(); ctx.arc(cx, cy, R0 - 9, 0, TAU); ctx.stroke(); ctx.setLineDash([]);
    // THE RIDGE — one continuous ground-line all the way around, shaped by this world's terrain style
    ctx.globalAlpha = 0.30; ctx.lineWidth = 1.6;
    ctx.beginPath();
    for (let i = 0; i < envRidge.length; i++) { const q = envRidge[i], rr = R0 - q.d;
      const x = cx + Math.cos(q.a) * rr, y = cy + Math.sin(q.a) * rr; i ? ctx.lineTo(x, y) : ctx.moveTo(x, y); }
    ctx.closePath(); ctx.stroke();
    // dust & scratches through the rim band
    ...
    for (const a of envBack) {
      ...
      ctx.globalAlpha = 0.16; fnOf(a.f)(ctx, a.s, a.v, a.p, t); ctx.restore();
    }
    ...
    for (const a of envAnchors) {
      const p2 = discPt(a.a, a.rad);
      ctx.save(); ctx.translate(p2.x, p2.y); ctx.rotate(rimRot(a.a));
      ctx.globalAlpha = a.sig ? 0.78 : a.lm ? 0.62 : 0.44;
      if (a.sig) ctx.lineWidth = 2.6;
      fnOf(a.f)(ctx, a.s, a.v, a.p, t);
      if (a.sig) {   // a signature formation gets its own survey rings + footing line
        ctx.globalAlpha = 0.20; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.arc(0, 0, 26 * (a.s / 5.4), 0, TAU); ctx.stroke();
        ctx.setLineDash([5, 7]); ctx.beginPath(); ctx.arc(0, 0, 38 * (a.s / 5.4), 0, TAU); ctx.stroke(); ctx.setLineDash([]);
        ctx.globalAlpha = 0.30; ctx.beginPath(); ctx.moveTo(-30, 0); ctx.lineTo(30, 0); ctx.stroke();
        ctx.lineWidth = 2;
      }
      ctx.restore();
    }
    // drifting motes riding the rim band
  ...
  function drawMineRig(t) {
    ...
    const R = Math.max(70, Math.min(W * 0.38, (H - 250) * 0.44)), sc = R / 100;
  ...
  function render() {
    ...
    if (state === "play" && !S.travel && planetMeta(S.galaxy).conquered && !wardenOn) {
      const v = S.vault[S.galaxy] || {}, cy = SH * 0.30, breathe = 0.85 + 0.15 * Math.sin(performance.now() / 900);
      ctx.save(); ctx.textAlign = "center"; ctx.fillStyle = "#fff";
      ctx.globalAlpha = 0.34 * breathe; ctx.font = "600 20px 'Space Grotesk', system-ui, sans-serif";
      ctx.fillText("✦ WORLD SETTLED — NO MORE ENEMIES HERE", SW / 2, cy);
      ctx.globalAlpha = 0.30; ctx.font = "500 13px 'Space Grotesk', system-ui, sans-serif";
      ctx.fillText(settleSpoils(S.galaxy) > 0 ? "⚑ victory spoils — settlement pays " + curSym(S.galaxy) + " " + fmt(settleIncomeRate()) + "/s while they last" : "settlement pays " + curSym(S.galaxy) + " " + fmt(settleIncomeRate()) + "/s — the frontier is where the money is", SW / 2, cy + 24);
      ctx.fillText(v.mine ? "⛏ core mine digging " + fmtMineRate(mineRate(S.galaxy)) + " — progress in the panel below" : "⛏ mine site ready — ▲ summon " + wardenOf(S.galaxy).n + " (win the mine) or buy it, panel below", SW / 2, cy + 43);
      ctx.fillText("launch ⟶ the next frontier when your wallet is ready", SW / 2, cy + 62);
      ctx.restore();
    }
  ...
  function resize() {
    ...
    fitDisc(); envFor = 0;                       // v18.26: re-fit the world disc to the new play band and rebuild its rim
