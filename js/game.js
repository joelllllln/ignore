...
(() => {
  ...
  const upCost = u => Math.ceil(Math.max(ecoCost() * 2, (S.peakGalaxy || 1) >= 2 ? farmCps() * 0.03 : 0) * BUY_MUL * Math.pow(u.mul, ecoLv(u.id)) * pk().cost * (UP_DISC[u.id] || 1) * TEST_MUL());
