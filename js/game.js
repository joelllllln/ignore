...
(() => {
  ...
  const krTick = () => { const now = performance.now(); if (now - krWin > 1000) { krPrev = krN; krN = 0; krWin = now; } krN++; };   // TRUE kills/s, pre-throttle
  const krRate = () => Math.max(krPrev, krN);
  function Audio_pop(big, tier) {   // a dot dies — pitched pop that climbs the combo ladder; armored/high-tier lands a deeper knock
    ...
    const d = sfxDuck();
    if (big) { sTone(190 + Math.random() * 40, 55, 0.14, "sine", 0.12 * d, 0, 0.15); sNoise(2600, 480, 0.07, 0.045 * d); return; }   // meaty knock + crack, touch of room
  ...
  function Audio_saber(x, y) {
    ...
    let speed = 0, vx = 0, vy = 0, turn = 0;
    if (Saber.lt && now - Saber.lt < 200) {
      const dtm = Math.max(8, now - Saber.lt);
      vx = (x - Saber.lx) / dtm * 1000; vy = (y - Saber.ly) / dtm * 1000;   // stroke velocity, world px/s
