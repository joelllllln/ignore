...
(() => {
  ...
  const UPS = [
    ...
    { id: "value",     tab: "eco", name: "Value",      base: 130, mul: 1.30, desc: () => "×" + derived.valueMul.toFixed(2) + " /dot" },
    { id: "spawnRate", tab: "eco", name: "Spawn Rate", base: 60, mul: 1.32, desc: () => { const raw = derived.spawnPerSec || 0, om = spawnOver(raw); return raw.toFixed(1) + " /s" + (om > 1.02 ? "  ·  ~" + spawnVis(raw).toFixed(0) + "/s on screen + ×" + om.toFixed(1) + " tougher" : ""); } },
  ...
  // supervision at ×20 the background tribute. Worst-case flat broke at conquest: launch (15% of
  // target) ≈ 12 min parked, the ⛏ mine (10%) ≈ 8 min — and typically far less, since the conquest
  // itself just paid the wallet. Off-planet it drops back to the normal ×1 empire tribute.
  const SETTLE_ON_SITE = 20;
  const settleIncomeRate = () => { const v = S.vault && S.vault[S.galaxy]; return v && v.conquered ? (v.bgRate || 0) * SETTLE_ON_SITE * pk().empire : 0; };
  ...
  function mineAccrue(sec, quiet) {   // fractional progress buffers in META.asc.mineBuf; whole cores land in the bank
    if (!META || !META.asc || !(sec > 0)) return 0;
    const r = mineRateTotal(); if (r <= 0) return 0;
    META.asc.mineBuf = (+META.asc.mineBuf || 0) + r * sec / 86400;
    const whole = Math.floor(META.asc.mineBuf); if (whole < 1) return 0;
    META.asc.mineBuf -= whole; META.asc.cores = (META.asc.cores | 0) + whole; META.asc.lifetime = (META.asc.lifetime | 0) + whole;
    if (!quiet) { floatTxt(W / 2, 120, "◈ +" + whole + " from your core mines"); Audio_tick(); }
    return whole;
  }
  ...
  function recompute() {
    ...
    derived.valueMul = (1 + 0.10 * ecoLv("value")) * pk().value;          // v18.0: +10% per GLOBAL level — fewer, chunkier levels on one continuous ladder. × Ascension value perk.
    ...
    const rawSpawn = (0.9 + 0.9 * ecoLv("spawnRate")) * pk().spawn;   // v18.0: global level, gentler slope — the ladder is continuous across the campaign
    ...
    try {
      ...
      if (d) {
        ...
        if (d.ts) { const e = clamp((Date.now() - d.ts) / 1000, 0, AWAY_CAP_H * 3600);
          ...
          const rate = (d.cps > 0 ? d.cps : 0) + (S.vault ? empireIdleRate() + settleIncomeRate() : 0), offTotal = rate > 0 ? Math.floor(rate * e) : 0;
  ...
  const Wheel = (() => {
    ...
    function apply(sg) {
      ...
      if (sg.kind === "core") { META.asc.cores = (META.asc.cores | 0) + 1; META.asc.lifetime = (META.asc.lifetime | 0) + 1; save(); return "◈ +1 CORE — banked to Ascension"; }
  ...
  function applyAway(e) {
    ...
    const rate = (cps > 0 ? cps : 0) + (S.vault ? empireIdleRate() + settleIncomeRate() : 0), offTotal = rate > 0 ? Math.floor(rate * e) : 0;   // away earnings = the on-screen $/s you were passively earning (collectors + empire) × seconds away
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    ...
    // v18.13 ARRIVAL FLOOR (owner catch: "went from planet 2 to 3 without upgrades and I'm still
    // demolishing planet 3's dots"): the bar-driven ramp used to reset menace to ZERO on every fresh
    // world, so a deep army's opening minutes were a massacre. Deeper arrivals now open with a
    // depth-scaled menace floor (P1 0 · P3 0.5 · P7+ 1.5) — the payout divisor below keys off the
    // same `men`, so tougher arrival dots pay proportionally more and income stays exactly on curve.
    const menFloor = clamp(0.25 * (S.galaxy - 1), 0, 1.5);
    const men = (pmSpawn && pmSpawn.conquered) ? 2.0 : Math.max(menFloor, 3.5 * Math.pow(clamp(curEarned / Math.max(1, conquerTarget(S.galaxy)), 0, 1), 0.7));
  ...
  function useAbility(k) {
    ...
    if (planetMeta(S.galaxy).conquered) { Audio_err(); return; }   // v18.12: NO abilities on settled worlds — there is nothing to storm, frenzy at, or vacuum; no cooldown burned on the refusal
  ...
  function update(dt) {
    ...
    const settled = planetMeta(S.galaxy).conquered;
    if (!settled) {
      spawnAcc += dt * visRate;
    ...
    { const bgSum = empireIdleRate() + settleIncomeRate(); if (bgSum > 0) { const add = bgSum * dt; S.cash = Math.max(S.cash, Math.min(derived.capacity, S.cash + add)); S.totalRun += add; META.totalEver += add; earnAcc += add;   // v17.21 audit: empire income now counts in cps — the boss bounty floor AND the unit-price anchor see your TRUE throughput. v18.9: + the settled CURRENT planet's supervised on-site tribute
  ...
  function render() {
    ...
    const n = planetMeta(S.galaxy).conquered ? 0 : S.units.length;   // v18.6 settled worlds: the army stands down — nothing to fight, nothing deployed
    for (let i = 0; i < n; i++) {
    ...
    for (const dr of drones) {
      ...
      // hive wingmate 0 also draws the shared hive radius — the roaming territory reads at a glance
      if (dr.wing === 0) { const h = hiveAnchors[dr.hive]; if (h) { ctx.strokeStyle = "rgba(255,255,255,0.07)"; ctx.lineWidth = 1; ctx.setLineDash([6, 10]); ctx.beginPath(); ctx.arc(h.x, h.y, HIVE_R, 0, TAU); ctx.stroke(); ctx.setLineDash([]); } }
    ...
    if (state === "play" && !S.travel && planetMeta(S.galaxy).conquered) {
      ...
      ctx.fillText("your settlement pays " + curSym(S.galaxy) + " " + fmt(settleIncomeRate()) + "/s to the fleet while you're parked here", SW / 2, cy + 24);
      ctx.fillText(v.mine ? "⛏ core mine digging " + fmtMineRate(mineRate(S.galaxy)) + " — progress in the panel below" : "⛏ mine site ready — build it in the panel below (" + curSym(S.galaxy) + " " + fmt(mineCost(S.galaxy)) + ")", SW / 2, cy + 43);
  ...
  function renderSettlePanel() {
    ...
    const rate = mineRateTotal();                                     // ◈/day across ALL built mines — mineBuf is one shared hopper
    const buf = clamp(+((META.asc || {}).mineBuf || 0), 0, 1);
    ...
    if (built) {
      html += '<div class="sp-mine"><div class="sp-dig">◈ CORE MINE at work — digging ' + fmtMineRate(mineRate(g)) + '</div>'
           + '<div class="sp-bar"><div class="sp-fill" style="width:' + (buf * 100).toFixed(1) + '%"></div></div>'
           + '<div class="sp-eta">next ◈ from your mines in <b>' + fmtEta(secs) + '</b></div></div>';
    } else {
      html += '<button id="sp-build" class="sp-build"' + (can ? "" : " disabled") + '>⛏ BUILD ◈ CORE MINE — ' + curSym(g) + ' ' + fmt(cost) + '</button>'
           + '<div class="sp-note">digs ' + fmtMineRate(mineRate(g)) + ' for the rest of this run — even while you’re away</div>';
    }
    html += '<div class="sp-inc">settlement pays <b>+' + curSym(g) + ' ' + fmt(settleIncomeRate()) + '/s</b> while you’re parked here'
  ...
  function syncHUD() {
    ...
    // v18.13: settled worlds swap the whole shop dock (tabs + list + abilities) for the settlement panel
    { if (conq !== settleShown) { settleShown = conq; settleKey = "";
        const sp = $("settle-panel"); if (sp) sp.style.display = conq ? "" : "none";
        for (const id of ["tabs", "up-list", "abilities"]) { const el = $(id); if (el) el.style.display = conq ? "none" : ""; } }
      if (conq) renderSettlePanel(); }
  ...
  if (typeof window !== "undefined") window.__SIM = {
    ...
    valueMul: lv => 1 + 0.10 * lv,   // v18.0 chunky global levels
