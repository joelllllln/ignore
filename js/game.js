...
(() => {
  ...
  let mineBuildT = -1, mineBuildStage = 0;
  ...
  function buildMine(g) {
    ...
    // belt, first ore. Only on the world you are standing on; a mine bought from the star map just is.
    if (g === S.galaxy) { mineBuildT = 0; mineBuildStage = 0;
  ...
  function wardenReset() {
    ...
    mineBuildT = -1; mineBuildStage = 0;   // v18.36: leaving mid-build lands you on a finished mine, never a half-built one
  }
  ...
  function drawEnv() {
    ...
    // on a settled world.
    if (mineBuilt(S.galaxy) && !wardenOn) drawMineRig(performance.now() / 1000, mineBuildP());
  }
  ...
  // Ten seconds is a lot on your fifth conquest: a tap or Esc takes the card immediately. The sequence
  // is a flourish, never a toll gate.
  // ...but only when it is actually ON SCREEN. If the player has the star map or a card open, the
  // sequence is playing behind it — their tap belongs to what they are looking at, not to a skip.
  const cineOnScreen = () => conqCardT > 0 && state === "play" && !document.querySelector(".modal.show:not(#home)");
  function skipConquestCine() {
    if (!cineOnScreen()) return false;
    const g = conqCardG, c = conqCardCores, s = conqCardSpoils;
    conqCineEnd();
    if (g === S.galaxy) Cards.push("conquest", () => showConquest(g, c, s));
    if (conqCardVic) { conqCardVic = false; Cards.push("victory", showVictory); }
    return true;
  }
  function drawConquestCine(e) {
    ...
    // ── the way out, once the film has had a moment to establish itself ────
    { const k = seg(2.2, 3.2) * out * (0.5 + 0.2 * Math.sin(e * 2.4));
      if (k > 0.02) { ctx.globalAlpha = k; ctx.fillStyle = "#fff";
        ctx.font = "600 11px ui-monospace,monospace"; ctx.textAlign = "center";
        ctx.fillText("TAP TO SKIP", cx, ch - Math.max(26, ch * 0.075)); } }
  ...
  function render() {
    ...
    drawParts();
    ctx.restore();
    if (conqCardT > 0) drawConquestCine(CONQ_CINE - conqCardT);   // v18.38: screen-space, over the world, under the DOM furniture
  ...
  function renderSettlePanel() {
    ...
    const key = [g, built ? 1 : 0, can ? 1 : 0, Math.floor(secs / 30), Math.floor(buf * 200), bp < 1 ? 1 : 0, Math.ceil(bp * 12)].join("|");
  ...
  document.addEventListener("keydown", e => {
    ...
    if (e.key === "Escape") {
      ...
      else if (skipConquestCine()) { }   // v18.38: nothing open and the conquest film is running → Esc takes the card
  ...
  // v18.38: a tap anywhere skips the conquest cinematic. Capture phase so it lands before the drawing
  // handler, and it only ever fires while the sequence is actually running.
  window.addEventListener("pointerdown", e => { if (cineOnScreen() && skipConquestCine()) e.stopPropagation(); }, true);
