...
(() => {
  ...
  function objectiveNow() {
    ...
    if (S.units.length < 2) return { t: "Buy a 2nd DEFENDER", s: "twice the guns, twice the kills" };
    if (S.collectors.length < 2 && revealed("collect"))
      return { t: "Buy a 2nd COLLECTOR", s: "cash orbs expire where they drop until one reaches them" };
    if (revealed("tree") && !treeNodeCount())
      return { t: "Spend a SKILL TREE node", s: "tap ⬆ Tree on a defender — damage, fire rate, range" };
    if (revealed("eco") && !(S.lv && (S.lv.value | 0) > 0))
      return { t: "Buy a VALUE upgrade", s: "ECONOMY tab — it multiplies every dot you kill" };
  ...
  function syncHUD() {
    ...
    { const dockCalm = conq && !wardenOn;
      if (dockCalm !== settleShown) { settleShown = dockCalm; settleKey = "";
        ...
        for (const id of ["tabs", "up-list"]) { const el = $(id); if (el) el.style.display = dockCalm ? "none" : ""; } }
  ...
  { const back = closeCards;
    closeCards = function () { back.apply(this, arguments); navAt = "play"; syncNav(); }; }
  ...
  const TUT_STEPS = [
    ...
    { sel: "#up-list", t: "Buy a defender", x: "More guns, more kills, more cash. Whenever you are unsure, the game keeps <b>one instruction</b> on screen for you — just follow it, and the rest opens up as you go." },
  ];
  const Tut = {
    i: 0,
