...
(() => {
  ...
  // ── THE ◈ CORE SILHOUETTE — one source of truth ──────────────────────────────────────────────
  // v18.63 (owner: "the ascension currency looks like a gem and then in animations it's almost kite
  // shaped — make sure it's all that thin kite shaped thing"). It really was drawn three ways: a
  // faceted flat-topped GEM as the icon, a tall narrow KITE in the award animation, and the ◈ glyph
  // as a third. The kite wins — it is the one that plays at full screen when a core lands. Rather
  // than keep an icon path and a canvas path in sync by hand, BOTH are generated from these numbers,
  // so the two cannot drift apart no matter who edits which.
  const CORE_ASPECT = 0.72;        // half-width / half-height — the "thin kite" proportion
  const CORE_FACET = 0.55;         // the single vertical centre line, as a fraction of half-height
  const coreIconPath = (() => {
    const cx = 12, cy = 12, hh = 9, hw = +(hh * CORE_ASPECT).toFixed(2), f = +(hh * CORE_FACET).toFixed(2);
    return '<path d="M' + cx + ' ' + (cy - hh) + ' ' + (cx + hw) + ' ' + cy + ' ' + cx + ' ' + (cy + hh) + ' ' + (cx - hw) + ' ' + cy + 'Z"/>'
      + '<path d="M' + cx + ' ' + (cy - f) + 'V' + (cy + f) + '"/>';
  })();
  const ICONS = {
    ...
    gem: coreIconPath,   // generated from CORE_ASPECT/CORE_FACET above — same numbers drawCoreFx uses
  ...
  const PLANET_DESC = [
    "A quiet inner planet. Sparse, fragile dots — find your rhythm.",
    "Drifting embers. Swarms move faster; keep collectors close.",
    "Scorched cinder fields. Hotter, tougher dots — Mortars forge here.",
    "The hearth planet. Dense clouds and richer payouts — feed your Mortars.",
  ...
  // v18.79 (owner: "make sure launching is free"). Launching used to cost 15% of the planet's conquer
  // target. Kept as a function returning 0 rather than deleted: every call site reads it, and the HUD
  // lines, the star-map card and the sims all fold away cleanly at zero — one place to change if it
  // ever comes back. See the spoils block below: this and that are the SAME decision.
  const travelCost = () => 0;
  ...
  // ─── v18.79: A CONQUERED WORLD PAYS NO CASH ───────────────────────────────────────────────
  // Owner: "make sure launching is free, conquered planets don't produce any normal currency."
  // Those are one decision, not two. Read the v18.9 note above: victory spoils exist BECAUSE the
  // launch cost money — a settled world spawns nothing, so a player who conquered while broke would
  // be stranded on a silent planet unable to afford the way out. Spoils were the patch for that.
  // Free launch removes the trap, and with the trap gone the pot has no job left. What a conquered
  // world gives you now is the planet, the ◈ seam and a place to stand — never cash. Money comes
  // from the frontier, which is what the how-to has claimed all along.
  const SETTLE_ON_SITE = 20;         // retained only so an old save's ⚑ pot can still be read and retired
  ...
  const SPOILS_PAYOUT_H = 26 / SETTLE_ON_SITE;   // v18.79: dead rate, exported only so the sims can price a launch save if one ever returns
  const SETTLE_SPOILS_FRAC = 0;      // v18.79: no pot is banked on a conquest any more
  const settleSpoils = () => 0;
  const settleIncomeRate = () => 0;   // v18.79: a taken world pays no cash at all, parked or not
  ...
  function settleOffline() { return 0; }   // v18.79: nothing accrues on a settled world, present or absent
  ...
  function freshMeta() { return { totalEver: 0, stats: freshStats(), opts: freshOpts(), asc: { cores: 0, lv: {}, runs: 0, best: 0, lifetime: 0, v: 3 }, tutorialDone: false, seen: {} }; }   // the ASCENSION layer lives in META — the only thing that survives an ascension. asc.v = shop schema (3 = the v16.4 flat-curve shop; older saves get every past spend refunded at its era's prices on load)
  ...
  // Where an earned ◈ flies to. drawCoreFx records the point it ACTUALLY used in coreFxLast, and
  // tools/play-hud.js hit-tests that recorded point against the live ASCEND control — asking the
  // helper alone would pass happily while the draw kept its own stale corner. This is precisely the
  // kind of coordinate that rots silently when the UI around it moves, so the gate watches the flight.
  let coreFxLast = null;
  function coreFxTarget() {
    const nb = qs('#nav .nv[data-nav="ascend"]');
    if (nb && nb.getBoundingClientRect) { const q = nb.getBoundingClientRect();
      if (q.width > 1 && q.height > 1) return { x: q.left + q.width / 2, y: q.top + q.height / 2, from: "nav" }; }
    return { x: SW - 54, y: Math.max(46, SH * 0.055), from: "fallback" };   // nav not up (home screen, a modal mid-swap)
  }
  ...
  function drawCoreFx(e, n) {
    ...
    // v18.81 (owner: "the menus is still broken on the play page"): every earned ◈ used to fly to
    // (width-54, 46) — the top-right corner, because that is where the ascension pill lived. v18.66
    // moved the ascend entry into the bottom NAV and v18.68 gave the top-right to the banner's ☰, so
    // the cores have been flying up and off to nothing ever since, ending as a diamond clipped under
    // the status bar. They land on the ASCEND nav item now, which is the thing whose count goes up.
    const T = coreFxTarget(); coreFxLast = T; const tx = T.x, ty = T.y;
    ...
    for (let i = 0; i < shown; i++) {
      ...
      const pts = [[0, -sz], [sz * CORE_ASPECT, 0], [0, sz], [-sz * CORE_ASPECT, 0]];   // v18.63: the icon is built from these same numbers
      ...
      if (dk >= 1) { ctx.globalAlpha = 0.20 * alive * (1 - fly * 0.5); ctx.beginPath();
        ...
        ctx.beginPath(); ctx.moveTo(0, -sz * CORE_FACET); ctx.lineTo(0, sz * CORE_FACET); ctx.stroke(); }   // the facet
      ctx.restore();
    }
  ...
  function showCorePop(n, why, pending) {
    ...
    $("cp-n").textContent = "+" + fmt(n) + " PRESTIGE CORE" + (n > 1 ? "S" : "");   // v18.63: no leading ◈ glyph — the kite icon is already sitting right beside it, and the glyph is a DIFFERENT silhouette to the icon, so doubling them showed both shapes side by side
  ...
  const ascStaged = () => wallAhead() && pendingCores() >= 1 && state === "play" && !conq0Hide();   // v18.66: ONE source for 'is the ascend badge on stage' — the visibility and the class must never disagree
  const ETA_SHOW_H = 24;   // above a day, a live-income ETA is noise, not a goal
  ...
  // v18.76: what you already OWN. Wheel drops and mine output are banked STRAIGHT here and never
  // appear in pendingCores(), which is why the ascend door stayed shut on a core you had won.
  const bankedCores = () => ((META && META.asc && META.asc.cores) | 0);
  ...
  function load() {
    ...
    try {
      ...
      if (d) {
        ...
        // v18.62 PROGRESSIVE REVEAL, FOR SAVES THAT PREDATE IT. META.seen does not exist on any save
        // written before v18.60, so a returning player would load in to find their COLLECTORS tab,
        // abilities and ascend button GONE — re-onboarded after hours of play. Any evidence of
        // progress unlocks everything at once.
        // This MUST sit after the META merge above: `META = Object.assign(freshMeta(), d.META)`
        // replaces the object wholesale, so an earlier revealAll() was written into a discarded one.
        // (It was, for exactly one version. tools/onboarding.js O14 caught it by planting a real
        // pre-v18.60 save and checking the tabs survive.)
        if (!META.seen || !Object.keys(META.seen).length) {
          const played = (S.peakGalaxy | 0) > 1 || (S.units || []).length > 1 || (S.collectors || []).length > 1
            || ((META.asc && META.asc.runs) | 0) > 0 || ((META.asc && META.asc.cores) | 0) > 0
            || Object.keys(S.classNodes || {}).some(t => Object.keys(S.classNodes[t] || {}).length > 0)
            || UPS.some(u => (S.lv[u.id] | 0) > 0);
          if (played) revealAll();
        }
    ...
    for (const k in S.vault) { const v = S.vault[k]; if (!v) continue;
      ...
      if (v.spoils != null) v.spoils = 0; }   // v18.79: the pot is retired — an old save stops carrying one
  ...
  function showWelcome(w) {
    ...
    if (!planetMeta(S.galaxy).conquered) txt += "  The conquer bar does not move while you are gone — planets are taken in person. Spend the haul and go and take it.";   // v18.22: the bar DOES move while you're away, at the idle allowance — say by how much
  ...
  function showConquest(g, cores, spoils) {
    ...
    const rows = [
      ...
      ["⟶ LAUNCH", "free — go when you like"],   // v18.79
    ];
    const cs = $("cq-rows"); if (cs) cs.innerHTML = rows.map(r => "<div class='vs-row'><span>" + r[0] + "</span><b>" + r[1] + "</b></div>").join("");
    const nt = $("cq-note");
    if (nt) nt.textContent = "You took " + wardenOf(g).n + " down and the planet with it. It is yours and at peace — the ◈ seam came with it, "
  ...
  function summonWarden() {
    ...
    { const Wd = wardenOf(S.galaxy);
      ...
      floatTxt(W / 2, H / 2 + 8, "kill it in 60s and the planet is yours — its hoard founds the ◈ mine"); }
    syncHUD(); return true;
  }
  function spawnBoss(warden) {
    ...
    const d = { x: wCX(), y: wCY(), vx: rnd(-18, 18), vy: rnd(-8, 8), hp, maxHp: hp, value: val, value0: val,   // v18.11: anchor at midfield — the patrol personalities orbit THIS point, and H*0.3 kept bosses hugging the top out of easy reach
      ...
      // v18.79: Math.min is load-bearing — mr() can return exactly 1.0 (nibble 15/15), which indexed
      // off the end of `styles`, so planet 12's boss had mstyle undefined and fell through to `dash`.
      // NB the comment is on its OWN line: an appended one swallows the rest of this literal, which is
      // exactly how mt/mphase/mfx/mfy/mdir/mrad went undefined for one build of this very edit and put
      // NaN into every boss position. The file warns about this at the conquest block for the same reason.
      mstyle: styles[Math.min(styles.length - 1, Math.floor(mr(0) * styles.length))],
      mt: 0, mphase: mr(1) * TAU, mfx: 0.5 + mr(2) * 0.9, mfy: 0.45 + mr(3) * 0.9, mdir: mr(4) < 0.5 ? -1 : 1, mrad: 95 + mr(5) * 75, mtimer: 0, mtx: wCX(), mty: wCY(), mdash: false,
    ...
    if (W_) {   // v18.24: bolt on this world's warden kit — same init semantics spawnDot uses, so every
      ...
      d.mrad = 46 + (g % 5) * 6; d.mdir = (g % 2) ? 1 : -1;
      ...
      // v18.79: that guarantee is a LEASH now (see bossMove), not "d.mstyle = orbit" hard-coded over
      // the top. Every warden had its own movement named in its kit and every one of them circled
      // instead — the line below this used to overwrite W_.m unconditionally, so eighteen wardens
      // shared one patrol. They keep their personality AND stay inside your guns.
  ...
  // ─── v18.79 BOSS MOVEMENT: FORCES, NOT A SCRIPT ──────────────────────────────────────────────
  // Owner: "the movement of bosses feels very stale and it feels like there is no interactivity...
  // make sure it's adjusted for using black hole events and everything."
  //
  // Every personality used to write d.x/d.y STRAIGHT FROM A CLOCK — lissajous, orbit and pace were
  // pure functions of t. Nothing you did could move a boss, because the next frame recomputed where
  // its clock said it should be. That is why the Black Hole appeared to do nothing to one: the well
  // dragged it, and bossMove teleported it back onto the curve. It also meant a boss never reacted
  // to being shot, so a duel was watching a screensaver with a health bar.
  //
  // Now every style names a TARGET and the boss STEERS toward it: acceleration, drag, a speed cap.
  // The boss's velocity is one shared thing, so the well, your knockback and its own patrol all add
  // into it and compete. Drag a boss into your rack and it has to swim back out.
  //
  // The warden keeps its guarantee. v18.24 pinned wardens to a tight orbit because roaming ones
  // drifted out of a thin rack's range and made the seam duel a coin flip — measured, the same
  // warden died in 6s on one attempt and escaped the full 60s on the next. That is preserved as a
  // LEASH (a radius it is hauled back inside) rather than as a scripted circle, so the fight is
  // still decided by your damage, but the warden can be shoved around inside its own arena and
  // finally gets the movement personality its kit always named and never used.
  const BOSS_DRAG = 2.4;             // velocity half-life; low enough that a shove still reads a second later
  const BH_BOSS_ACC = 780;           // the well's grip on a boss, px/s^2 — strong enough to haul, never a teleport
  function bossMove(d, dt) {
    ...
    let tx = cx, ty = cy, acc = 300, vmax = 150;

    if (d.mstyle === "lissajous") {                                   // graceful serpentine figure-weave
      tx = cx + Math.sin(t * d.mfx + d.mphase) * (R - L) / 2 * 0.86; ty = cy + Math.sin(t * d.mfy * 1.4) * (B - T) / 2 * 0.82;
      acc = 340; vmax = 165;
    } else if (d.mstyle === "orbit") {                                // territorial guardian, circling
      const a = t * 0.55 * d.mdir + d.mphase;
      tx = cx + Math.cos(a) * d.mrad; ty = cy + Math.sin(a) * d.mrad * 0.6;
      acc = 430; vmax = 190;
      ...
      tx = d.mtx; ty = d.mty; acc = 620; vmax = 280;
    } else if (d.mstyle === "pace") {                                 // pacing sentinel along the top, bobbing
      tx = cx + Math.sin(t * 0.9 * d.mdir + d.mphase) * (R - L) / 2 * 0.92; ty = T + 38 + Math.abs(Math.sin(t * 2)) * 34;
      acc = 520; vmax = 230;
    } else if (d.mstyle === "prowl") {                                // erratic predator: sudden bursts & turns
      d.mtimer -= dt; if (d.mtimer <= 0) { const a = Math.random() * TAU, sp = rnd(70, 175);
        d.mtx = d.x + Math.cos(a) * sp * 1.6; d.mty = d.y + Math.sin(a) * sp * 1.6; d.mtimer = rnd(0.5, 1.4); }
      tx = clamp(d.mtx, L, R); ty = clamp(d.mty, T, B); acc = 480; vmax = 180;
    } else {                                                          // dash: twitchy — holds, then darts to a new spot
      d.mtimer -= dt;
      if (d.mdash) { if (Math.hypot(d.mtx - d.x, d.mty - d.y) < 16 || d.mtimer <= 0) { d.mdash = false; d.mtimer = rnd(0.8, 1.7); burst(d.x, d.y, 9, 90, 1.7); ring(d.x, d.y, d.r, d.r + 34, 0.3); } }
      else if (d.mtimer <= 0) { d.mdash = true; d.mtx = rnd(L, R); d.mty = rnd(T, B); d.mtimer = 0.7; }
      tx = d.mdash ? d.mtx : d.x; ty = d.mdash ? d.mty : d.y;
      acc = d.mdash ? 1500 : 220; vmax = d.mdash ? 620 : 90;
    }

    // ---- a WARDEN patrols INSIDE its arena ---------------------------------------------------
    // The leash used to be a pull applied after the fact, so a personality that aims outside it —
    // `pace` walks the TOP of the field — spent the whole duel straining against the tether and
    // overshot it anyway (measured 189px out of a 142px leash). Clamping the TARGET instead means
    // the warden performs its own patrol, at its own rhythm, scaled into the arena it must hold.
    if (d.warden) { const lr = d.leashR || (d.leashR = 46 + (d.bg % 5) * 6 + 78);
      const ox = tx - cx, oy = ty - cy, ol = Math.hypot(ox, oy);
      if (ol > lr) { tx = cx + ox / ol * lr; ty = cy + oy / ol * lr; } }

    // ---- steer toward the target ------------------------------------------------------------
    d.vx = d.vx || 0; d.vy = d.vy || 0;
    { const dx = tx - d.x, dy = ty - d.y, dl = Math.hypot(dx, dy) || 1;
      const ease = Math.min(1, dl / 90);                              // ease off near the target so it settles instead of buzzing
      d.vx += dx / dl * acc * ease * dt; d.vy += dy / dl * acc * ease * dt; }

    // ---- v18.80 EVASION: the boss dodges your FINGER, harder the deeper you are ---------------
    // This is the whole tracking game. It only reacts inside EVADE_R, so a boss is not fleeing the
    // arena — it is squirming out from under your finger while you try to stay on it. The strength
    // is a fraction of the steering budget and it rides planet depth, so P1 barely flinches and P18
    // is genuinely slippery. It is bounded on purpose: a finger moves as fast as you can move it,
    // and the boss's own speed cap still applies, so "harder" never becomes "impossible".
    if (performance.now() / 1000 - fingerT < 0.25) {
      const fx = d.x - fingerX, fy = d.y - fingerY, fl = Math.hypot(fx, fy) || 1;
      if (fl < EVADE_R) {
        const near = 1 - fl / EVADE_R;                                // hardest right under the finger
        const em = evadeMul(d.bg || S.galaxy);
        d.vx += fx / fl * EVADE_ACC * em * near * dt; d.vy += fy / fl * EVADE_ACC * em * near * dt;

        // THE JUKE is the part that actually makes a boss hard to track, and steady repulsion is not.
        // Measured: a boss cannot out-RUN a finger — a hand crosses the field faster than any speed a
        // boss may have, so pushing it away read as 98% on target at every depth. What a hand cannot
        // do is react instantly. So the dodge is an impulsive LATERAL cut, perpendicular to the line
        // between you and it, sides chosen at random: the thing that beats a tracker with a reaction
        // delay. Deeper worlds cut harder and more often. Bounded, and inside the arena either way.
        d.jukeT = (d.jukeT || 0) - dt;
        if (d.jukeT <= 0) {
          d.jukeT = rnd(0.5, 1.05) / (0.45 + em);
          const side = Math.random() < 0.5 ? 1 : -1;
          // Sized from what it takes to actually break contact: a hand reacts in ~190ms, and the
          // brush reaches 30px past the boss's own radius, so a cut has to move it about 150px inside
          // that window to shake you. That is ~800px/s at the moment of the cut, not the ~400 a
          // gentler number gives — measured, 430px/s left P9 indistinguishable from P1 at 98% on target.
          const imp = (300 + 900 * em) * (0.55 + 0.45 * near);
          d.vx += -fy / fl * side * imp; d.vy += fx / fl * side * imp;
          d.evading = 0.35;
        }
        // a juke has to survive the patrol's speed cap for the moment it happens, or it is clamped
        // away before it moves anything.
        vmax = Math.max(vmax, 250 + 900 * em);
        d.evading = Math.max(d.evading || 0, 0.2);                    // for the draw layer — a juking boss should LOOK like it
      }
    }
    if (d.evading > 0) d.evading -= dt;

    // ---- the BLACK HOLE is a force on that same velocity, not a teleport ----------------------
    // This is the interactivity the owner is asking for: park a well on your rack and the boss is
    // hauled into your guns and has to fight its way back out.
    if (blackholeT > 0) { const dx = cx - d.x, dy = cy - d.y, dl = Math.hypot(dx, dy) || 1;
      d.vx += dx / dl * BH_BOSS_ACC * dt; d.vy += dy / dl * BH_BOSS_ACC * dt;
      vmax = Math.max(vmax, 320); }

    // ---- drag, then the cap ------------------------------------------------------------------
    { const k = Math.exp(-BOSS_DRAG * dt); d.vx *= k; d.vy *= k; }
    { const sp = Math.hypot(d.vx, d.vy); if (sp > vmax) { d.vx *= vmax / sp; d.vy *= vmax / sp; } }
    d.x += d.vx * dt; d.y += d.vy * dt;

    // ---- and it may be SHOVED past the leash, but never left there ---------------------------
    // A well or a heavy volley can push a warden out — that is the interactivity. What it cannot do
    // is park it out of your reach: outside the leash it is hauled back, and a hard stop at a small
    // margin makes the arena a real boundary rather than a suggestion.
    if (d.warden) { const lr = d.leashR, cap2 = lr * 1.25;
      const dx = d.x - cx, dy = d.y - cy, dl = Math.hypot(dx, dy) || 1;   // ||1 is load-bearing: a boss SPAWNS at exactly (cx,cy), and 0/0 put NaN straight into its position
      if (dl > lr) { const pull = (dl - lr) * 9;                      // proportional haul-back: the further out, the harder
        d.vx -= dx / dl * pull * dt; d.vy -= dy / dl * pull * dt; }
      if (dl > cap2) { d.x = cx + dx / dl * cap2; d.y = cy + dy / dl * cap2; } }

    d.x = clamp(d.x, L, R); d.y = clamp(d.y, T, B);
  }
  // v18.60 ARRIVAL SEED — the measured cause of a dead opening. On a virgin save the lone turret
  // kills a dot in 2.1 SECONDS, but nothing is ever in range: 28 dots spawned in the first 31s and
  // exactly 2 died, because dots enter at the map rim and are still walking in. Coverage is 21.5% of
  // the field, so the first arrivals take half a minute to cross. That dead zone is not a tutorial
  // problem — it happens on EVERY landing, all game.
  // So a field arrives already populated: the SAME dots from the SAME spawnDot(), just pulled part
  // of the way in, so the battle is underway the moment you look at it. This deliberately touches
  // nothing the sims price — not spawn rate, not HP, not value, not payout — only where the first
  // handful start standing.
  function seedField(n) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      spawnDot();
      if (dots.length === before) break;            // field cap reached — stop asking
      const d = dots[dots.length - 1];
      const f = rnd(0.45, 0.93);                    // 0 = still at the rim, 1 = on top of the guns
      d.x = cx + (d.x - cx) * (1 - f); d.y = cy + (d.y - cy) * (1 - f);
    }
  }
  const SEED_N = 12;   // enough that a couple sit inside the opening 21.5% coverage immediately
  ...
  function hitDot(d, dmg, src) {
    ...
    if (d.boss && src === "draw") d.drawDealt = (d.drawDealt || 0) + dmg;   // v18.80: every boss tracks it (the readout needs it), not just wardens mid-calibration
    if (d.shield > 0) {
    ...
    d.hp -= dmg; d.hit = 0.08;
    // v18.79 BOSS KNOCKBACK. A boss used to absorb an entire rack without so much as twitching,
    // because its position came from a clock — nothing you did to it moved it. Now that a boss
    // steers on a real velocity, damage can push. Scaled to the FRACTION of its health the hit took,
    // so a chip is a nudge and a heavy volley visibly rocks it, and capped so a big crit cannot
    // punt a warden across the arena. Bosses only: an ordinary dot has its own movement.
    if (d.boss && d.maxHp > 0) {
      const frac = Math.min(0.06, dmg / d.maxHp), cx0 = wCX(), cy0 = wCY();
      const ox = d.x - cx0, oy = d.y - cy0, ol = Math.hypot(ox, oy) || 1;   // pushed OUTWARD from midfield, so it reads as recoil
      d.vx = (d.vx || 0) + ox / ol * frac * 2600; d.vy = (d.vy || 0) + oy / ol * frac * 2600;
    }
    if (d.hp <= 0) {
      d.dead = true;
  ...
  // ─── v18.80 THE DUEL IS A TRACKING GAME ──────────────────────────────────────────────────────
  // Owner: "make it a finger tracking game where each boss becomes harder and harder to track (not
  // impossible) and finger damage should mean a lot to a boss."
  //
  // The substrate for this was already right and it is worth stating, because it is what makes a big
  // finger multiplier SAFE. A warden's HP pool is calibrated from a 4s sample of your output with the
  // finger's contribution SUBTRACTED (see d.cdraw) — the pool is sized to what your turrets sustain
  // alone. So finger damage can be worth a great deal without making the duel unfair: a player who
  // never touches the screen still gets the ~26s turret duel the pool was built for, and a player who
  // tracks well is rewarded with a much shorter one. Raising the multiplier cannot inflate the pool.
  // The finger's boss damage is a FRACTION OF THE BOSS'S OWN POOL, not a multiple of a unit's shot.
  // Measured the multiplier way first and it barely moved the duel — brushDmg() is derived from one
  // unit's damage and lands at most 1/drawCd times a second, so against a whole rack's sustained
  // output it stayed a rounding error however big the multiplier got (x7 took a P1 duel 27s -> 23.5s).
  // Priced as "seconds of unbroken contact to kill a boss by hand" it is scale-free instead: it means
  // the same at P1 and P18, on a thin rack or a maxed one, and it is a number you can reason about.
  const BOSS_FINGER_TTK = 20;     // perfect tracking alone kills a boss in 20s; with your guns, roughly half a duel
  const BOSS_DRAW_MUL = 7;        // floor for early/tiny pools, where a fraction of maxHp is less than a real hit
  const EVADE_R = 150;            // the boss only jukes when your finger is CLOSE — it dodges you, it does not flee the arena
  const EVADE_ACC = 620;          // px/s^2 at full depth, against a 620-1500 steering budget: pressure, never escape
  let fingerX = 0, fingerY = 0, fingerT = -99;   // last brush point and when — bossMove reads these
  // 0.12 on P1 up to 1.0 on P18: the first seam keeper barely flinches, the last one is genuinely slippery.
  const evadeMul = g => { const f = clamp(((g || 1) - 1) / (TOTAL_PLANETS - 1), 0, 1); return 0.10 + 0.90 * Math.pow(f, 0.75); };
  function brushAt(x, y) { Audio_saber(x, y); const R = 30, dmg = brushDmg();
    fingerX = x; fingerY = y; fingerT = performance.now() / 1000;   // v18.80: the bosses dodge THIS
    for (const d of dots) { if (d.dead) continue; const rr = R + d.r; if ((d.x - x) ** 2 + (d.y - y) ** 2 <= rr * rr && d.drawCd <= 0) {
    // v18.80: the finger is THE boss weapon. drawCd caps contact at 1/0.07 hits a second, so a hit is
    // worth that share of the pool — unbroken contact for BOSS_FINGER_TTK seconds is a kill by hand.
    const dd = d.boss ? Math.max(dmg * BOSS_DRAW_MUL, d.maxHp * (0.07 / BOSS_FINGER_TTK)) : dmg;
  ...
  function update(dt) {
    ...
    // v18.74 (owner: "the black hole shouldn't ever kill a dot directly"). It used to deal ~0.6s of
    // total fleet DPS to every dragged dot over its 5s, and on a tanky field that was most of the
    // killing — measured 3 of 4 kills credited to the ability itself. It is a GRAVITY WELL now,
    // nothing more: it drags the whole field into your guns and your guns do the work. Every kill
    // during a Black Hole belongs to a defender, which is also the only reading of the ability that
    // makes sense with the rest of the game — damage comes from what you built.
    for (const d of dots) {
      ...
      if (d.boss) {
        ...
        if (d.life >= (d.ttl || 60)) { d.dead = true; burst(d.x, d.y, 30, 200, 2.6); ring(d.x, d.y, d.r, d.r + 130, 0.5);
          ...
          if (d.warden) { wardenOn = false; wardenClear = true; floatTxt(W / 2, H / 2 + 8, "the planet holds — keep farming and call it out again when you are ready"); }   // v18.18: a lost duel re-settles the world; the panel offers the rematch
      ...
      // v18.79: BOSSES FIRST. The well used to write a boss's position directly and bossMove then
      // recomputed it from the clock the very same frame, so a Black Hole visibly did nothing to a
      // boss. bossMove owns a boss's movement now and folds the well in as a FORCE — see BH_BOSS_ACC.
      if (d.boss) { bossMove(d, dt); }
      else if (blackholeT > 0) { const dx = W / 2 - d.x, dy = H / 2 - d.y, dl = Math.hypot(dx, dy) || 1; d.x += dx / dl * 220 * dt; d.y += dy / dl * 220 * dt; }   // v18.74: PULL ONLY — no direct damage, ever
  ...
  function drawBoss(d) {
    ...
    // v18.80: how much of this boss YOUR FINGER took. The mechanic is only worth having if it is
    // legible, and a share of the bar is the honest way to say it — the same bar the damage came off.
    { const drew = d.drawDealt || 0, tot = (d.maxHp || 0) + (d.shieldMax || 0);
      const share = tot > 0 ? clamp(drew / tot, 0, 1) : 0;
      if (share > 0.02) { const fw = bw * share;
        ctx.fillStyle = "rgba(255,255,255,0.55)"; ctx.fillRect(bx + bw - fw, by + 5.5, fw, 1.5);   // a rule under the bar, right-anchored: the slice you removed by hand
        ctx.fillStyle = "rgba(255,255,255,0.85)"; ctx.font = "bold 8px ui-monospace,monospace"; ctx.textAlign = "right";
        ctx.fillText("✎ " + Math.round(share * 100) + "%", bx + bw, by + 19); } }
  ...
  function conquerWorld() {
    ...
    pm.bgRate = 0;
    pm.spoils = 0;   // v18.79: no pot is banked — a taken world pays no cash
  ...
  function render() {
    ...
    if (state === "play" && !S.travel && planetMeta(S.galaxy).conquered && !wardenOn && conqCardT <= 0) {
      const cy = Math.max(70, SH * 0.10) + 26, breathe = 0.85 + 0.15 * Math.sin(performance.now() / 900);
      ctx.save(); ctx.textAlign = "center"; ctx.fillStyle = "#fff";
      // v18.81: this used to print FOUR lines over the field — no credits, the mine's rate, and that
      // launching is free. The settlement panel in the dock says every one of those, better, with the
      // live mine countdown; one of these lines literally ended "panel below". So the screen carried
      // the same paragraph twice, the floating copy drifting across the play field and colliding with
      // the ◈ popup. The overlay keeps only the thing the panel cannot say, because the panel is not
      // what you are looking at: WHY the battlefield in front of you is empty.
      const head = "✦ WORLD SETTLED — NO MORE ENEMIES HERE";
      const body = ["the settlement panel below has the rest"];
  ...
  function renderSettlePanel() {
    ...
    if (built && bp < 1) {
      ...
      html += '<div class="sp-note">⛏ the ◈ seam on this planet is unclaimed — it came with the planet and will be sunk shortly.</div>';
    }
    // v18.79: a taken world pays no credits at all. It is worth saying out loud on the panel, because
    // the whole point of standing here is the ◈ seam above — not a wallet that is not filling.
    html += '<div class="sp-inc">this world pays <b>no credits</b> — its ◈ seam is the yield. Launching is <b>free</b>, so go when you like.</div>';
    sp.innerHTML = html;
  }
  // ============ v18.60 PROGRESSIVE REVEAL + ONE OBJECTIVE ============
  // Measured on a virgin save: the play screen opened with 28 controls of which exactly ONE was
  // affordable, four of six defender rows were locked adverts for planets hours away (each still
  // offering a ⬆ Tree button for a unit you cannot buy), and after 60 real seconds of watching the
  // player had earned 8 cash with the conquer bar still reading 0.00%. Every shipped idle game does
  // the opposite: Cookie Clicker opens with one button, AdVenture Capitalist with one business, and
  // both reveal the next system only when it becomes relevant.
  //
  // So features now APPEAR the first time they matter and never leave again. The flag lives in META
  // (not S), because a veteran starting a fresh run after ascending must not be re-onboarded.
  // The same event that reveals a feature also fires its one-line coach mark — which is how the
  // ten-step, 331-word opening tutorial got replaced by teaching delivered just in time.
  const REVEALS = [
    // a fresh save already OWNS one defender and one collector, so "do you have one" reveals
    // everything at t=0 and teaches nothing. Each trigger is the moment the system starts to matter:
    // an orb actually lying on the ground, a purchase actually made, a squad actually built.
    { k: "collect", when: () => orbs.length > 0 || S.collectors.length >= 2,
      tip: "Dead dots drop CASH ORBS — and orbs EXPIRE. Collectors gather them for you." },
    { k: "tree",    when: () => S.units.length >= 2 || S.collectors.length >= 2,
      tip: "Your defenders have SKILL TREES. Tap ⬆ Tree to spend cash on damage, fire rate and range." },
    // v18.61: ECONOMY IS NOT GATED — v18.60 hid it behind "own 3 units" and that was simply wrong.
    // Measured at second zero on a virgin save: cash 400, and the four economy upgrades cost 40 / 54
    // / 117 / 140, i.e. ALL FOUR affordable, three of them the cheapest purchases in the game — while
    // the only thing left visible was a 393 turret costing 98% of the purse. Hiding it did not reduce
    // complexity, it removed the affordable actions and manufactured the very "one thing you can buy"
    // problem the reveal system exists to fix. It is also the SIMPLEST system here ("more money per
    // dot" needs no teaching) and the one the whole economy rides on. Gating belongs on the complex
    // things — six defender classes with niches, skill webs, abilities, prestige — not on this.
    { k: "abil",    when: () => dots.length >= 18,
      tip: "The field is filling up. ABILITIES clear it fast — they run on cooldowns, so spend them." },
    // v18.76 (owner: "you shouldn't have the ascension screen blocked, reason being you can win cores
    // in the roulette of killing bosses"). This asked only about PENDING cores — the ones a conquest
    // stages for you — but the BOSS BOUNTY WHEEL banks a core directly on a 2% roll, and a boss can
    // die long before your first conquest. The wheel even says "◈ +1 CORE — banked to Ascension"
    // while the nav's ASCEND door sat at pointer-events:none. Holding the currency is the
    // qualification, wherever it came from: banked, pending, or both.
    { k: "ascend",  when: () => pendingCores() >= 1 || bankedCores() >= 1,
      tip: "You hold ◈ CORES. Ascending resets the run and makes every future run permanently faster." },
  ];
  const seenMap = () => (META.seen || (META.seen = {}));
  const ALWAYS = { eco: 1 };   // v18.61: economy is never hidden — see the REVEALS note above
  const revealed = k => !!ALWAYS[k] || !!seenMap()[k];
  function revealAll() { const m = seenMap(); for (const r of REVEALS) m[r.k] = 1; }   // test hook + used on load for veteran saves
  let coachQ = [], coachT = 0;
  function checkReveals() {
    const m = seenMap(); let changed = false;
    for (const r of REVEALS) { if (!m[r.k] && r.when()) { m[r.k] = 1; changed = true; if (r.tip) coachQ.push(r.tip); } }
    if (changed) { renderList(); save(); }
  }
  // the just-in-time coach mark: one line, one system, at the moment it starts mattering
  function coachTick(dt) {
    const el = $("coach"); if (!el) return;
    // v18.62: never stack two teaching UIs. Blocking does not just stop the NEXT line — a coach
    // already on screen when the tutorial opens is pulled and re-queued, so nothing is lost and
    // nothing paints over the cards.
    const busy = state !== "play" || qs(".modal.show") || qs("#tutorial.show");
    if (busy) { if (coachT > 0) { coachQ.unshift(el.textContent); coachT = 0; el.classList.remove("show"); } return; }
    if (coachT > 0) { coachT -= dt; if (coachT <= 0) el.classList.remove("show"); return; }
    if (!coachQ.length) return;
    el.textContent = coachQ.shift(); el.classList.add("show"); coachT = 7; vibe(15); Audio_tick();
  }

  // THE OBJECTIVE — one line, always current. A sandbox with 28 affordances and no sequence is the
  // thing that makes this game hard to pick up; a single "do this next" turns it into a chain.
  function objectiveNow() {
    if (state !== "play") return null;
    const conq = S.vault && S.vault[S.galaxy] && S.vault[S.galaxy].conquered;
    // a fresh save starts with ONE defender and ONE collector, so the ladder starts at the second
    if (S.units.length < 2) return { t: "Buy a 2nd DEFENDER", s: "twice the guns, twice the kills", go: "upgrades" };
    if (S.collectors.length < 2 && revealed("collect"))
      return { t: "Buy a 2nd COLLECTOR", s: "cash orbs expire where they drop until one reaches them", go: "upgrades" };
    if (revealed("tree") && !treeNodeCount())
      return { t: "Spend a SKILL TREE node", s: "tap ⬆ Tree on a defender — damage, fire rate, range", go: "upgrades" };
    if (revealed("eco") && !(S.lv && (S.lv.value | 0) > 0))
      return { t: "Buy a VALUE upgrade", s: "on the ECONOMY screen — it multiplies every dot you kill", go: "economy" };
    if (conq) return { t: "LAUNCH to the next planet", s: "your whole army travels with you" };
    if (barFull()) return { t: "SUMMON the warden", s: "beat it and the planet is yours" };
    const pct = Math.floor(clamp(curEarned / conquerTarget(S.galaxy), 0, 1) * 100);
    return { t: "Fill the CONQUER bar — " + pct + "%", s: "every dot you kill fills it" };
  }
  const treeNodeCount = () => { let n = 0; for (const t in (S.classNodes || {})) n += Object.keys(S.classNodes[t] || {}).length; return n; };

  // v18.62: a blocking MODAL hides the objective; the tutorial does NOT. #tutorial is a coach-mark
  // overlay, and its second step literally says "the line above the shop always tells you what to do
  // next" — hiding that line behind the tutorial made the tutorial contradict itself on screen.
  // The settlement panel replaces the shop entirely, so the objective stands down for that too.
  // v18.64: tools/balance-check.js evaluates this file under a MINIMAL document stub that has no
  // querySelector at all, so every DOM lookup outside $() must degrade instead of throwing. The
  // Playwright tools never hit that path, which is exactly why it went unnoticed until it crashed.
  const qs = sel => (typeof document !== "undefined" && document.querySelector) ? document.querySelector(sel) : null;
  // v18.68: the objective lives on the UPGRADES screen now, so it must be VISIBLE while that
  // modal is open — the old rule hid it behind any .modal.show, which is now its own home.
  const conq0Hide = () => settleShown || !!qs('#menu.show') || !!qs('#settings.show') || !!qs('#how.show');
  function syncHUD() {
    coachTick(1 / 60);
    if (qs('#tutorial.show')) Tut.place();   // v18.64: the spotlight follows its target
    ...
    { const ah = $("ui-ascend-hint");
      if (ah) { const pend = pendingCores();
        ...
        // v18.62: the centre-stage badge (v18.60) is strictly louder and says the same thing, so the
        // quiet text hint stands down whenever it is up rather than doubling the message.
        const due = pend >= 1 && isFinite(secs) && secs > ASC_HINT_S && state === "play" && !wallAhead();
    ...
    // v18.60: one idle title A/B-tested moving its prestige offer from a small top-right icon to a
    // CENTRE-STAGE animated badge the moment the player qualifies, and it won. Ours was exactly that
    // small top-right icon. It still lives there, but when the wall actually lands it grows, moves
    // to the middle of the field and says what it is — you cannot miss the one decision that matters.
    { const p = pendingCores(), ab = $("ascend-n"); if (ab) ab.textContent = "+" + fmt(p); const abtn = $("btn-ascend"); if (abtn) { abtn.classList.toggle("has", p >= 5);
      abtn.classList.toggle("stage", ascStaged());   // the ALWAYS-VISIBLE offer (v16.0): what ascending right now banks; glows once it's a real haul
    ...
    let label, dis = true, ready = false, enroute = false, plainConquer = false;
    if (S.travel) { enroute = true; label = "⟶ IN WARP…"; dis = true; }   // v17.29: travel is a 7s cinematic — pay-to-skip removed (there is nothing to skip)
      ...
      else { ready = true; dis = false; label = "LAUNCH ⟶ NEXT PLANET"; }   // v18.79: launching is free — no price to quote and nothing to be short of
      ...
      else { plainConquer = true; const e = wallEtaH();   // v18.68: this state has no ACTION — the banner shows the %, so the button stands down
        // ...but ONLY while the number means something. wallEtaH divides by your LIVE income, and at
        // the start of a planet that income is near zero against a multi-million target, so the
        // honest answer is "~876h" — measured, and far worse than saying nothing. Above ETA_SHOW_H
        // the objective line is what guides the player, so the bar keeps quiet.
        label = "CONQUER " + Math.floor(clamp(curEarned / tgt, 0, 1) * 100) + "%"
          + (isFinite(e) && e > 0 && e <= ETA_SHOW_H ? "  ·  ~" + fmtHrs(e) : ""); }
    // v18.68 THE PROGRESS BANNER owns the conquer readout. It used to be stranded on the LAUNCH
    // button at the very bottom of the screen, a whole viewport away from the bar it described.
    { const cq = $("ui-conq");
      if (cq) { const t = plainConquer ? label : "CONQUER " + Math.floor(clamp(conq ? 1 : curEarned / tgt, 0, 1) * 100) + "%";
        if (cq.textContent !== t) cq.textContent = t; } }
    // v18.77 (owner: "include a timer until the next mini boss spawns in"). A mini-boss is the only
    // routine source of the bounty wheel, and the wheel is where ◈ cores come from outside a
    // conquest — so "when is the next one" is a real planning question, and nothing on screen
    // answered it. It reads the SAME state the spawner does rather than a parallel countdown:
    // bossAcc only advances while the field is boss-free and the world is unsettled, so the readout
    // freezes exactly when the spawn does instead of lying about a timer that is not running.
    { const be = $("ui-boss");
      if (be) { const onField = dots.some(d => d.boss);
        const left = Math.max(0, BOSS_INTERVAL - bossAcc);
        // the word BOSS is its own span so the narrowest phone can drop it and keep "▲ 3:58" —
        // at 280px the full label pushed the banner's foot row into wrapping onto two lines.
        // the warden IS a boss, so it freezes the same clock — but calling the world's keeper a
        // "boss" while you are duelling it reads as a different thing arriving. Name it.
        const body = wardenOn ? "WARDEN" : onField ? "HERE"
          : Math.floor(left / 60) + ":" + String(Math.floor(left % 60)).padStart(2, "0");
        const t = settleShown ? ""                                   // a settled world spawns none — an empty readout, not a frozen one
          : '<span class="ub-sep">  ·  </span>\u25b2' + (wardenOn ? " " : '<span class="ub-w"> BOSS</span> ') + body;
        if (be._t !== t) { be._t = t; be.innerHTML = t; }
        be.classList.toggle("soon", !onField && !settleShown && left <= 15);   // the last fifteen seconds — time to hold an ability
        be.classList.toggle("here", onField); } }
    const bt = $("btn-travel");
    // ...and when there is nothing to DO, the button goes away rather than restating the banner
    { const want = !plainConquer;
      if ((bt.style.display === "none") === want) bt.style.display = want ? "" : "none"; }
    ...
    { const dockCalm = conq && !wardenOn;
      if (dockCalm !== settleShown) { settleShown = dockCalm; settleKey = "";
        ...
        // v18.62: #abilities is NOT in this list any more. Progressive reveal (v18.60) also owns its
        // display, and this line ran on every settle/un-settle transition setting it back to "",
        // un-hiding abilities the reveal gate had deliberately hidden. One owner per property: the
        // reveal pass decides abilities, and it now folds settleShown into that decision.
        // v18.67: #tabs and #up-list live on the UPGRADES screen now, not in this dock, so the
        // settle swap must not reach across and hide them — a settled world would have left the
        // whole Upgrades page blank. It only swaps what it actually owns: the objective row.
        { const ob = $("objective"); if (ob && dockCalm) ob.classList.remove("show"); } }
    ...
    for (const id in listRows) {
      ...
      if (row.kind === "unit") {
        ...
        // v18.60: no tree for a class you do not own — it was the single most common dead control
        if (row.up) { const want = n > 0 && revealed("tree");
          if ((row.up.style.display === "none") === want) row.up.style.display = want ? "" : "none"; }
    ...
    // v18.67: the UPGRADES screen covers the top bar, so the wallet has to come with it. A store that
    // cannot tell you what you can afford is the most basic thing there is to get wrong.
    // v18.70: the Economy screen is a second shop and covers the same bar, so it carries the same wallet.
    { const t = curSym(S.galaxy) + " " + fmt(S.cash), t2 = "+" + curSym(S.galaxy) + fmt(cps) + "/s";
      for (const id of ["up-cash", "eco-cash"]) { const e = $(id); if (e && e.textContent !== t) e.textContent = t; }
      for (const id of ["up-cps", "eco-cps"]) { const e = $(id); if (e && e.textContent !== t2) e.textContent = t2; } }
    // v18.60 PROGRESSIVE REVEAL — hide what cannot be played yet, and never hide it again once shown
    checkReveals();
    { const setVis = (el, on) => { if (el && (el.style.display === "none") === on) el.style.display = on ? "" : "none"; };
      const dt = tabBtns.drone;
      setVis(dt && (dt.closest(".tslot") || dt), revealed("collect"));
      // v18.72 (owner: "make sure the 3 upgrades disappear if you're not on the main shooting page").
      // v18.69 put the powers on every screen so a Black Hole was never more than one tap away; the
      // owner has since asked for the opposite, and they are right that it reads better — you FIRE
      // these at the field, and a live power tile on the Economy page is offering something you
      // cannot see the effect of. They belong to PLAY, so they are PLAY's.
      // v18.73 (owner: "I don't see the 3 abilities ever now, supposed to be on the play page").
      // !settleShown was in this condition, and a CONQUERED planet is settled — which is most of a
      // mature save, since you sit on a conquered world farming the travel cost. So the powers were
      // gone exactly when you most want them. That clause is a leftover from v18.13, when abilities
      // were DOCK furniture and the settlement panel replaced the whole dock; v18.69 moved them into
      // the nav, which the settle panel does not own and does not overlap. Reveal and screen, nothing
      // else, decide whether the powers are on screen.
      setVis($("abilities"), revealed("abil") && navAt === "play");
      setVis($("btn-ascend"), ascStaged());   // v18.66: the NAV carries the persistent entry now; this button exists only to become the centre-stage badge at the wall
      // v18.66: the nav's ASCEND item wears the pending count, so the offer is visible from any screen
      // navBusy guard: a NAV_OPEN handler calls renderList() -> syncHUD() BEFORE it adds .show, so
      // without this the reconcile fires mid-navigation and sends you straight back to PLAY.
      { const id = navBusy ? null : NAV_EL[navAt], e = id && $(id);
        if (e && e.classList && e.classList.contains && !e.classList.contains("show")) { navAt = "play"; syncNav(); } }
      syncDockEmpty();
    // v18.69: --nav-h was a fixed 58px. The nav is two tiers now and the power row hides until the
    // abilities are revealed (and on a settled world), so the real height varies — measure it and
    // publish it, or the dock and every modal reserve space for a bar that is not that tall.
    { const nv = $("nav"), root = (typeof document !== "undefined" && document.documentElement) || null;
      if (nv && nv.getBoundingClientRect && root && root.style) {
        const h = Math.round(nv.getBoundingClientRect().height);
        if (h > 0 && h !== navH) { navH = h; root.style.setProperty("--nav-h", h + "px"); } } }
      { const nb = $("nv-asc"), pend = pendingCores();
        if (nb) { const t = pend > 0 ? "+" + fmt(pend) : "";
          if (nb.textContent !== t) nb.textContent = t;
          nb.classList.toggle("on", pend > 0); nb.classList.toggle("hot", wallAhead() && pend >= 1); }
        const na = qs('#nav .nv[data-nav="ascend"]');   // qs(), not document.querySelector — balance-check runs this file under a document stub with no querySelector at all
        if (na) na.classList.toggle("locked", !revealed("ascend")); }
      // a hidden tab must never stay selected — fall back to DEFENCE, which is always available
      if (activeTab === "drone" && !revealed("collect")) {
        activeTab = "def"; for (const k in tabBtns) tabBtns[k].classList.toggle("sel", k === "def"); renderList(); } }
    // THE OBJECTIVE — one line, always current
    { const ob = $("objective"), o = objectiveNow();
      if (ob) { const on = !!o && !conq0Hide();
        ob.classList.toggle("show", on);
        // v18.70: the objective lives on the ARMY screen, so most of the time its destination IS the
        // screen you are already reading it on — "tap to open" was pointing at the room you are in.
        // The affordance appears only when tapping would actually take you somewhere (e.g. ECONOMY).
        const go = (o && o.go && o.go !== navAt) ? o.go : null;
        if (on) { const t = "\u25b8 " + o.t, sub = o.s + (go ? "  ·  tap to open" : "");
          ob.onclick = go ? () => navGo(go) : null;
          ob.classList.toggle("tappable", !!go);
          if (ob._t !== t) { ob._t = t; ob.querySelector(".ob-t").textContent = t; }
          if (ob._s !== sub) { ob._s = sub; ob.querySelector(".ob-s").textContent = sub; } } } }
    ...
    for (const k in tabBtns) { const b = tabBtns[k], warn = k === "drone" ? leak : false, n = aff[k];
    ...
    // v18.70: ARMY and ECONOMY are destinations now, so the "something is affordable in here" count
    // has to live on the NAV — the tab badges only speak for the two tabs still inside the army shop,
    // and the economy has no tab left to wear one at all.
    { const navBadge = (id, n, warn) => { const b = $(id); if (!b || !b.classList) return;
        const txt = warn ? "!" : n > 9 ? "9+" : String(n);
        if (b.textContent !== txt) b.textContent = txt;
        b.classList.toggle("on", warn || n > 0); b.classList.toggle("warn", !!warn); };
      navBadge("nv-up", aff.def + aff.drone, leak);
      navBadge("nv-eco", aff.eco, capped); }
  ...
  // v18.70: ONE call renders BOTH shops. Economy is its own screen now, but syncHUD walks a single
  // listRows map every frame to price and enable every row — so the eco rows must be built whether
  // or not you are looking at them, exactly as they were when they shared a list with the army.
  // Splitting the render but not the map is the whole trick: nothing downstream of listRows changed.
  function renderList() {
    listRows = {};
    renderArmy();
    renderEco();
  ...
  function renderArmy() {
    ...
    {
      const order = activeTab === "drone" ? COL_ORDER : DEF_ORDER, col = activeTab === "drone" ? "var(--drone)" : "#fff";
      // v18.60: a LOCKED class used to render a full row advertising a planet hours away — four of
      // the six defence rows on a virgin save, each with a live ⬆ Tree button for a unit you cannot
      // buy. They collapse into one honest line at the bottom instead.
      const shown = order.filter(t => S.free || S.peakGalaxy >= TY(t).gal), hidden = order.length - shown.length;
      ...
      if (hidden > 0) { const n = document.createElement("div"); n.className = "up-more";
        n.textContent = "▸ " + hidden + " more " + (activeTab === "drone" ? "collector" : "defender") + " class" + (hidden > 1 ? "es" : "") + " unlock as you travel";
        wrap.appendChild(n); }
    }
  }
  function renderEco() {
    const wrap = $("eco-list"); if (!wrap) return; wrap.innerHTML = "";
    { const n = document.createElement("div"); n.id = "cap-note"; n.className = "tab-note"; n.textContent = "⚠ Wallet PINNED at the Capacity ceiling — everything you earn past it is lost. Raise Capacity."; wrap.appendChild(n); }
    for (const u of UPS) { if (u.tab !== "eco") continue;
      const el = document.createElement("div"); el.className = "up";
      el.innerHTML = `<span class="u-mile"></span><span class="u-shine"></span><span class="u-dot" style="background:var(--eco)"></span><div class="u-mid"><div class="u-name">${u.name}<span class="lv"></span><span class="u-milechip">✦ MILESTONE</span></div><div class="u-desc"></div></div><button class="u-info" title="Info">i</button><button class="u-buy"></button>`;
      wrap.appendChild(el);
  ...
  // v18.72 (owner: "the upgrade tree is having some issues with resetting position when I click
  // certain things"): this used to call STree.resize() on every toggle, because v18.71 made the
  // panel a flex item that stole height from the canvas. It does not steal height any more — it is
  // anchored inside .st-body — so there is nothing to re-measure and, crucially, nothing to move.
  const stPanel = on => { const p = $("st-info"); if (p && p.classList) p.classList.toggle("show", on); };
  function showNodeInfo(n) {
    const panel = $("st-info"), type = STree.type;
    if (!n || n.kind === "start") { stPanel(false); STree.sel = n ? n.id : null; return; }
    STree.sel = n.id;
  ...
  const STree = {
    ...
    render(dt) {
      ...
      // v18.72: the canvas only re-measured on a WINDOW resize, so any layout change from inside the
      // game (the nav growing a tier, the PICK-mode pill appearing) left the backing store the wrong
      // size and the tree drawn stretched. Self-heal: if the CSS box has drifted, re-measure.
    ...
    tap(x, y) {
      ...
      if (!best) { this.sel = null; stPanel(false); return; }
      showNodeInfo(best.n);
    },
  ...
  function showGalaxyInfo(g) {
    ...
    const action = current ? "<span class='gi-tag'>▶ You are here</span> <button id='gi-visit'>⊙ Zoom to base ▸</button>"
      ...
      : next ? (conqHere
          ? "<button id='gi-travel'>Travel here ▸  free</button>"   // v18.79: no price, so no locked-out state to render
          : "<span class='gi-tag'>" + iconMarkup("lock") + "Conquer " + galName(S.galaxy) + " first</span>")
      : "<span class='gi-tag'>" + iconMarkup("lock") + "Conquer earlier planets first</span>";
  ...
  const INFO = {
    ...
    swarm: "Faster with a wider net — covers more of the field than a lone collector.",
    ...
    blackhole: "Drags every dot on the map into your guns for 5s. It deals no damage itself — your defenders do the killing, they just get everything at once. Cooldown 60s.",
  };
  ...
  function pendSrc() {
    ...
    if (!parts.length) return "take a planet and it fills";
  ...
  // v18.65: a verdict, not an essay. Each of these was a sentence and a half explaining its own
  // reasoning; the page already shows the haul, the bars and the ETA, so the line only has to COMMIT.
  function ascVerdict(pend) {
    const eta = wallEtaH();
    if (pend < 1) return { kind: "none", line: "Take a planet first" };
    if (!isFinite(eta)) return { kind: "wait", line: "No income here yet" };
    if (eta <= 0) return { kind: "go", line: "Planet taken — hop when you like" };
    if (eta > ASC_HOP_H) return { kind: "go", line: "THE WALL — hop now · " + fmtHrs(eta) + " left" };
    return { kind: "wait", line: "Take this bar first · " + fmtHrs(eta) };
  }
  ...
  // v18.65: sub-captions cut (owner: "less wordy"). The numbered icon and a two-or-three word title
  // ARE the step; "planets · units · cash" under "LOSE THE RUN" only restated it. 32 words -> 11.
  const ASC_LOOP = [
    ["reset",  "LOSE THE RUN"],
    ["gem",    "PLANETS PAY ◈"],
    ["coin",   "◈ BUY INCOME"],
  ...
  const ascLoopHtml = () => '<div class="ax-loop">' + ASC_LOOP.map((s, i) =>
    ...
    + iconMarkup(s[0]) + '<b>' + s[1] + '</b></div>').join('') + '</div>';
  ...
  function ascHeroHtml(pend) {
    ...
    return '<div class="ax-hero ' + v.kind + '">'
      // v18.76: this said "NOTHING BANKED" while the header right above it read "1 ◈ banked" — two
      // numbers on one screen contradicting each other. It was never about the bank: it is what THIS
      // RUN would hand over if you ascended now. A wheel core makes that difference visible, because
      // it is the one way to hold cores with nothing staged.
      + '<span class="ax-k">' + (pend < 1 ? "NOTHING TO BANK YET" : "YOU BANK") + '</span>'
      + '<b class="ax-haul">' + (pend < 1 ? "—" : "+" + fmt(pend) + iconMarkup("gem", "ax-gem")) + '</b>'
      + body
      + ascNextHtml()
  ...
  function ascEngineHtml() {
    ...
    return '<div class="ax-eng"><div class="ax-eng-head">'
      ...
      + '<div class="ax-eng-now"><b>×' + multFmt(Math.pow(ASC_E, lv)) + '</b><span>+25% income per level</span></div>'
  ...
  function renderAscend() {
    ...
    const html = ascLoopHtml()
      ...
      + ascSplitHtml()
      + ascSourcesHtml()
      ;   // v18.65: the 22-word ladder paragraph is gone — the verdict above already tells you when to
          // hop, and the same advice still lives inside the WHERE THE ◈ COME FROM details.
  ...
  function buildMetrics() {
    ...
    $("metrics-body").innerHTML =
      ...
      sec("Empire", grid(
        ...
        row("Empire idle income", "none — planets pay ⚑ spoils once, then nothing (v18.43)"))) +
      sec("Economy", grid(
  ...
  const GMap = {
    ...
    focusSys(si) {
      ...
      this.navSys = si;   // v18.75: the tab you picked is the system the grid lists, until you pick a planet
      { const gi = $("gm-info"); if (gi && gi.classList) gi.classList.remove("show"); }   // the open card describes a planet you have just navigated away from
      this.focusSystem(si); this.tzoom = this.restZoom();
      this.updateNav();
    },
    // v18.75 THE NAVIGATOR. The old bar was a planet STEPPER (◀ P2 · Em… ▶) plus three system pills,
    // all truncated past readability. This lays the chosen system out in full: pick a system, see
    // every world in it by name and number, each wearing where you are, where you have been and how
    // far your frontier reaches. Nothing to step through, so the steppers are gone.
    navSysShown() {
      if (this.navSys != null) return this.navSys;
      return PLANET_SYS[planetIdx(this.navG || S.galaxy)];
    },
    updateNav() {
      const si = this.navSysShown();
      for (let i = 0; i < 3; i++) { const b = $("gm-sys" + i); if (!b) continue;
        const sy = SYSTEMS[i];
        const html = '<span class="ms-n">' + (sy ? sy.name.toUpperCase() : "SYS " + (i + 1)) + '</span>'
          + '<span class="ms-c">' + (sy ? sy.planets : 0) + ' WORLDS</span>';
        if (b._h !== html) { b._h = html; b.innerHTML = html; }
        b.classList.toggle("on", si === i); }
      const grid = $("gm-planets"); if (!grid) return;
      // planet numbers of the shown system, in order
      const list = []; for (let g = 1; g <= TOTAL_PLANETS; g++) if (PLANET_SYS[planetIdx(g)] === si) list.push(g);
      // rebuild only when the SET changes; state classes are cheap and updated every call
      const key = si + ":" + list.join(",");
      if (grid._key !== key) { grid._key = key; grid.innerHTML = ""; grid._btn = {};
        for (const g of list) { const el = document.createElement("button");
          el.className = "mpl"; el.dataset.g = g;
          el.innerHTML = '<span class="mp-n">' + g + '</span><span class="mp-l">' + galName(g) + '</span>';
          el.onclick = () => { Audio_click(); GMap.navSys = null; GMap.focusPlanet(g); showGalaxyInfo(g); };
          grid.appendChild(el); grid._btn[g] = el; } }
      for (const g of list) { const el = grid._btn[g]; if (!el) continue;
        const v = S.vault && S.vault[g];
        el.classList.toggle("here", g === S.galaxy);
        el.classList.toggle("done", !!(v && v.conquered));
        el.classList.toggle("on", g === this.navG && g !== S.galaxy);
    ...
    render(dt) {
      ...
      if (this.flight) {                                     // zoom-into-base animation overrides the camera
        ...
        if (p >= 1 && !fl.done) { fl.done = true; const cb = fl.onArrive, gg = fl.g; this.flight = null; this._warp = 1; this._diveP = null; if (tv) { tv.style.background = "#000"; tv.style.opacity = "1"; } if (cb) cb();
          ...
          seedField(SEED_N);   // v18.60: land into a live field, not a 30-second walk-in
    ...
    tap(x, y) { let best = null, bd = Infinity; for (const h of this.hit) { const q = (h.x - x) ** 2 + (h.y - y) ** 2; if (q < bd && q < h.r * h.r) { bd = q; best = h; } }
      if (!best) return;
      // v18.75: tapping the CANVAS has to move the navigator too, or the dock keeps listing a
      // system you are no longer looking at — the map and its own nav would disagree on screen.
      if (best.sun != null) { this.navSys = best.sun; this.navG = null; this.focusSystem(best.sun); this.sel = null; $("gm-info").classList.remove("show"); this.updateNav(); }   // tap a sun -> recenter on its system
      else { this.sel = best.g; this.navSys = null; this.navG = best.g; showGalaxyInfo(best.g); this.updateNav(); } },
  };
  ...
  function travel() {   // LAUNCH an expedition to the next planet: costs treasury + takes a real journey
    ...
    // v18.79: launching is FREE. Kept as a priced path rather than deleted (launchPrice() reads
    // travelCost(), which is one line) so a price could return without re-threading travel — but at
    // zero there is nothing to check and nothing to deduct, and no way to be stranded.
    const cost = launchPrice();
    if (cost > 0) { if (!(S.cash >= cost)) return; S.cash -= cost; }
  ...
  // v18.67: galaxy-map, skilltree and the new upgrades screen were NOT in this list, so closeCards()
  // left them open underneath whatever you opened next — the audit found the star map still live
  // (gm-close, gm-next, gm-sys0…) behind the ascension page. Every screen closes every other screen.
  const CARD_MODALS = ["menu", "settings", "metrics", "ascend", "auto-modal", "how", "victory", "info-modal",
    "galaxy-map", "skilltree", "upgrades", "economy"];
  ...
  function setScreen(s) {
    ...
    // v18.66: #btn-menu and #btn-metrics no longer exist — the persistent nav owns those
    // destinations. #nav itself is shown only in play, below.
    { const nv = $("nav"); if (nv) nv.style.display = (s === "play") ? "flex" : "none"; }
  ...
  // v18.70: the banner is a nav route like any other — go through navGo so PLANETS lights up and the
  // dock stands down, and so leaving puts the bar back on PLAY instead of stranding it.
  $("galaxy-open").onclick = () => navGo("map"); $("gm-close").onclick = () => { closeCards(); GMap.hide(); };
  ...
  { const nv = (id, fn) => { const b = $(id); if (b) b.onclick = () => { Audio_click(); fn(); }; };
    // v18.75: ◀ ▶ and the truncated "P2 · Em…" pill are gone — every planet in the system is its own
    // chip now, built in updateNav(), so there is nothing left to step through or to abbreviate.
    nv("gm-you", () => { GMap.focusPlanet(S.galaxy); showGalaxyInfo(S.galaxy); });
  ...
  const openMetrics = () => { closeCards(); buildMetrics(); $("metrics").classList.add("show"); };
  ...
  const openMore = () => { closeCards(); $("menu").classList.add("show"); };
  for (const id of ["up-close", "eco-close"]) { const uc = $(id); if (uc) uc.onclick = () => navGo("play"); }
  // v18.70: MORE left the nav so PLAY could stay in the middle with ECONOMY beside it (five
  // destinations, not six). Settings/stats/how-to are META, not a system you play — the ☰ in the
  // progress banner is the right weight for them, and it is on screen at all times either way.
  // stopPropagation is load-bearing: the ☰ sits INSIDE the banner, and the whole banner opens the
  // star map — without it every tap on MORE opened PLANETS underneath the menu.
  { const bm = $("btn-more"); if (bm) bm.onclick = e => { if (e && e.stopPropagation) e.stopPropagation(); navGo("more"); }; }
  { const ms = $("menu-stats"); if (ms) ms.onclick = () => { closeCards(); buildMetrics(); $("metrics").classList.add("show"); }; }

  // ═══ v18.66 THE NAV — the game's destinations, at thumb size, with their names on them ═══
  // Measured before this: thirteen screens existed and every one was reached from a 46px UNLABELLED
  // corner icon (#btn-menu), an icon captioned "▁▄█" (#btn-metrics), a "+0" chip that never said the
  // word ASCENSION, a tap on the planet bar with no affordance saying "map", or a button buried
  // inside a shop row. That is the "small buttons on one screen" complaint exactly.
  // The nav is persistent, sits ABOVE the modals (z-index) so switching screens is one tap from
  // anywhere, and carries live state — the ASCEND item wears the pending ◈ count as a badge.
  const NAV_OPEN = {
    map:      () => { closeCards(); $("galaxy-map").classList.add("show"); GMap.show(); syncAutoBtn(); },
    upgrades: () => { closeCards(); renderList(); $("upgrades").classList.add("show"); },
    economy:  () => { closeCards(); renderList(); $("economy").classList.add("show"); },
    play:     () => closeCards(),
    ascend:   () => openAscend(),
    more:     () => openMore(),
  };
  // v18.70: which element each destination actually OPENS. Half a dozen close buttons and the whole
  // ESC ladder remove a modal's .show directly without going through closeCards(), which left the
  // nav claiming you were on a screen that was no longer there — and, because the dock only shows on
  // PLAY, left the field with no dock at all. Rather than patch every exit (and every future one),
  // syncHUD reconciles once a frame: if the screen navAt names is not open, you are on PLAY.
  const NAV_EL = { map: "galaxy-map", upgrades: "upgrades", economy: "economy", ascend: "ascend", more: "menu" };
  let navAt = "play", navBusy = false, navH = 0;
  function navGo(k) {
    if (!NAV_OPEN[k]) return;
    // v18.67: every NAV_OPEN handler calls closeCards() first, and the wrapper below resets navAt to
    // "play" — so navigating anywhere immediately un-navigated itself and the PLAY dock came back
    // underneath the screen you just opened. The flag makes the reset apply only to closeCards()
    // calls that come from somewhere ELSE (a ✕, the DONE pill, ESC).
    navBusy = true;
    try { navAt = k; Audio_click(); NAV_OPEN[k](); } finally { navBusy = false; }
    syncNav();
  }
  // v18.68: with the shop moved out, the objective moved out and no action to take, the dock could
  // be left holding nothing but its own "Minimise" toggle. An empty container should not be on screen.
  function syncDockEmpty() {
    const d = $("dock"); if (!d || !d.classList) return;
    // v18.78: the Minimise toggle only earns its place when there is something BULKY to collapse —
    // the settlement panel is 117px, the action row is one button. Offering to minimise a single
    // button is what put a chrome pill on top of ▲ SUMMON in the first place.
    { const tg = $("dock-toggle"), sp = $("settle-panel");
      const worth = !!(sp && sp.style && sp.style.display !== "none");
      if (tg && tg.style) { const want = worth ? "" : "none";
        if (tg.style.display !== want) tg.style.display = want; }
      if (!worth && d.classList.contains("min")) { d.classList.remove("min");   // never leave the dock stuck collapsed with no way back
        if (tg) tg.textContent = "\u25be Minimise"; } }
    // NB: balance-check evaluates this file under a MINIMAL document stub whose elements have no
    // .children (and no querySelector — see qs() above). Everything here degrades instead of throwing.
    const alive = ["settle-panel", "actions"].some(id => { const e = $(id);   // v18.69: abilities left the dock for the nav
      if (!e || !e.style || e.style.display === "none") return false;
      if (id === "settle-panel") return true;
      const kids = e.children ? Array.prototype.slice.call(e.children) : null;
      return kids ? kids.some(c => c.style && c.style.display !== "none") : true;
    });
    d.classList.toggle("empty", !alive);
  }
  function syncNav() {
    for (const b of document.querySelectorAll("#nav .nv")) b.classList.toggle("sel", b.dataset.nav === navAt);
    // v18.67: the dock is PLAY's furniture — objective, abilities, the launch row. It used to sit
    // under every screen you opened (the audit found ab-frenzy, DEFENCE, ✦393 and btn-travel still
    // present behind the ascension page), which is exactly the "everything on one screen" problem.
    const d = $("dock"); if (d) d.style.display = (navAt === "play" && state === "play") ? "flex" : "none";
    syncDockEmpty();
  }
  for (const b of document.querySelectorAll("#nav .nv")) b.onclick = () => navGo(b.dataset.nav);
  // any other route back to the field (a close button, ESC, the DONE pill) must un-select too, or
  // the bar keeps claiming you are on a screen you already left
  { const back = closeCards;
    closeCards = function () { back.apply(this, arguments); if (!navBusy) { navAt = "play"; syncNav(); } }; }
  // v18.70: MORE is reached by navGo (banner ☰), so leaving it must go back through closeCards() —
  // removing the class directly left navAt stuck on "more" and the PLAY dock hidden after you closed.
  $("menu-close").onclick = () => closeCards();
  $("menu-resume").onclick = () => closeCards();
  ...
  // v18.60 — TWO STEPS, NOT TEN. The old opener was 331 words across ten cards (~100 seconds of
  // reading) delivered before the player had killed a single dot: step 4 taught keystones and
  // multishot, step 8 taught three solar systems and racial weaknesses, and step 9 taught PRESTIGE,
  // a system nobody touches for hours. Every source on idle onboarding says the same thing — front
  // loading every system in the first ten minutes is what makes new players bail.
  // What survives is the two things you must know to act at all. Everything else is taught by the
  // REVEALS table above, one line at a time, at the moment that system first becomes relevant.
  const TUT_STEPS = [
    // no sel: the field IS the whole screen, so a spotlight on it lights everything and dims nothing.
    // This step gets the centred-card-on-a-dim-backdrop treatment instead (see Tut.place).
    { t: "Drag to shoot", x: "<b>Drag across the field</b> — go on, try it now. Your defenders also fire on their own, and <b>killing dots is your entire economy</b>." },
    // the spotlight covers #up-list only, so the copy must not ask the player to read something
    // outside it — v18.62 fixed the objective being hidden behind the tutorial, and this stops the
    // step pointing at a line the spotlight does not actually light up.
    // v18.67: the shop moved out of the dock onto its own screen, so this step points at the way IN
    // rather than at a list that is not on this screen. (It targeted #up-list, which now lives inside
    // a hidden panel — the spotlight would have had nothing to ring.)
    { sel: '#nav .nv[data-nav="upgrades"]', t: "Everything you buy", x: "Defenders and collectors live in <b>ARMY</b>; your income upgrades live in <b>ECONOMY</b>, the other side of PLAY. Open ARMY and the line at the top always tells you <b>what to buy next</b>." },
  ];
  const Tut = {
    i: 0,
    ...
    // v18.64: the spotlight is re-measured EVERY FRAME while the tutorial is up. It used to be
    // positioned once at render(), so anything that reflowed underneath — a progressive reveal
    // firing mid-tutorial and adding the COLLECTORS tab, the shop list growing a row — left the
    // highlight ring floating over empty space, pointing at nothing.
    place() {
      const s = TUT_STEPS[this.i]; if (!s) return;
      const wrap = $("tutorial"), spot = $("tut-spot"), card = $("tut-card");
      const el = s.sel ? document.querySelector(s.sel) : null;
      let r = el && el.getBoundingClientRect();
      // A "spotlight" the size of the screen highlights nothing AND cancels the dim — which is
      // exactly what step 1 did by targeting #game, the full-viewport canvas: the card floated over
      // a fully lit game with no focus at all. Anything covering most of the viewport falls back to
      // the centred-card treatment, which is what such a step actually wants.
      if (r && r.width * r.height > innerWidth * innerHeight * 0.55) r = null;
  ...
  document.addEventListener("keydown", e => {
    ...
    if (e.key === "Escape") {
      ...
      // v18.70: ARMY and ECONOMY were never on this ladder — Esc closed every other screen in the
      // game and left the two shops sitting there. They go through navGo so the nav follows.
      else if (vis("upgrades") || vis("economy")) navGo("play");
  ...
  $("home-play").onclick = () => { renderList(); setScreen("play"); if (!dots.length) seedField(SEED_N); if (!META.tutorialDone) setTimeout(() => Tut.start(), 550); };   // first-run coach marks once the play UI is laid out
  ...
  if (typeof window !== "undefined") window.__SIM = {
    ...
    queueCoreFx, coreFxTarget, coreFxLast: () => coreFxLast,   // v18.81: tools/play-hud.js flies a real core and asks where it landed
    hitDot,   // v18.79: tools/boss-feel.js drives real damage through the real path, knockback and all
    bossLeft: () => Math.max(0, BOSS_INTERVAL - bossAcc), setBossAcc: v => { bossAcc = +v || 0; }, BOSS_INTERVAL,   // v18.77: the countdown's own state, for tools/boss-timer.js
    ...
    baseTarget, conquerHours, IDLE_FRAC, ACTIVE_REF, IDLE_PAYBACK_H, EMPIRE_RAMP, SPOILS_PAYOUT_H,   // v18.79: spoilsRate is gone — a conquered world pays nothing, so there is no rate to export
