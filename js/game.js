...
(() => {
  ...
  const VERSION = "v18.87";   // v18.87 = THE POWERS STAY ON THE PLAY SCREEN (owner: "the 3 temporary boosts like black hole, the boxes — make sure on other screens they're not affecting things"). MEASURED BOTH WAYS THEY COULD. GEOMETRY IS CLEAN and stays that way under a gate: #abilities is the first child of #nav, but the nav grows UPWARD (107px on play against 48px elsewhere), so the destination row never moves — top 849 and ASCEND's centre 873 on every screen — and a tap where the row sits lands on the panel underneath (gm-planets / up-list / eco-list / ascend), never on a ghost box. REACH WAS NOT CLEAN. useAbility guarded on `state`, which is the HOME-vs-PLAY screen and stays "play" on every nav tab, so it only ever blocked the home screen. The boxes are hidden off the play tab, which made it look handled — but the 1/2/3 KEYBOARD SHORTCUT calls useAbility directly and its own guard excludes only `.modal.show`. The star map is a modal; the Economy and Army panels are not. So on the desktop shell you could burn a 60-second Black Hole while reading the shop: cooldown paid, effect unseen. Now gated at the SOURCE on navAt, so the buttons, the keys and anything added later all inherit it. My first probe of this said the opposite and was wrong in a way worth recording: abil maps key -> COOLDOWN NUMBER, and I reset it as if the values were objects (a[k].cd = 0), so every call after the first refused on cooldown and I read that as the screen guard working. It was firing on all five screens. tools/ability-check.js gains A4 (the row is on play and nowhere else, and never shifts the nav) and A5 (an ability cannot be SPENT off the play screen); reverting the guard reports A5 on all four other screens.
  ...
  // =============== v18.82 ARMY POWER — WHAT THE ENEMY IS ACTUALLY UP AGAINST ===============
  // Owner: "near max turrets, planet 3, level 50 economy — the turrets are completely carrying, and
  // as soon as it pops in radius it dies." Measured, and it is not a feel, it is arithmetic. dmg and
  // rate each grow LINEARLY with node count and then MULTIPLY, so a finished tree is a QUADRATIC in
  // nodes: a full turret reached ×85 dmg × ×56.9 rate × ×3.44 crit = ×16,600, and nova ×216,000.
  // Against that, everything on the enemy's side of the ledger moves ×13 — three planets of
  // enemyHpMul is ×1.63, and fifty levels of Value is ×8.1. Measured on the live field at P3: the
  // army could kill 3,246 dots a second while the field asked for 28, so it stood idle 99.1% of the
  // time and the average number of dots INSIDE a gun's radius was 0.5. There was nothing to watch.
  //
  // The first attempt was a soft knee on dmg and rate, and it is worth recording why that was wrong,
  // because it looked right: it SATURATED. Swept across knee 3-16 and pass 0.08-0.30, the in-range
  // population stalled around 5 whatever I did, because crit (×3.4), multishot (×5) and the unit
  // count are not in the knee — you cannot fix a SLOPE mismatch by bending one of its terms.
  //
  // What is actually wrong is the slope, so the fix is on the slope. A dot's armour now rides the
  // army's OWN invested tree power, and because tree power is quadratic in nodes (P ~ n²), taking it
  // to the ~0.5 makes the player's NET gain per node roughly LINEAR — which is the shape the rest of
  // the game already has. Three properties come free, and they are why this beats the knee:
  //   · the cold open is untouched BY CONSTRUCTION, not by tuning — a bare tree is P=1 is ×1 armour,
  //     so the opening minutes are bit-identical, and they are tuned to the gram.
  //   · the keystone CLIFF damps itself. The last quarter of a tree was worth ×9.3 raw; through a
  //     0.55 exponent the player feels 9.3^0.45 = ×2.7. No separate anti-cliff rule to keep in sync.
  //   · not one number in the economy moves. Payout keys off hp/avg (see spawnDot) where the base
  //     cancels exactly, so dots get tougher and income, conquer targets and prices are all identical.
  ...
  const armyPower = () => {
    ...
    for (const t of DEF_ORDER) {
      if (!S.units || !S.units.some(u => u.type === t)) continue;   // only classes you actually FIELD
      const c = (derived.cls || {})[t]; if (!c) continue;
      p += Math.max(1, c.dmg * c.rate); n++;
    }
    return n ? p / n : 1;
  };
  const armyArmor = () => Math.pow(Math.max(1, armyPower()), MENACE_TREE);
  ...
  // v18.82: was a flat 0.4. Measured across the planets it governs, 0.4 bought ×1.63 of enemy HP for
  // THREE whole planets (P1 ×1.00 → P4 ×1.63) — travelling to a new world was not a step up, it was a
  // rounding error, which is half of why a P3 field folded on contact. Raising the exponent is the
  // cheapest honest lever in the file because payout keys off hp/avg (see spawnDot), where the base
  // cancels: dots get genuinely tougher and NOT ONE cash number moves. HP_POW is a let so the sweep in
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    // v18.82: armyArmor() is the third factor and it belongs in BASE for the same reason the other two
    // do — everything in `base` cancels out of the payout below (val keys off hp/avg), so this makes
    // dots tougher without moving a single number in the economy. See ARMY POWER above.
    const g = S.galaxy, vscale = Math.pow(derived.valueMul, 1.3), base = 18 * enemyHpMul(g) * vscale * armyArmor();   // HP scales SUPER-linearly with Value — Value genuinely & heavily toughens enemies; cash is unaffected (it keys off hp/avg, where base cancels)
  ...
  function useAbility(k) {
    // v18.87 (owner: "the 3 temporary boosts — make sure on other screens they're not affecting
    // things"). `state` is the HOME-vs-PLAY screen and stays "play" on every nav tab, so this guard
    // only ever blocked the home screen. The boxes are hidden off the play tab, which made it look
    // handled — but the 1/2/3 KEYBOARD SHORTCUT goes straight here, and its own guard only excludes
    // `.modal.show` (the star map). The Economy and Army panels are not modals, so on the desktop
    // shell you could burn a 60-second Black Hole while reading the shop, with a cooldown you paid
    // for and an effect you never saw. Gated at the source rather than at each call site, so the
    // buttons, the keys and anything added later all inherit it.
    if (abil[k] > 0 || state !== "play" || navAt !== "play") return;
  ...
  if (typeof window !== "undefined") window.__SIM = {
    ...
    // v18.82 balance knobs, so tools/overkill.js can SWEEP them against the measured in-range
