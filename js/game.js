...
(() => {
  ...
  function ascHeroHtml(pend) {
    ...
    let xform;
    if (after > now)
      xform = '<span class="ax-from">×' + multFmt(now) + '</span><span class="ax-arrow">→</span>'
        + '<span class="ax-to">×' + multFmt(after) + '</span><span class="ax-xk">income, forever</span>';
    else
      xform = '<span class="ax-from">×' + multFmt(now) + '</span><span class="ax-xk">income · '
        + (lv + aftR.levels >= l.max ? "Engine maxed"
           : aftR.short > 0 ? fmt(aftR.short) + "◈ more reaches the next level"
           : "the next level is already yours to build")
        + '</span>';
    return '<div class="ax-hero ' + v.kind + '">'
      ...
      + '<div class="ax-xform">' + xform + '</div>'
      + '<div class="ax-verdict">' + v.line + '</div></div>';
  }

  // THE TRADE — prestige asks you to throw a run away, and the old page answered that in one
  // run-on sentence at the very bottom. Two columns, no prose, read in a second.
  const ascTradeHtml = () => '<div class="ax-trade">'
    + '<div class="ax-col lose"><span class="ax-col-h">✕ RESETS</span>'
    + ['planets', 'empire', 'units', 'skill trees', 'cash'].map(s => '<span>' + s + '</span>').join('')
    + '</div><div class="ax-col keep"><span class="ax-col-h">✓ KEEPS</span>'
    + ['◈ cores', 'Engine levels', 'every ◈ ever banked'].map(s => '<span>' + s + '</span>').join('')
    + '</div></div>';

  // THE ENGINE — the only sink cores have. The old page plotted x-income against cumulative cores
  // spent, which is the mathematically interesting view and the least actionable one on a phone.
  // The question in your head is "what do my cores actually get me", so this answers it literally:
  // the next three levels, their RUNNING total cost, and the multiplier each one lands you on.
  function ascEngineHtml() {
    ...
    return '<div class="ax-eng"><div class="ax-eng-head">'
      + '<span class="ax-eng-t">' + iconMarkup(l.ico) + 'SINGULARITY ENGINE</span>'
      + '<span class="ax-eng-lv">Lv ' + lv + ' / ' + l.max + '</span></div>'
      + '<div class="ax-eng-now"><b>×' + multFmt(Math.pow(ASC_E, lv)) + '</b><span>to ALL income · +25% a level · never resets</span></div>'
      + (steps ? '<div class="ax-steps">' + steps + '</div>' : '')
      + '<div class="ax-eng-sub">' + (maxed ? "★ fully built"
          : reach > lv ? "your <b>" + fmt(banked) + "◈</b> reach <b>Lv " + reach + "</b> right now"
          : "you hold <b>" + fmt(banked) + "◈</b> — the next level costs <b>" + fmt(c) + "◈</b>") + '</div>'
  ...
  function renderAscend() {
    ...
    const html = ascHeroHtml(pend)
      + ascTradeHtml()
      + ascEngineHtml()
      + ascMinesHtml()
      + ascSplitHtml()
      + ascSourcesHtml()
      + '<p class="muted ax-ladder"><b>THE LADDER:</b> hop at ~3 worlds on run 1, then ride every ascension a few worlds deeper. Grinding a wall is never faster than hopping it.</p>';
