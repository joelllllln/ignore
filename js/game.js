...
(() => {
  ...
  function update(dt) {
    ...
    if (minePendingBuild && conqCardT <= 0 && !Cards.busy()) {
  ...
  function conquerWorld() {
    ...
    pm.conquered = true; pm.bgRate = Math.max(pm.bgRate || 0, baseTarget(S.galaxy) / (IDLE_PAYBACK_H * 3600)); pm.spoils = Math.round(SETTLE_SPOILS_FRAC * conquerTarget(S.galaxy)); const cg = coreVal(S.galaxy); recompute();
  ...
  function ascProductionHtml() {
    const A = (META && META.asc) || { cores: 0, lv: {} };
    const pend = pendingCores(), rateD = mineRateTotal(), banked = A.cores | 0;
    const eng = ascLv("engine") || 0;
    let html = '<div class="asc-viz">';

    // ── KPI ROW — four headline numbers. A number is not a chart. ──────────
    html += '<div class="av-tiles">'
      + ascTile("◈ BANKED", fmt(banked), "spendable now")
      + ascTile("◈ PENDING", "+" + fmt(pend), "if you ascend")
      + ascTile("⛏ DIG RATE", rateD > 0 ? fmtMineRate(rateD) : "—", rateD > 0 ? "every mine, always" : "no mines yet")
      + ascTile("✦ ENGINE", "×" + multFmt(Math.pow(ASC_E, eng)), "Lv " + eng + " · all income")
      + '</div>';

    // ── PER-PLANET PRODUCTION — horizontal bars, one row per world you hold ─
    const rows = [];
    for (let g = 1; g <= TOTAL_PLANETS; g++) { const v = S.vault && S.vault[g];
      if (v && v.conquered) rows.push({ g, name: galName(g), rate: v.mine ? mineRate(g) : 0, bounty: coreVal(g), mine: !!v.mine }); }
    if (rows.length) {
      const maxR = Math.max(...rows.map(r => r.rate), 0.0001);
      html += '<div class="av-sec"><div class="av-h">⛏ ◈ DUG PER DAY, BY WORLD</div>'
        + '<div class="av-sub">every mine runs on the wall clock — asleep, away, all of it</div><div class="av-bars">';
      for (const r of rows) {
    ...
    // ── THIS RUN'S ◈, PART TO WHOLE — conquest bounties vs what the mines dug ─
    { const bounty = pend;                       // what ascending would bank right now (bounties + part-bar)
      const dug = S.minedRun | 0;                // what the mines have already banked this run, live
      const tot = bounty + dug;
      if (tot > 0 && dug > 0) {
        const bp = bounty / tot * 100;
        html += '<div class="av-sec"><div class="av-h">WHERE THIS RUN’S ◈ CAME FROM</div>'
          + '<div class="av-stack">'
          + '<span class="av-seg s1" style="width:' + bp.toFixed(1) + '%"></span>'
          ...
          + '<span><i class="av-sw s2"></i>mined, already banked <b>' + fmt(dug) + '◈</b></span></div></div>';
      } else if (tot > 0) {                        // one source only — a 100% stacked bar is not a chart
        html += '<div class="av-sec"><div class="av-h">WHERE THIS RUN’S ◈ CAME FROM</div><div class="av-sub">'
          + (dug > 0 ? 'all <b>' + fmt(dug) + '◈</b> of it dug by your mines so far — conquest pays the rest when you ascend'
                     : 'all of it from conquest so far — <b>' + fmt(bounty) + '◈</b> pending. Your mines bank theirs the moment each whole ◈ lands.')
          + '</div></div>';
      } }

    // ── BANK vs DIG — two bars, ONE ◈ axis. Ascending should always win. ────
    if (rateD > 0 && pend > 0) {
      const days = pend / rateD, m = Math.max(pend, rateD);
      html += '<div class="av-sec"><div class="av-h">ASCEND NOW, OR SIT ON THE FARMS?</div><div class="av-bars">'
        + '<div class="av-row"><span class="av-lbl">ascend now</span><span class="av-track"><span class="av-bar" style="width:'
        + (pend / m * 100).toFixed(1) + '%"></span></span><span class="av-val">+' + fmt(pend) + '◈</span></div>'
        + '<div class="av-row"><span class="av-lbl">mines, one day</span><span class="av-track"><span class="av-bar dim" style="width:'
        + (rateD / m * 100).toFixed(1) + '%"></span></span><span class="av-val">' + fmtMineRate(rateD) + '</span></div>'
        + '</div><div class="av-sub">parked, the same ◈ takes <b>' + (days >= 1 ? Math.round(days) + " days" : Math.max(1, Math.round(days * 24)) + "h") + '</b> to dig</div></div>';
    }

    // ── THE ENGINE — multiplier against ◈ ACTUALLY SPENT, which is the real decision ──
    // Plotting ×income against LEVEL would be a straight line (+25% flat) and tell you nothing. Against
    // cumulative cores spent it bends, and the bend IS the question: how much further does the next ◈ go.
    { const lv = eng, span = 10, pts = []; let spend = 0;
      for (let i = 0; i <= span; i++) {
        pts.push({ x: spend, y: Math.pow(ASC_E, lv + i) });   // the REAL curve: the Engine compounds (ASC_E^lv), it does not add
        spend += ascCost(ASC_BY.engine, lv + i);
      }
      const xMax = Math.max(1, pts[pts.length - 1].x), yMin = pts[0].y, yMax = pts[pts.length - 1].y;
      // viewBox aspect ≈ the rendered box, and NO preserveAspectRatio override: stretching the box turns
      // every circular marker into an ellipse, which reads as a mistake rather than a mark.
      const PADL = 8, PADR = 10, PADT = 8, PADB = 12, W2 = 300, H2 = 56;
      const px = v => PADL + (v / xMax) * (W2 - PADL - PADR);
      const py = v => H2 - PADB - ((v - yMin) / Math.max(0.001, yMax - yMin)) * (H2 - PADT - PADB);
      let d = ""; pts.forEach((pt, i) => { d += (i ? "L" : "M") + px(pt.x).toFixed(1) + " " + py(pt.y).toFixed(1) + " "; });
      const afford = A.cores | 0, reach = pts.filter(pt => pt.x <= afford).pop() || pts[0];
      html += '<div class="av-sec"><div class="av-h">✦ SINGULARITY ENGINE — WHAT THE NEXT ◈ BUY</div>'
        + '<svg class="av-svg" viewBox="0 0 ' + W2 + ' ' + H2 + '" role="img" aria-label="income multiplier against cores spent">'
        + '<line class="av-grid" x1="0" y1="' + (H2 - PADB) + '" x2="' + W2 + '" y2="' + (H2 - PADB) + '"/>'
        + '<path class="av-line" d="' + d.trim() + '"/>'
        + '<circle class="av-dot" cx="' + px(0).toFixed(1) + '" cy="' + py(pts[0].y).toFixed(1) + '" r="4.5"/>'
        + (reach !== pts[0] ? '<circle class="av-dot ghost" cx="' + px(reach.x).toFixed(1) + '" cy="' + py(reach.y).toFixed(1) + '" r="4"/>' : '')
        + '</svg>'
        + '<div class="av-axis"><span>now · Lv ' + lv + ' · ×' + multFmt(pts[0].y) + '</span>'
        + '<span>◈ ' + fmt(Math.round(xMax)) + ' more · ×' + multFmt(yMax) + '</span></div>'
        + '<div class="av-sub">' + (reach !== pts[0]
            ? 'your <b>' + fmt(afford) + '◈</b> reach <b>×' + multFmt(reach.y) + '</b> right now'
            : 'the next level costs <b>' + fmt(ascCost(ASC_BY.engine, lv)) + '◈</b>')
        + ' — +25% to ALL income per level, and it never resets. Conquer targets never move, so every level melts the early worlds.</div></div>';
    }
  ...
  function renderAscend() {
    ...
    let html = ascProductionHtml() + '<button class="big asc-go" id="ascend-go"' + ((pend < 1 || locked) ? " disabled" : "") + '>' + (locked ? "\u25c8 CONQUER 3 PLANETS TO ASCEND (" + conqN2 + "/3)" : "\u25c8 ASCEND NOW \u2014 BANK +" + fmt(pend)) + '</button>'
      + coach
      + srcHtml
