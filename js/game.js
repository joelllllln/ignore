...
(() => {
  ...
  // INTO the conquest. Offline now obeys the same rule as live idle — cash still banks in FULL, but
  // the conquer BAR only moves at the idle allowance, so worlds are taken by playing, not by waiting.
  const offlineBarGain = (total, secs) => Math.max(0, Math.min(+total || 0, IDLE_FRAC * ACTIVE_REF * eco(S.galaxy) * (S.conquest || 1) * Math.max(0, +secs || 0)));
  ...
  // the WALL CLOCK — P1 pays exactly 1◈/week, each deeper planet ×1.3 faster (P5 ≈ 1◈/2.5d, P10 ≈
  // 1.5◈/day). Mines run while you play AND while you're away (no away-cap: gone a week, the P1 mine
  // pays its 1). They stop when Ascension resets the map — re-conquering re-opens them — so the
  // trickle is a property of THIS RUN's empire, a garnish on top of ascension banking, never a
  // replacement (crosscheck-ladder gates campaign mining ≤ 25% of banked cores; measured ~5-15%).
  const MINE_BASE_D = 1 / 7, MINE_K = 1.3;   // K ≤ CORE_B (1.3): the mine curve must never outgrow the bounty curve, or deep parking out-digs ascending (L7 measured 28-37% at K=1.45)
  ...
  function showWelcome(w) {
    ...
    if (!planetMeta(S.galaxy).conquered) txt += "  Conquest waits for you: the bar only advances while you PLAY — spend this haul and take the world.";   // v18.21: says plainly why the bar didn't move
  ...
  function summonWarden() {
    ...
    spawnBoss(true);
    floatTxt(W / 2, H / 2 - 40, "⛏ THE MINE WARDEN GUARDS THE SEAM");
    floatTxt(W / 2, H / 2 - 16, "kill it in 60s — its hoard is the mine price (" + curSym(S.galaxy) + " " + fmt(mineCost(S.galaxy)) + ")");
    syncHUD(); return true;
  }
  function spawnBoss(warden) {
    ...
    const hp = warden ? Math.max(base * 30, dps * 45) : Math.max(base * 20, dps * 30);   // one honest HP pool, no shield — a full rack alone clears it in ~30s of the 60s window; finger-drawing (x2 damage) seals it far faster and a miss still pays salvage. The WARDEN is ×1.5 that — decently strong: a full rack alone barely makes it, active drawing seals it
    ...
    dots.push({ x: W / 2, y: H * 0.44, vx: rnd(-18, 18), vy: rnd(-8, 8), hp, maxHp: hp, value: val, value0: val,   // v18.11: anchor at midfield — the patrol personalities orbit THIS point, and H*0.3 kept bosses hugging the top out of easy reach
    ...
    floatTxt(W / 2, H / 2 - 70, warden ? "⛏ MINE WARDEN ⛏" : "▲ " + bossName(g) + " ▲"); flashAdd(0.55); shakeAdd(9);
  }
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    ...
    if (fromTop) { ex = rnd(60, W - 60); ey = rnd(70, H * 0.6); }   // v18.11: RAIN LANDS ON THE FIELD — spread across the upper field, not piled on the sky line crawling down (owner: "most of the spawned dots keep near the top")
    else if (edge === 0) { ex = an ? clamp(an.x + rnd(-28, 28), 34, W - 34) : rnd(34, W - 34); ey = 44 + j(); }         // top
  ...
  function hitDot(d, dmg, src) {
    ...
    if (d.hp <= 0) {
      ...
      if (d.boss && d.warden) {   // v18.18 the WARDEN's hoard IS the mine price — no wheel, the reward is the build
        ...
        floatTxt(d.x, d.y - d.r - 12, "⛏ MINE WARDEN DOWN");
        floatTxt(d.x, d.y - d.r - 30, "+" + curSym(S.galaxy) + " " + fmt(amt) + " — the ◈ mine is funded");
        showBossReward("⛏ MINE WARDEN", amt, false, false, false);
  ...
  function update(dt) {
    ...
    for (const d of dots) {
      ...
      if (d.boss) {
        ...
        if (d.life >= (d.ttl || 60)) { d.dead = true; burst(d.x, d.y, 30, 200, 2.6); ring(d.x, d.y, d.r, d.r + 130, 0.5);
          ...
          const escNm = d.warden ? "⛏ MINE WARDEN" : bossName(d.bg || S.galaxy);
    ...
    for (const dr of drones) if (dr.wing === 0) {
      ...
      h.x = clamp(h.x, 60, W - 60); h.y = clamp(h.y, 60, H - 140);
    }
  ...
  function drawBoss(d) {
    ...
    ctx.fillStyle = "#fff"; ctx.font = "bold 10px ui-monospace,monospace"; ctx.textAlign = "center"; ctx.fillText((d.warden ? "⛏ MINE WARDEN" : bossName(g)) + "  ·  " + left + "s", d.x, by - 4);
  }
  ...
  let envFor = 0, envW = 0, envH = 0, envAnchors = [], envDust = [], envMotes = [], envDash = 12, envBack = [], envRidge = [];
  function ensureEnv() {
    if (envFor === S.galaxy && envW === W && envH === H && envAnchors.length) return;
    envFor = S.galaxy; envW = W; envH = H; envAnchors = []; envDust = []; envMotes = [];
    ...
    const put = (edge, t) => {
      ...
      // v18.5: ~1 in 5 features is a LANDMARK — same motif drawn bigger and bolder, so each rim reads
      // as a real skyline (big formations with scatter between) instead of an even picket fence
      const lm = er() < 0.19;
      envAnchors.push({ x, y, edge, s: (lm ? 3.1 : 1.7) + er() * 1.2, v: er(), p: er() * TAU, lm });
    };
    const nx = Math.max(6, Math.round(W / 135)), ny = Math.max(4, Math.round(H / 155));   // v18.8 second density notch (v18.5 was /155,/175)
    ...
    // SILHOUETTE BACK ROW (v18.8) — a second, smaller, fainter rank of the same motifs pressed right
    // against the edge BEHIND the foreground features: instant depth, the rim reads as a landscape
    envBack = [];
    for (let i = 0; i < nx - 1; i++) { for (const edge of [0, 1]) { const t = (i + 0.5 + er() * 0.5) / nx, m = 3 + er() * 7;
      envBack.push({ x: 50 + t * (W - 100), y: edge === 0 ? m : H - 62 - m, edge, s: 0.9 + er() * 0.7, v: er(), p: er() * TAU }); } }
    for (let i = 0; i < ny - 1; i++) { for (const edge of [2, 3]) { const t = (i + 0.5 + er() * 0.5) / ny, m = 3 + er() * 7;
      envBack.push({ x: edge === 2 ? m : W - m, y: 70 + t * (H - 230), edge, s: 0.9 + er() * 0.7, v: er(), p: er() * TAU }); } }
    // TERRAIN RIDGE (v18.8) — a continuous jagged ground-line along each edge, threading through the
    // features' feet so the rim is ONE landscape instead of floating stamps
    envRidge = [];
    for (let edge = 0; edge < 4; edge++) {
      const L = edge < 2 ? W : H, pts = [], n2 = Math.max(8, Math.round(L / 46));
      for (let i = 0; i <= n2; i++) { const t = i / n2; pts.push({ t, d: 22 + er() * 15 + Math.sin(t * 8.5 + edge * 1.7) * 3.5 }); }
      envRidge.push(pts);
    }
    // DUST LAYER — tiny seeded ground marks (pebbles, scratches) strewn through the rim band between
    // the features: the terrain gets texture without any new per-frame cost worth naming
    const nd = Math.round((W + H) / 26);   // v18.8: more scatter
    for (let i = 0; i < nd; i++) { const edge = (er() * 4) | 0, t = er(), m = 6 + er() * 30;
      let x, y; if (edge === 0) { x = 40 + t * (W - 80); y = m; } else if (edge === 1) { x = 40 + t * (W - 80); y = H - 62 - m; }
      else if (edge === 2) { x = m; y = 66 + t * (H - 220); } else { x = W - m; y = 66 + t * (H - 220); }
      envDust.push({ x, y, r: 0.7 + er() * 1.6, a: er() * TAU, ln: er() < 0.4 ? 3 + er() * 5 : 0 }); }
    // MOTES — a few slow glints drifting along each edge (ambient life shared by every world)
    for (let i = 0; i < 14; i++) envMotes.push({ edge: i % 4, off: er(), spd: 0.008 + er() * 0.02, ph: er() * TAU, m: 14 + er() * 26 });
    envDash = 9 + Math.round(er() * 10);   // per-planet dash fingerprint on the inner frame line
  }
  ...
  function drawEnv() {
    ...
    // v18.5 LAYERED FRAME: solid world edge + a per-planet dashed survey line inside it + corner
    // brackets — the map boundary reads as built territory markings, not a stray rectangle
    ctx.globalAlpha = 0.10; ctx.strokeRect(1.5, 1.5, W - 3, H - 3);
    ...
    ctx.globalAlpha = 0.26; ctx.lineWidth = 2;
    const BK = 24;
    for (const [cx2, cy2, sx2, sy2] of [[3, 3, 1, 1], [W - 3, 3, -1, 1], [3, H - 3, 1, -1], [W - 3, H - 3, -1, -1]]) {
      ctx.beginPath(); ctx.moveTo(cx2 + sx2 * BK, cy2); ctx.lineTo(cx2, cy2); ctx.lineTo(cx2, cy2 + sy2 * BK); ctx.stroke();
    }
    // v18.8 SILHOUETTE BACK ROW — depth behind the foreground features
    ctx.lineWidth = 1.6;
    ...
    // v18.8 TERRAIN RIDGE — the continuous ground-line each edge's features stand on
    ctx.globalAlpha = 0.15; ctx.lineWidth = 1.5;
    for (let edge = 0; edge < 4; edge++) {
      const pts = envRidge[edge]; if (!pts) continue;
      ctx.beginPath();
      for (let i = 0; i < pts.length; i++) {
        const p2 = pts[i]; let x, y;
        if (edge === 0) { x = 40 + p2.t * (W - 80); y = p2.d; }
        else if (edge === 1) { x = 40 + p2.t * (W - 80); y = H - 62 - p2.d; }
        else if (edge === 2) { x = p2.d; y = 66 + p2.t * (H - 220); }
        else { x = W - p2.d; y = 66 + p2.t * (H - 220); }
        i ? ctx.lineTo(x, y) : ctx.moveTo(x, y);
      }
      ctx.stroke();
    }
    // dust layer — pebbles & scratches through the rim band
    ctx.globalAlpha = 0.14; ctx.lineWidth = 1;
    for (const d of envDust) {
      if (d.ln) { ctx.beginPath(); ctx.moveTo(d.x - Math.cos(d.a) * d.ln, d.y - Math.sin(d.a) * d.ln); ctx.lineTo(d.x + Math.cos(d.a) * d.ln, d.y + Math.sin(d.a) * d.ln); ctx.stroke(); }
      else { ctx.beginPath(); ctx.arc(d.x, d.y, d.r, 0, TAU); ctx.fill(); }
    }
    // themed features — landmarks draw bolder so each rim has a skyline
    ctx.lineWidth = 2;
    for (const a of envAnchors) {
      ctx.save(); ctx.translate(a.x, a.y); ctx.rotate(ENV_ROT[a.edge]);
      ctx.globalAlpha = a.lm ? 0.62 : 0.44;
      fn(ctx, a.s, a.v, a.p, t);
      ctx.restore();
    }
    // drifting edge motes — quiet ambient life along the boundary
    for (const m of envMotes) {
      const u = (m.off + t * m.spd) % 1, tw = 0.05 + 0.09 * (0.5 + 0.5 * Math.sin(t * 1.7 + m.ph));
      let x, y; if (m.edge === 0) { x = 40 + u * (W - 80); y = m.m; } else if (m.edge === 1) { x = 40 + (1 - u) * (W - 80); y = H - 62 - m.m; }
      else if (m.edge === 2) { x = m.m; y = 66 + u * (H - 220); } else { x = W - m.m; y = 66 + (1 - u) * (H - 220); }
      ctx.globalAlpha = tw; ctx.beginPath(); ctx.arc(x, y, 1.5, 0, TAU); ctx.fill();
    }
    // ⛏ ◈ CORE MINE COMPLEX (v18.5, rebuilt v18.19 — owner: "the mine should be a lot bigger visually
    // and moving parts and cool looking"). A built mine only ever shows on a SETTLED calm world (mines
    // exist on conquered planets; the army is stood down), so it's the field's CENTERPIECE: a big
    // working rig planted mid-field — headframe tower with spinning winding wheel + idler, a cable
    // elevator cage that actually rides down the shaft, a nodding pump-jack, an engine house puffing
    // steam with a spinning flywheel and a live ◈-hopper gauge (fill = THIS mine's real mineBuf), a
    // conveyor walking ore chunks onto the spoil heap, a blinking beacon, the seam glowing beneath.
  ...
  function drawMineRig(t) {
    ...
    const mx = W * 0.5, my = H * 0.52, sc = Math.min(2.4, Math.max(1.6, W / 210));
    ctx.save(); ctx.translate(mx, my); ctx.scale(sc, sc);
    ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.strokeStyle = "#fff"; ctx.fillStyle = "#fff";
    // ── the seam glows beneath the whole works — a slow breathing pulse ──
    { const g = ctx.createRadialGradient(0, 8, 2, 0, 8, 44); const pa = 0.10 + 0.05 * Math.sin(t * 1.3);
      g.addColorStop(0, "rgba(255,255,255," + pa.toFixed(3) + ")"); g.addColorStop(1, "rgba(255,255,255,0)");
      ctx.fillStyle = g; ctx.beginPath(); ctx.arc(0, 8, 44, 0, TAU); ctx.fill(); ctx.fillStyle = "#fff"; }
    ctx.globalAlpha = 0.72; ctx.lineWidth = 1.6;
    // ── deck: platform, legs, hazard dashes; seam cracks glowing out from the collar ──
    ctx.beginPath(); ctx.moveTo(-84, 0); ctx.lineTo(84, 0); ctx.stroke();
    for (const lx of [-78, -46, -14, 14, 46, 78]) { ctx.beginPath(); ctx.moveTo(lx, 0); ctx.lineTo(lx, 7); ctx.stroke(); }
    ctx.globalAlpha = 0.3; ctx.setLineDash([5, 6]); ctx.beginPath(); ctx.moveTo(-84, 3.5); ctx.lineTo(84, 3.5); ctx.stroke(); ctx.setLineDash([]);
    for (const [cx2, ex, ey] of [[-14, -30, 13], [13, 26, 15], [-9, -20, 19], [10, 34, 9]]) {
      ctx.globalAlpha = 0.18 + 0.1 * (0.5 + 0.5 * Math.sin(t * 1.3 + cx2)); ctx.beginPath(); ctx.moveTo(cx2, 6); ctx.lineTo((cx2 + ex) / 2 + 3, (6 + ey) / 2); ctx.lineTo(ex, ey); ctx.stroke(); }
    // ── shaft: collar, darkness, depth rungs fading down ──
    ctx.globalAlpha = 1; ctx.fillStyle = "#060606"; ctx.fillRect(-11, 0, 22, 20);
    ctx.globalAlpha = 0.72; ctx.strokeRect(-11, 0, 22, 20);
    for (let i = 1; i < 4; i++) { ctx.globalAlpha = 0.4 - i * 0.09; ctx.beginPath(); ctx.moveTo(-11, i * 5); ctx.lineTo(11, i * 5); ctx.stroke(); }
    ctx.fillStyle = "#fff";
    // ── headframe tower: tall A-frame with X-bracing, service ladder, blinking beacon ──
    ctx.globalAlpha = 0.78; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(-22, 0); ctx.lineTo(-6, -66); ctx.lineTo(6, -66); ctx.lineTo(22, 0); ctx.stroke();
    for (let i = 0; i < 3; i++) { const y1 = -12 - i * 17, y2 = y1 - 17, w1 = 22 * (1 + (y1 / 66) * 0.72), w2 = 22 * (1 + (y2 / 66) * 0.72);
      ctx.lineWidth = 1.2; ctx.beginPath(); ctx.moveTo(-w1, y1); ctx.lineTo(w2, y2); ctx.moveTo(w1, y1); ctx.lineTo(-w2, y2); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(-w1, y1); ctx.lineTo(w1, y1); ctx.stroke(); }
    { ctx.globalAlpha = 0.4; ctx.lineWidth = 1; const lxx = 24;   // ladder up the right leg
      ctx.beginPath(); ctx.moveTo(lxx, -2); ctx.lineTo(13, -46); ctx.stroke();
      for (let i = 0; i < 6; i++) { const u = i / 6, x1 = lxx + (13 - lxx) * u, y1 = -2 + (-44) * u; ctx.beginPath(); ctx.moveTo(x1 - 2.4, y1 + 1) ; ctx.lineTo(x1 + 2.4, y1 - 1); ctx.stroke(); } }
    { const blink = Math.sin(t * 2.7) > 0.55 ? 0.9 : 0.18;   // beacon at the tip
      ctx.globalAlpha = blink; ctx.beginPath(); ctx.arc(0, -70, 1.8, 0, TAU); ctx.fill();
      const rr = (t * 0.7) % 1; ctx.globalAlpha = 0.25 * (1 - rr) * (blink > 0.5 ? 1 : 0.4); ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(0, -70, 3 + rr * 12, 0, TAU); ctx.stroke(); }
    // ── winding wheel + idler, both live; cable down to the ELEVATOR CAGE riding the shaft ──
    const wr = 11, rot = t * 1.6;
    ctx.globalAlpha = 0.85; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(0, -74 + wr, wr, 0, TAU); ctx.stroke();
    ctx.lineWidth = 1.2; for (let i = 0; i < 4; i++) { const a2 = rot + i * TAU / 4; ctx.beginPath(); ctx.moveTo(Math.cos(a2) * wr, -74 + wr + Math.sin(a2) * wr); ctx.lineTo(-Math.cos(a2) * wr, -74 + wr - Math.sin(a2) * wr); ctx.stroke(); }
    ctx.beginPath(); ctx.arc(0, -74 + wr, 2, 0, TAU); ctx.fill();
    { const ir = 5, ix = -16, iy = -52, irot = -t * 2.6;   // idler wheel on the back leg
      ctx.lineWidth = 1.4; ctx.beginPath(); ctx.arc(ix, iy, ir, 0, TAU); ctx.stroke();
      for (let i = 0; i < 2; i++) { const a2 = irot + i * Math.PI / 2; ctx.beginPath(); ctx.moveTo(ix + Math.cos(a2) * ir, iy + Math.sin(a2) * ir); ctx.lineTo(ix - Math.cos(a2) * ir, iy - Math.sin(a2) * ir); ctx.stroke(); } }
    { const ph = (t * 0.16) % 1, depth = ph < 0.5 ? ph * 2 : (1 - ph) * 2;   // cage cycle: down, then back up
      const cy = -56 + depth * 66;                                            // -56 (at the wheel) → +10 (into the collar)
      ctx.globalAlpha = 0.6; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, -74 + wr * 2); ctx.lineTo(0, Math.min(cy, 18)); ctx.stroke();   // the cable itself
      if (cy < 14) { ctx.globalAlpha = 0.85; ctx.lineWidth = 1.4;             // the cage (visible until it sinks past the collar)
        ctx.strokeRect(-4.5, cy, 9, 9); ctx.beginPath(); ctx.moveTo(-4.5, cy + 4.5); ctx.lineTo(4.5, cy + 4.5); ctx.moveTo(0, cy); ctx.lineTo(0, cy + 9); ctx.stroke(); } }
    // ── pump-jack (nodding donkey) on the left: walking beam, horse head, counterweight, sucker rod ──
    { const px = -52, ang = Math.sin(t * 1.9) * 0.2;
      ctx.globalAlpha = 0.78; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(px - 7, 0); ctx.lineTo(px, -22); ctx.lineTo(px + 7, 0); ctx.stroke();          // pivot post
      ctx.save(); ctx.translate(px, -22); ctx.rotate(ang);
      ctx.beginPath(); ctx.moveTo(-20, 0); ctx.lineTo(16, 0); ctx.stroke();                                       // walking beam
      ctx.beginPath(); ctx.moveTo(16, 0); ctx.lineTo(20, -3); ctx.lineTo(20, 4); ctx.lineTo(16, 3); ctx.closePath(); ctx.stroke();   // horse head
      ctx.beginPath(); ctx.arc(-20, 0, 4.5, 0, TAU); ctx.stroke();                                                // counterweight
      ctx.restore();
      const hx = px + Math.cos(ang) * 18, hy = -22 + Math.sin(ang) * 18;                                          // sucker rod down from the head
      ctx.lineWidth = 1.2; ctx.beginPath(); ctx.moveTo(hx, hy + 3); ctx.lineTo(hx, -1); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(hx - 3, 0); ctx.lineTo(hx + 3, 0); ctx.stroke(); }
    // ── engine house on the far left: shack, spinning flywheel, steam puffs, live ◈-hopper gauge ──
    { const sx = -84; ctx.globalAlpha = 0.72; ctx.lineWidth = 1.6;
      ctx.strokeRect(sx, -18, 20, 18);                                                                            // shack
      ctx.beginPath(); ctx.moveTo(sx - 2, -18); ctx.lineTo(sx + 10, -26); ctx.lineTo(sx + 22, -18); ctx.stroke(); // roof
      ctx.beginPath(); ctx.moveTo(sx + 16, -26); ctx.lineTo(sx + 16, -32); ctx.stroke();                          // chimney
      for (let i = 0; i < 3; i++) { const kk = (t * 0.4 + i / 3) % 1;                                             // steam puffs
        ctx.globalAlpha = 0.26 * (1 - kk); ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(sx + 16 + Math.sin(kk * 5 + i) * 3, -34 - kk * 22, 1.5 + kk * 4.5, 0, TAU); ctx.stroke(); }
      { const fr = 5.5, frot = t * 3.2; ctx.globalAlpha = 0.7; ctx.lineWidth = 1.3;                              // flywheel on the shack wall
        ctx.beginPath(); ctx.arc(sx + 6, -9, fr, 0, TAU); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(sx + 6 + Math.cos(frot) * fr, -9 + Math.sin(frot) * fr); ctx.lineTo(sx + 6 - Math.cos(frot) * fr, -9 - Math.sin(frot) * fr); ctx.stroke(); }
      { const gx = sx + 24, gh = 14;                                                                              // ◈-hopper gauge — fill is THIS mine's real progress to the next core
        ctx.globalAlpha = 0.6; ctx.lineWidth = 1; ctx.strokeRect(gx, -16, 6, gh);
        ctx.globalAlpha = 0.75; ctx.fillRect(gx + 1, -16 + (1 - buf) * (gh - 2) + 1, 4, buf * (gh - 2));
        const gp = 0.5 + 0.3 * Math.sin(t * 2.2); ctx.globalAlpha = gp; ctx.font = "5px ui-monospace,monospace"; ctx.textAlign = "center"; ctx.fillText("◈", gx + 3, -19); } }
    // ── conveyor to the spoil heap on the right: belt on trestles, ore chunks walking up, heap growing ──
    { const x0 = 13, y0 = -3, x1 = 62, y1 = -14;
      ctx.globalAlpha = 0.66; ctx.lineWidth = 1.6; ctx.beginPath(); ctx.moveTo(x0, y0); ctx.lineTo(x1, y1); ctx.stroke();
      ctx.lineWidth = 1; for (const u of [0.3, 0.65, 0.95]) { const tx = x0 + (x1 - x0) * u, ty = y0 + (y1 - y0) * u; ctx.beginPath(); ctx.moveTo(tx, ty); ctx.lineTo(tx, 0); ctx.stroke(); }
      for (let i = 0; i < 4; i++) { const u = (t * 0.34 + i / 4) % 1, tx = x0 + (x1 - x0) * u, ty = y0 + (y1 - y0) * u - 1.6;
        ctx.globalAlpha = 0.8; ctx.beginPath(); ctx.arc(tx, ty, 1.4, 0, TAU); ctx.fill(); }
      ctx.globalAlpha = 0.55; ctx.lineWidth = 1.4;                                                                // the heap it feeds
      ctx.beginPath(); ctx.moveTo(58, 0); ctx.quadraticCurveTo(68, -10, 78, 0); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(63, 0); ctx.quadraticCurveTo(68, -5.5, 73, 0); ctx.stroke(); }
    // ── ◈ glints rising out of the shaft — the seam paying out, bigger and brighter than v18.5 ──
    for (let i = 0; i < 3; i++) { const kk = (t * 0.3 + i / 3) % 1;
      const gy = 6 - kk * 46, gs = 2 + 2.2 * (1 - kk), gxx = Math.sin(kk * 6 + i * 2.1) * 4;
      ctx.globalAlpha = 0.7 * (1 - kk); ctx.lineWidth = 1.3;
      ctx.beginPath(); ctx.moveTo(gxx, gy - gs); ctx.lineTo(gxx + gs, gy); ctx.lineTo(gxx, gy + gs); ctx.lineTo(gxx - gs, gy); ctx.closePath(); ctx.stroke(); }
    ctx.restore();
  }
  ...
  function render() {
    ...
    if (state === "play" && !S.travel && planetMeta(S.galaxy).conquered && !wardenOn) {
      ...
      ctx.fillText(v.mine ? "⛏ core mine digging " + fmtMineRate(mineRate(S.galaxy)) + " — progress in the panel below" : "⛏ mine site ready — ▲ summon the WARDEN (win the mine price) or buy it, panel below", SW / 2, cy + 43);
  ...
  function renderSettlePanel() {
    ...
    if (built) {
      ...
      html += '<button id="sp-warden" class="sp-build">▲ SUMMON THE MINE WARDEN — win ' + curSym(g) + ' ' + fmt(cost) + '</button>'
           + '<div class="sp-note">a 60s boss duel over the ◈ seam — its hoard pays the mine in full. Or buy it outright:</div>'
