...
(() => {
  ...
  let conqCardT = 0, conqCardG = 0, conqCardCores = 0, conqCardSpoils = 0;
  ...
  function update(dt) {
    ...
    { const pm = planetMeta(S.galaxy); if (!pm.conquered && curEarned >= conquerTarget(S.galaxy)) { pm.conquered = true; pm.bgRate = Math.max(pm.bgRate || 0, baseTarget(S.galaxy) / (IDLE_PAYBACK_H * 3600)); pm.spoils = Math.round(SETTLE_SPOILS_FRAC * conquerTarget(S.galaxy)); const cg = coreVal(S.galaxy); recompute();
        ...
        if (totConq >= TOTAL_PLANETS && !S.victory) { S.victory = true; flashAdd(0.9); shakeAdd(9); ring(W / 2, H / 2, 14, Math.max(W, H), 0.8); burst(W / 2, H / 2, 60, 320, 3.2); Audio_victory(); Cards.push("victory", showVictory); save(); } } }   // v16.5: a PERSISTENT victory screen — the old floating text faded in ~2s and the summit of ~55 active hours was missable by an alt-tab
