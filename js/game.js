...
(() => {
  ...
  function syncHUD() {
    ...
    { const p = pendingCores(), ab = $("ascend-n"); if (ab) ab.textContent = "+" + fmt(p); const abtn = $("btn-ascend"); if (abtn) { abtn.classList.toggle("has", p >= 5);
      abtn.classList.toggle("stage", wallAhead() && p >= 1 && state === "play" && !conq0Hide());   // the ALWAYS-VISIBLE offer (v16.0): what ascending right now banks; glows once it's a real haul
    ...
    { const setVis = (el, on) => { if (el && (el.style.display === "none") === on) el.style.display = on ? "" : "none"; };
      ...
      setVis($("btn-ascend"), revealed("ascend"));
