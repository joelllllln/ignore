...
(() => {
  ...
  const CONQUEST_BEAT = 1.2;
  let conqCardT = 0, conqCardG = 0, conqCardCores = 0, conqCardSpoils = 0, conqCardVic = false;
  ...
  function showConquest(g, cores, spoils) {
    ...
    const rows = [
      ...
      ["⛏ MINE SITE", "seam found — " + fmtMineRate(mineRate(g))],
    ...
    const nt = $("cq-note");
    if (nt) nt.textContent = "The world is yours and at peace. Duel " + wardenOf(g).n
      + " for the ◈ seam or buy the mine outright — both wait in the settlement panel. Nothing starts until you say so.";
  ...
  function wardenReset() {
    ...
    wardenOn = false; wardenClear = false; conqCardT = 0;   // v18.34: a pending conquest report dies with the world it belongs to
  ...
  function update(dt) {
    ...
    if (conqCardT > 0) { conqCardT -= dt; if (conqCardT <= 0) { const g = conqCardG, c = conqCardCores, s = conqCardSpoils;
    ...
    { const pm = planetMeta(S.galaxy); if (!pm.conquered && curEarned >= conquerTarget(S.galaxy)) { pm.conquered = true; pm.bgRate = Math.max(pm.bgRate || 0, baseTarget(S.galaxy) / (IDLE_PAYBACK_H * 3600)); pm.spoils = Math.round(SETTLE_SPOILS_FRAC * conquerTarget(S.galaxy)); const cg = coreVal(S.galaxy); recompute();
        ...
        conqCardG = S.galaxy; conqCardCores = cg; conqCardSpoils = pm.spoils; conqCardT = CONQUEST_BEAT;
        flashAdd(0.5); shakeAdd(4); vibe([40, 30, 90]); Audio_conquer(); syncHUD();
        ...
        if (totConq >= TOTAL_PLANETS && !S.victory) { S.victory = true; flashAdd(0.9); shakeAdd(9); ring(W / 2, H / 2, 14, Math.max(W, H), 0.8); burst(W / 2, H / 2, 60, 320, 3.2); Audio_victory(); conqCardVic = true; save(); } } }   // v18.34: victory rides the SAME beat as the report and queues AFTER it — pushing it here fired it at t=0 and buried what the final conquest actually paid   // v16.5: a PERSISTENT victory screen — the old floating text faded in ~2s and the summit of ~55 active hours was missable by an alt-tab
