...
(() => {
  ...
  // v18.32 (owner: "having the menu up and down changes the spawning locations and the size of map
  // when it shouldn't"): the play band used to be measured from the dock LIVE, so every time the dock
  // changed height — Minimise, a tab switch, the settlement panel swapping in — the world was refitted
  // underneath the player: the field resized and the spawn ring moved with it. The band is now pinned
  // to bandRef, the TALLEST the dock has ever been at this screen size. Minimising or flipping tabs
  // never touches it; it is only ever re-measured on a real resize or rotate.
  let bandRef = 0;
  function measureBand() {
    const d = $("dock"); if (!d || d.classList.contains("min")) return false;   // a minimised dock says nothing about the dock's real size
    const t = d.getBoundingClientRect().top;
    if (!(t > 10)) return false;
    if (!bandRef || t < bandRef - 0.5) { bandRef = t; return true; }            // only ever the tallest dock we have seen
    return false;
  }
  ...
  function fitWorld() {
    ...
    WORLD.vw = Math.max(40, SW / 2 / z); WORLD.vh = Math.max(40, bandH / 2 / z);   // the screen rectangle at NORMAL zoom, in world units
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    ...
    if (fromTop) {   // Dot Rain falls INTO the bowl — spread over the disc, not piled on one line
      ...
      // fauna comes in over the EDGE OF THE VIEW, evenly all the way around it, landing just inside
      // so you always see it arrive. It walks the LIVE view, so this holds at any zoom.
      const vhw = liveHW(), vhh = liveHH(), bp = perim(Math.random(), vhw, vhh);
  ...
  function bandWatch() {
    ...
    if (measureBand()) fitWorld();   // only fires if the dock has grown past anything seen before
  }
  function drawEnv() {
    bandWatch();
    ...
    // on a settled world.
    if (mineBuilt(S.galaxy) && !wardenOn) drawMineRig(performance.now() / 1000);
  }
  ...
  function setScreen(s) {
    ...
    if (s === "play") { syncAutoBtn(); measureBand(); fitWorld(); }   // the dock/HUD only have real geometry once play is on screen — fit the map to the band they leave
  ...
  function resize() {
    ...
    bandRef = 0; measureBand(); fitWorld();      // a real resize/rotate is the ONLY thing that re-measures the band
  ...
  // v18.32: the dock changing height is NOT a reason to refit the world — that is exactly what made
  // Minimise resize the map and move the spawn ring. The observer now only feeds measureBand(), which
  // ignores a minimised dock and only ever accepts a dock TALLER than anything seen at this screen
  // size; the band is otherwise pinned until a real resize or rotate.
  if (typeof ResizeObserver !== "undefined") {
    let rfT = 0;
    const ro = new ResizeObserver(() => { const n = performance.now(); if (n - rfT < 120) return; rfT = n; if (measureBand()) fitWorld(); });
