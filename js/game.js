...
(() => {
  ...
  // v18.26 THE WORLD IS A DISC (owner: "make the maps like a big circle you can zoom in and out of
  // instead of rectangle"). You are standing on a PLANET, seen from above — so the playable ground is
  // a circle inscribed in the W×H world box, not a rectangle. Everything rides it: dots spawn around
  // its rim, the army and collectors are held inside it, the boss patrols within it, and the rim art
  // wraps the circumference. Outside the circle is orbital VOID, which is what fills the corners once
  // you pull the camera all the way back (see drawVoid).
  // The disc is fitted to the VISIBLE PLAY BAND — the strip of screen between the top HUD and the
  // dock — measured from the real DOM at every resize. That is what makes this work on every phone
  // shape: tall 20:9, short 16:9, a tablet, or landscape all get a correctly framed world with the
  // same margin of void around it, instead of a circle sized to an invisible world box.
  // v18.27 (owner: "the circle should be big enough that you don't see the edges normally — only when
  // you zoom out do you see the circle, the map is so small"): the limb is pushed BEYOND the corners
  // of the play band at normal zoom, so wherever you look you are looking at ground. Pull the camera
  // back and the whole planet — rim terrain, outer wastes and all — comes into frame.
  //   DISC.r        = the whole planet (its limb is off-screen at normal zoom)
  //   DISC.pr       = the PLAY radius: the disc actually on screen at normal zoom. Anything that must
  //                   stay where the player can see it (boss patrol, Dot Rain, the mine) uses this.
  //   DISC.vw / .vh = half-extents of the VISIBLE BAND in world units at normal zoom — the actual
  //                   rectangle of screen you are looking at. viewEdge(a) walks it, and that is where
  //                   fauna comes in (v18.28, owner: "they're not spawning on the edges").
  let DISC = { cx: 0, cy: 0, r: 200, pr: 160, vw: 200, vh: 200 };
  let VIEW_CY = 0, VIEW_T = 0, VIEW_B = 0;   // the play band on screen: the camera looks at its centre, not at SH/2
  let camMin = 0;     // furthest the camera pulls back: the whole planet framed inside the play band
  const discCX = () => DISC.cx, discCY = () => DISC.cy, discR = () => DISC.r, playR = () => DISC.pr;
  function fitDisc() {
    const dockEl = $("dock"), topEl = $("top");
    // a hidden dock measures 0 — treat anything unmeasurable as "not on screen yet" and fall back to
    // sensible fractions, then re-fit for real the moment the play screen actually opens
    const dr = dockEl && dockEl.getBoundingClientRect(), tr = topEl && topEl.getBoundingClientRect();
    const dockTop = (dr && dr.top > 10) ? dr.top : SH * 0.62;
    const topBot = (tr && tr.bottom > 4) ? tr.bottom : SH * 0.09;
    const bandTop = Math.max(8, Math.min(topBot + 6, SH * 0.4));
    const bandBot = Math.max(bandTop + 40, Math.min(dockTop - 6, SH));   // a 120px minimum here used to OVERRIDE the real dock position on a landscape phone, so the disc was fitted to a band that did not exist and drew straight through the dock
    const bandH = bandBot - bandTop, bandCY = (bandTop + bandBot) / 2;
    const z = camFit || 1;
    VIEW_CY = bandCY; VIEW_T = bandTop; VIEW_B = bandBot;         // the camera frames the band, not the screen
    DISC.r = Math.max(60, Math.hypot(SW, bandH) / 2 * 1.24 / z);  // limb well past every corner of the band (v18.28: 1.06 -> 1.24, owner "make the map slightly bigger" — a bigger world with more of it left to find when you pull back)
    DISC.pr = Math.max(40, Math.min(SW, bandH) / 2 * 0.98 / z);   // the ground you can actually see right now
    DISC.vw = SW / 2 / z; DISC.vh = bandH / 2 / z;                // the screen rectangle, in world units
    DISC.cx = W / 2; DISC.cy = H / 2;                             // the planet IS the world centre, so the army stands on the middle of it
    camMin = Math.min(SW, bandH) * 0.46 / DISC.r;                 // zoom-out stop: whole planet + a margin of outer waste
  }
  // pull a point back inside the disc (m = margin from the rim); returns whether it had to move
  function discClamp(o, m) {
    const cx = discCX(), cy = discCY(), rr = Math.max(10, discR() - (m || 0));
    const dx = o.x - cx, dy = o.y - cy, q = dx * dx + dy * dy;
    if (q <= rr * rr) return false;
    const dl = Math.sqrt(q) || 1; o.x = cx + dx / dl * rr; o.y = cy + dy / dl * rr; return true;
  }
  // pull a point back into the part of the disc that is ON SCREEN at normal zoom
  function viewClamp(o, m) {
    const cx = discCX(), cy = discCY(), rr = Math.max(10, playR() - (m || 0));
    const dx = o.x - cx, dy = o.y - cy, q = dx * dx + dy * dy;
    if (q <= rr * rr) return false;
    const dl = Math.sqrt(q) || 1; o.x = cx + dx / dl * rr; o.y = cy + dy / dl * rr; return true;
  }
  // how far the EDGE OF THE SCREEN is from the middle of the world along a bearing, in world units.
  // Measured at normal zoom so it never moves when the player pinches — spawning on this line means
  // fauna always walks in over the edge of what you are looking at, on every side, whatever the shape
  // of the phone. A circle can't do that: on the diagonals a circle sits well inside the corners of
  // the screen, which is exactly where dots were seen popping into open ground.
  function viewEdge(a) {
    const ca = Math.abs(Math.cos(a)), sa = Math.abs(Math.sin(a));
    return Math.min(ca > 1e-4 ? DISC.vw / ca : 1e9, sa > 1e-4 ? DISC.vh / sa : 1e9);
  }
  const discPt = (a, rad) => ({ x: discCX() + Math.cos(a) * rad, y: discCY() + Math.sin(a) * rad });
  ...
  // v18.27: the zoom-out stop is no longer a constant — it is camMin, computed in fitDisc() as exactly
  ...
  const UPS = [
    { id: "capacity",  tab: "eco", name: "Capacity",   base: 40, mul: 1.42,   /* v18.0 fixed global curves */ desc: () => curSym(S.galaxy) + " " + fmt(derived.capacity) },
    ...
    { id: "value",     tab: "eco", name: "Value",      base: 130, mul: 1.46, desc: () => "×" + derived.valueMul.toFixed(2) + " /dot" },
    { id: "spawnRate", tab: "eco", name: "Spawn Rate", base: 60, mul: 1.48, desc: () => { const raw = derived.spawnPerSec || 0, om = spawnOver(raw); return raw.toFixed(1) + " /s" + (om > 1.02 ? "  ·  ~" + spawnVis(raw).toFixed(0) + "/s on screen + ×" + om.toFixed(1) + " tougher" : ""); } },
    { id: "luck",      tab: "eco", name: "Luck",       base: 140, mul: 1.34, desc: () => (derived.luck * 100).toFixed(1) + "% special" },
  ];
  ...
  const upCost = u => Math.ceil(u.base * Math.pow(u.mul, ecoLv(u.id)) * pk().cost * (UP_DISC[u.id] || 1) * TEST_MUL());
  ...
  const SETTLE_ON_SITE = 20;         // spoils-window on-site multiplier
  ...
  const settleIncomeRate = () => { const v = S.vault && S.vault[S.galaxy]; if (!(v && v.conquered)) return 0;
    const mul = settleSpoils(S.galaxy) > 0 ? SETTLE_ON_SITE : (1 + EMPIRE_RAMP * conqueredCount());   // steady phase = exactly what this world would pay you from ANYWHERE (see empireIdleRate) — parking buys nothing
    return (v.bgRate || 0) * mul * pk().empire; };
  ...
  function settleOffline(e) {
    ...
    const bg = (v.bgRate || 0) * pk().empire; if (!(bg > 0)) return 0;
    const pool = settleSpoils(S.galaxy), hi = bg * SETTLE_ON_SITE, tHi = Math.min(e, pool / hi);
    v.spoils = Math.max(0, pool - hi * tHi);
    return hi * tHi + bg * (1 + EMPIRE_RAMP * conqueredCount()) * Math.max(0, e - tHi);
  }
  ...
  function fresh() {
    ...
    return { v18: 1, cash: Math.floor(eco(1) * startMul(1)), galaxy: 1, lv, classNodes, units: [newUnit("turret")], collectors: [{ type: "drone" }], totalRun: 0, peakGalaxy: 1, runSec: 0, vault: {}, travel: null, conquest: 1, victory: false, auto: defaultAuto() };
  ...
  let wardenOn = false, wardenClear = false, wardenAutoT = 0;
  ...
  function buildMine(g) {
    ...
    floatTxt(W / 2, H / 2 - 20, "⛏ ◈ CORE MINE BUILT — " + galName(g)); floatTxt(W / 2, H / 2 + 6, "digging " + fmtMineRate(mineRate(g)));
    // v18.18: buying the mine mid-duel retires its warden — the seam is claimed, the fight is moot
    // (no double payout: the boss despawns without a kill, so its hoard never drops)
    if (g === S.galaxy && wardenOn) { for (const d of dots) if (d.warden) d.dead = true; wardenOn = false; wardenClear = true; floatTxt(W / 2, H / 2 + 32, "the warden abandons the claimed seam"); }
    Audio_buy(); flashAdd(0.25); vibe([30, 20, 60]); envFor = 0;   // rim rebuilds so the headframe appears immediately
  ...
  let corePopT = null;
  function showCorePop(n, why) {
    const el = $("core-pop"); if (!el || !(n > 0)) return;
    $("cp-n").textContent = "◈ +" + fmt(n) + " PRESTIGE CORE" + (n > 1 ? "S" : "");
    $("cp-src").textContent = why;
    $("cp-tot").textContent = "banked: " + fmt((META.asc && META.asc.cores) | 0) + " ◈ — spend them in Ascension";
  ...
  function mineAccrue(sec, quiet) {
    ...
    if (total < 1) return 0;
    META.asc.cores = (META.asc.cores | 0) + total; META.asc.lifetime = (META.asc.lifetime | 0) + total;
    if (!quiet) showCorePop(total, "dug by your ⛏ ◈ core mines");
    return total;
  }
  ...
  function ascend() {
    ...
    showCorePop(pend, "banked by ASCENSION — run " + META.asc.runs + " complete");
  ...
  function recompute() {
    ...
    derived.capacity = ecoCost() * 220 * Math.pow(1.60, L.capacity) * (derived.incomeMul || 1);   // cash ceiling rides the FRONTIER economy (v17.4 — visiting an old world must not crash the ceiling below your wallet and freeze banking) × the SAME income multiplier as the payouts (engine incl.) so it never lags your income
    derived.valueMul = (1 + 0.13 * ecoLv("value")) * pk().value;          // v18.14: +13% per GLOBAL level at ×1.37 cost (was +10%/×1.30) — CHUNKIER: each buy is a felt jump and the ladder needs ~25% fewer clicks (owner: "always bottlenecked by value and spawn") — long-run cost-per-doubling ~12% cheaper. × Ascension value perk.
    ...
    const rawSpawn = (0.9 + 1.15 * ecoLv("spawnRate")) * pk().spawn;   // v18.14: +1.15/s per level at ×1.39 cost (was +0.9/×1.32) — chunkier buys, fewer clicks, same continuous global ladder
      ...
      if (d) {
        ...
        if (d.S) { S = Object.assign(fresh(), d.S); S.lv = Object.assign(fresh().lv, d.S.lv || {});
          ...
          if (!d.S.v18) {   // v18.0 migration — keyed off the SAVE's own flag, never the merged state: fresh() carries v18:1 (see fresh), so Object.assign would mask a v17 save's missing flag and skip its fold. Fold the re-baseline era into ONE global ladder — your EFFECTIVE
            ...
            delete S.ecoSnap; delete S.cpsS; delete S.launchQuote; S.v18 = 1; } }   // the adaptive-era trackers ride out with the snapshot
  ...
  function showWelcome(w) {
    ...
    if (!planetMeta(S.galaxy).conquered) txt += "  Your empire also pushed the conquer bar at " + Math.round(IDLE_FRAC * 100) + "% of active pace — playing is ~" + Math.round(1 / IDLE_FRAC) + "× faster, so spend this haul and take the world.";   // v18.22: the bar DOES move while you're away, at the idle allowance — say by how much
  ...
  const Wheel = (() => {
    ...
    function apply(sg) {
      ...
      if (sg.kind === "core") { META.asc.cores = (META.asc.cores | 0) + 1; META.asc.lifetime = (META.asc.lifetime | 0) + 1; save(); showCorePop(1, "rare drop — BOSS BOUNTY WHEEL"); return "◈ +1 CORE — banked to Ascension"; }
  ...
  function wardenReset() {
    ...
    wardenOn = false; wardenAutoT = 0; wardenClear = false;
  }
  function summonWarden() {
    const pm = planetMeta(S.galaxy);
    if (!pm.conquered || pm.mine || wardenOn || S.travel || state !== "play") return false;
    wardenOn = true; bossAcc = 0;
    spawnBoss(true);
    { const Wd = wardenOf(S.galaxy);
      floatTxt(W / 2, H / 2 - 40, "⛏ " + Wd.n + " GUARDS THE SEAM");
      floatTxt(W / 2, H / 2 - 16, Wd.tell);
      floatTxt(W / 2, H / 2 + 8, "kill it in 60s — its hoard founds the mine (" + curSym(S.galaxy) + " " + fmt(mineCost(S.galaxy)) + ")"); }
    syncHUD(); return true;
  }
  function spawnBoss(warden) {
    ...
    const d = { x: discCX(), y: discCY(), vx: rnd(-18, 18), vy: rnd(-8, 8), hp, maxHp: hp, value: val, value0: val,   // v18.11: anchor at midfield — the patrol personalities orbit THIS point, and H*0.3 kept bosses hugging the top out of easy reach
      ...
      mstyle: styles[Math.floor(mr(0) * styles.length)], mt: 0, mphase: mr(1) * TAU, mfx: 0.5 + mr(2) * 0.9, mfy: 0.45 + mr(3) * 0.9, mdir: mr(4) < 0.5 ? -1 : 1, mrad: 95 + mr(5) * 75, mtimer: 0, mtx: discCX(), mty: discCY(), mdash: false,
  ...
  function bossMove(d, dt) {
    ...
    const pr = playR() * 0.80, cx = discCX(), cy = discCY(), L = cx - pr, R = cx + pr, T = cy - pr, B = cy + pr;   // v18.27: the PLAY radius — a boss that patrolled the whole planet would spend half the duel off-screen
  ...
  function spawnDot(special, fromTop) {   // fromTop (v18.8): Dot Rain falls from the SKY edge, not all four
    ...
    if (fromTop) {   // Dot Rain falls INTO the bowl — spread over the disc, not piled on one line
      const ra = Math.random() * TAU, rr = playR() * Math.sqrt(Math.random()) * 0.94;
      ex = discCX() + Math.cos(ra) * rr; ey = discCY() + Math.sin(ra) * rr;
    } else {
      const an = envAnchors.length ? envAnchors[Math.floor(Math.random() * envAnchors.length)] : null;
      const a = an ? an.a + rnd(-0.055, 0.055) : Math.random() * TAU;
      const p2 = discPt(a, Math.min(discR() * 0.96, viewEdge(a) * rnd(0.97, 1.03)));   // v18.28: ON the edge of the screen, all the way around — they cross the rim of the view instead of appearing in front of you
      ex = p2.x; ey = p2.y;
    }
    const ia = Math.atan2(discCY() - ey, discCX() - ex) + rnd(-0.55, 0.55), isp = mv * rnd(0.55, 1.0);
  ...
  function hitDot(d, dmg, src) {
    ...
    if (d.hp <= 0) {
      ...
      if (d.boss && d.warden) {   // v18.18 the WARDEN's hoard IS the mine price — no wheel, the reward is the build
        ...
        floatTxt(d.x, d.y - d.r - 12, "⛏ " + (d.wname || "MINE WARDEN") + " DOWN");
        floatTxt(d.x, d.y - d.r - 30, "its hoard (" + curSym(S.galaxy) + " " + fmt(amt) + ") founds the ◈ mine");
        showBossReward("⛏ " + (d.wname || "MINE WARDEN"), amt, false, false, false);
        ...
        buildMine(S.galaxy);                    // spends exactly the hoard — claims the seam so no second duel can be summoned
        recompute(); syncHUD();
        return;
      }
  ...
  function update(dt) {
    ...
    if (wardenAutoT > 0) { wardenAutoT -= dt; if (wardenAutoT <= 0) summonWarden(); }   // the post-conquest challenge announces itself moments after the fireworks
    if (!settled) {
    ...
    for (const d of dots) {
      ...
      if (d.boss) {
        ...
        if (d.warden && d.calib) {
          ...
          if (d.calib === 1 && d.life >= 1.0) { d.calib = 2; d.cmark = dealt; }
          else if (d.calib === 2 && d.life >= 5.0) {
            d.calib = 0;
            const obs = (dealt - (d.cmark || 0)) / 4.0;   // FOUR seconds: early racks fire in lumpy mortar salvos, and a shorter sample caught one salvo or none
            if (obs > 0) {
              const want = obs * WARDEN_TTK * (d.wdur || 1);
        ...
        if (d.life >= (d.ttl || 60)) { d.dead = true; burst(d.x, d.y, 30, 200, 2.6); ring(d.x, d.y, d.r, d.r + 130, 0.5);
          ...
          if (d.warden) { wardenOn = false; wardenClear = true; floatTxt(W / 2, H / 2 + 8, "the seam holds — summon the warden again from the settlement panel"); }   // v18.18: a lost duel re-settles the world; the panel offers the rematch
      ...
      if (blackholeT > 0) { const dx = W / 2 - d.x, dy = H / 2 - d.y, dl = Math.hypot(dx, dy) || 1; d.x += dx / dl * 220 * dt; d.y += dy / dl * 220 * dt; hitDot(d, bhDmg * dt, "blackhole"); }
        ...
        // v18.26: the ground is a disc — bounce off the RIM, not off four invisible walls
        { const cx = discCX(), cy = discCY(), rr = discR() - 26, dx = d.x - cx, dy = d.y - cy, q = dx * dx + dy * dy;
          if (q > rr * rr) { const dl = Math.sqrt(q) || 1, nx = dx / dl, ny = dy / dl;
            const vn = d.vx * nx + d.vy * ny; if (vn > 0) { d.vx -= 1.5 * vn * nx; d.vy -= 1.5 * vn * ny; }
            d.x = cx + nx * rr; d.y = cy + ny * rr; } }
      }
    }
    ...
    // wardenAutoT, and the stale flag would sweep the freshly-summoned warden right off the field)
    ...
    for (const dr of drones) {
      ...
      dr.x += dr.vx * dt; dr.y += dr.vy * dt; discClamp(dr, 12);   // v18.26: collectors are held on the ground disc
    ...
    { const sRate = settleIncomeRate(); const bgSum = empireIdleRate() + sRate;
    ...
    { const pm = planetMeta(S.galaxy); if (!pm.conquered && curEarned >= conquerTarget(S.galaxy)) { pm.conquered = true; pm.bgRate = Math.max(pm.bgRate || 0, baseTarget(S.galaxy) / (IDLE_PAYBACK_H * 3600)); pm.spoils = Math.round(SETTLE_SPOILS_FRAC * conquerTarget(S.galaxy)); const cg = coreVal(S.galaxy); recompute(); floatTxt(W / 2, H / 2 - 40, "✦ PLANET CONQUERED"); floatTxt(W / 2, H / 2 - 16, "+" + cg + " ◈ PENDING — Ascension charging (" + pendingCores() + " ready)"); floatTxt(W / 2, H / 2 + 8, "⚑ VICTORY SPOILS — settlement pays ×" + SETTLE_ON_SITE + " while " + curSym(S.galaxy) + " " + fmt(pm.spoils) + " lasts"); floatTxt(W / 2, H / 2 + 32, "⛏ THE MINE WARDEN IS COMING — hold position, then launch");   // v18.21: the order is spoken aloud — duel first, travel after flashAdd(0.5); shakeAdd(4); vibe([40, 30, 90]); Audio_conquer(); syncHUD();
        ...
        for (const d of dots) if (Math.random() < 0.3) burst(d.x, d.y, 3, 70, 0.9);
        dots.length = 0; shells.length = 0;
        wardenAutoT = 2.5;   // v18.18: moments after the fireworks, the ⛏ MINE WARDEN challenge announces itself — beat it and the mine is funded, no AFK saving
  ...
  let envFor = 0, envW = 0, envH = 0, envR = 0, envAnchors = [], envDust = [], envMotes = [], envBack = [], envRidge = [], envOuter = [], envGround = [];
  // v18.26 PER-PLANET BIOMES, rebuilt in v18.27 (owner: "all the planets terrain looks fucked and
  // mistakes"). A world's landscape is composed from four things that all differ planet to planet:
  //   mix    — 2-3 DIFFERENT formations woven together, so a horizon reads as an ecosystem
  //   layout — how they are distributed around the belt: evenly, in clumps with empty stretches,
  //            packed into one arc, two opposing bands, or sparse-and-monumental
  //   ridge  — the terrain silhouette: jagged, rolling dunes, spikes, low flats, broken plateaus
  //   gnd    — the SURFACE the fight happens on: cratered, cracked, pooled, combed into dunes,
  //            fractured into plates, drifted with ash, or torn by rifts
  // plus SIGNATURE formations: one to three vast bespoke structures per world that anchor the skyline.
  // Everything is sized as a FRACTION of the planet (U below), never in raw pixels — v18.26 sized the
  // art in pixels against a disc a third of today's size, which is why the formations came out as
  // wall-sized shapes piled through each other and through the horizon.
  const BIOMES = [null,
    { mix: [1, 16],      layout: "clumps", dens: 1.00, ridge: "jag",    sig: 1, gnd: "craters" },   // P1  Vesta    — cratered moonscape pocked with burrows
    { mix: [2, 14],      layout: "arcs",   dens: 1.10, ridge: "shard",  sig: 2, gnd: "cracks"  },   // P2  Ember    — cinder cones over ash pyres
    { mix: [3, 2, 15],   layout: "twin",   dens: 1.20, ridge: "broken", sig: 1, gnd: "cracks"  },   // P3  Cinder   — slag fissures, vents, void wells
    { mix: [4, 7],       layout: "even",   dens: 1.00, ridge: "dune",   sig: 1, gnd: "pools"   },   // P4  Hearth   — tidal pools in reed flats
    { mix: [5, 13],      layout: "clumps", dens: 1.25, ridge: "jag",    sig: 2, gnd: "pools"   },   // P5  Azure    — forest stands with wisp markers
    { mix: [6, 5],       layout: "arcs",   dens: 1.15, ridge: "dune",   sig: 1, gnd: "dunes"   },   // P6  Verdant  — pylon line through woodland
    { mix: [7, 12],      layout: "even",   dens: 0.90, ridge: "flat",   sig: 2, gnd: "plates"  },   // P7  Cobalt   — reed plain under monoliths
    { mix: [8, 9],       layout: "sparse", dens: 0.75, ridge: "dune",   sig: 1, gnd: "dunes"   },   // P8  Mistral  — mirage arches, storm rods
    { mix: [9, 8, 13],   layout: "twin",   dens: 1.05, ridge: "shard",  sig: 2, gnd: "dunes"   },   // P9  Halcyon  — rod forest and arches
    { mix: [10, 9],      layout: "clumps", dens: 1.20, ridge: "jag",    sig: 1, gnd: "cracks"  },   // P10 Tempest  — rift clusters, lightning rods
    { mix: [11, 10],     layout: "arcs",   dens: 1.00, ridge: "broken", sig: 2, gnd: "rifts"   },   // P11 Umbral   — shadow rifts, shards
    { mix: [12, 11],     layout: "sparse", dens: 0.80, ridge: "shard",  sig: 1, gnd: "plates"  },   // P12 Frost    — monoliths on an ice shelf
    { mix: [13, 12, 1],  layout: "even",   dens: 1.10, ridge: "flat",   sig: 2, gnd: "plates"  },   // P13 Onyx     — marker fields, broken stone
    { mix: [14, 15],     layout: "clumps", dens: 1.15, ridge: "broken", sig: 1, gnd: "ash"     },   // P14 Wraith   — pyres around void wells
    { mix: [15, 14, 3],  layout: "twin",   dens: 1.05, ridge: "jag",    sig: 2, gnd: "ash"     },   // P15 Pyreling — wells, pyres, fissures
    { mix: [16, 17],     layout: "arcs",   dens: 1.20, ridge: "dune",   sig: 1, gnd: "craters" },   // P16 Abyssal  — burrow fields, null gates
    { mix: [17, 16, 6],  layout: "even",   dens: 1.10, ridge: "shard",  sig: 2, gnd: "craters" },   // P17 Devourer — gates, maws, pylons
    { mix: [18, 15, 11], layout: "sparse", dens: 0.90, ridge: "broken", sig: 3, gnd: "rifts"   },   // P18 Null     — monumental, almost empty
  ];
  const biomeOf = g => BIOMES[clamp(g | 0, 1, 18)] || BIOMES[1];
  // A properly mixing seeded PRNG (mulberry32). What came before was a LINEAR hash of its own
  // counter — consecutive draws differed by a constant step, so every angle sequence taken from it
  // was an arithmetic progression and everything scattered with it landed in even rings and lines.
  // That is what "all the planets terrain looks fucked" actually was: the scatter was never random.
  function rng(seed) {
    let a = seed >>> 0;
  ...
  function ensureEnv() {
    if (envFor === S.galaxy && envW === W && envH === H && envR === DISC.r && envAnchors.length) return;
    envFor = S.galaxy; envW = W; envH = H; envR = DISC.r;
    envAnchors = []; envBack = []; envOuter = []; envGround = []; envDust = []; envMotes = [];
    const B = biomeOf(S.galaxy), R0 = discR();
    const U = R0 / 420;   // ONE unit of scale: everything below is a fraction of the planet, so the art is the same composition on a 320px phone and a desktop
    const er = rng(Math.imul(S.galaxy * 92821 + 7, 268435459));
    const pick = () => B.mix[Math.floor(er() * B.mix.length)];
    const halfW = s => 18 * s;             // how wide a formation of scale s actually draws (craters and arches spread well past their footing)
    // ── WHERE THE FORMATIONS STAND ─────────────────────────────────────────────────────────────
    // counts scale with the SIZE of the world, so a bigger planet is a bigger landscape rather than
    // the same handful of features stretched thinner (capped so a desktop-sized disc stays cheap)
    const n = clamp(Math.round(24 * B.dens * U), 9, 60);
    const angles = [];
    if (B.layout === "even") { for (let i = 0; i < n; i++) angles.push((i + 0.3 + er() * 0.4) / n * TAU); }
    else if (B.layout === "clumps") { const cl = 3 + Math.floor(er() * 3), m = Math.ceil(n / cl);
      for (let c = 0; c < cl; c++) { const c0 = (c + er() * 0.35) / cl * TAU, w = 0.5 + er() * 0.45;
        for (let i = 0; i < m; i++) angles.push(c0 + (er() - 0.5) * w); } }
    else if (B.layout === "arcs") { const a0 = er() * TAU, span = 1.7 + er() * 0.7;
      for (let i = 0; i < n; i++) angles.push(er() < 0.72 ? a0 + er() * span : er() * TAU); }
    else if (B.layout === "twin") { const a0 = er() * TAU;
      for (let i = 0; i < n; i++) angles.push((i % 2 ? a0 : a0 + Math.PI) + (er() - 0.5) * 1.6); }
    else { const m = Math.max(5, Math.round(n * 0.45)); for (let i = 0; i < m; i++) angles.push((i + er() * 0.6) / m * TAU); }
    const big = B.layout === "sparse";
    const cand = angles.map(a => {
      const lm = er() < 0.20;
      // a normal formation stands ~7% of the planet's radius, a landmark ~10%, a signature ~16% —
      // features you notice on the horizon, not walls you stand next to
      const s = ((lm ? (big ? 3.2 : 2.6) : (big ? 2.3 : 1.65)) + er() * 0.7) * U;
      return { a: (a % TAU + TAU) % TAU, s, v: er(), p: er() * TAU, lm, f: pick() };
    }).sort((x, y) => x.a - y.a);
    // NO PILE-UPS: walk the belt in order and drop anything that would grow through its neighbour.
    // v18.26 had no such pass, so every clumped world came out as a heap of overlapping shapes.
    const belt = R0 * 0.88;
    // SIGNATURE FORMATIONS first — one to three colossal structures, spaced around the world, each
    // holding its own ground so nothing else grows through it
    const sigs = [];
    for (let i = 0; i < B.sig; i++)
      sigs.push({ a: (((i / B.sig) * TAU + er() * 0.7) % TAU), rad: R0 * (0.93 - er() * 0.10),
        s: (4.1 + er() * 1.1) * U, v: er(), p: er() * TAU, lm: true, sig: true, f: B.mix[0] });
    const clearOf = c => sigs.every(g => {
      const dd = Math.abs(((c.a - g.a + Math.PI * 3) % TAU) - Math.PI);
      return dd > (halfW(g.s) + halfW(c.s)) * 1.15 / belt;
    });
    const kept = [];
    for (const c of cand) {
      const prev = kept[kept.length - 1];
      if (prev && c.a - prev.a < (halfW(prev.s) + halfW(c.s)) * 1.15 / belt) continue;
      if (!clearOf(c)) continue;
      kept.push(c);
    }
    if (kept.length > 2) {   // and close the ring: the last one must clear the first one too
      const f = kept[0], l = kept[kept.length - 1];
      if (TAU - l.a + f.a < (halfW(f.s) + halfW(l.s)) * 1.15 / belt) kept.pop();
    }
    // the belt has DEPTH: formations sit anywhere from the innermost terrain (just past the edge of
    // view, which is where the fauna crawls out) to the limb itself
    for (const c of kept) { c.rad = R0 * (0.985 - er() * 0.26); envAnchors.push(c); }
    for (const g of sigs) envAnchors.push(g);
    // BACK ROW — a fainter, smaller rank pressed against the limb, for depth
    for (let i = 0; i < Math.round(n * 0.9); i++)
      envBack.push({ a: er() * TAU, rad: R0 * (0.995 - er() * 0.05), s: (0.85 + er() * 0.6) * U,
        v: er(), p: er() * TAU, f: pick() });
    // OUTER WASTES — the same landscape continuing past the limb, thinning as it recedes (v18.27,
    // owner: "the edges should be the environment/terrain of the planet not random shit")
    for (let i = 0, on = clamp(Math.round(46 * U * U), 20, 130); i < on; i++) {
      const q = er();
      envOuter.push({ a: er() * TAU, rad: R0 * (1.035 + q * 0.60), s: (1.45 - q * 0.65) * U,
        v: er(), p: er() * TAU, al: 0.04 + 0.26 * (1 - q) * (1 - q), f: pick() });
    }
    // THE SURFACE — this world's own ground texture, laid faintly under the fight
    // surface detail is SMALL — a few percent of the planet. v18.27's first pass drew it at the same
    // scale as the horizon formations, which is where the "ring of overlapping circles" came from.
    { const gn = clamp(Math.round(92 * B.dens * U * U), 24, 240);
      for (let i = 0; i < gn; i++) { const q = er();
        envGround.push({ a: er() * TAU, rad: R0 * (0.16 + Math.sqrt(er()) * 0.80),
          s: (0.34 + q * q * 0.72) * U, v: er(), p: er() * TAU }); } }
    // THE RIDGE — one closed ground-line running the whole circumference. Shaped by coherent waves
    // and spikes, never per-vertex noise: v18.26's random-per-step ridge milled itself into a gear.
    envRidge = [];
    { const steps = 240, ph = [er() * TAU, er() * TAU, er() * TAU], spikes = [];
      if (B.ridge === "shard") for (let i = 0; i < 13; i++) spikes.push({ a: er() * TAU, w: 0.07 + er() * 0.10, h: 0.030 + er() * 0.055 });
      const sm = x => x * x * (3 - 2 * x);
      for (let i = 0; i <= steps; i++) {
        const a = i / steps * TAU; let d;
        if (B.ridge === "jag") d = 0.042 + 0.016 * Math.sin(a * 9 + ph[0]) + 0.005 * Math.sin(a * 19 + ph[1]);
        else if (B.ridge === "dune") d = 0.054 + 0.026 * Math.sin(a * 3 + ph[0]) + 0.010 * Math.sin(a * 7 + ph[1]);
        else if (B.ridge === "shard") { d = 0.024;
          for (const sp of spikes) { const dd = Math.abs(((a - sp.a + Math.PI * 3) % TAU) - Math.PI); if (dd < sp.w) d += sp.h * sm(1 - dd / sp.w); } }
        else if (B.ridge === "flat") d = 0.028 + 0.006 * Math.sin(a * 5 + ph[0]) + 0.004 * Math.sin(a * 11 + ph[1]);
        else d = 0.024 + 0.046 * sm(clamp(0.5 + 0.9 * Math.sin(a * 4.2 + ph[0]), 0, 1));   // broken: plateaus split by gulfs
        envRidge.push({ a, d: d * R0 });
      } }
    // dust & drifting motes live in the terrain belt
    for (let i = 0; i < Math.round(TAU * R0 / 34); i++)
      envDust.push({ a: er() * TAU, rad: R0 * (0.995 - er() * 0.12), r: (0.6 + er() * 1.5) * Math.max(1, U * 0.9), ln: er() < 0.35 ? (2 + er() * 5) * Math.max(1, U) : 0, an: er() * TAU });
    for (let i = 0; i < 16; i++) envMotes.push({ off: er(), spd: 0.006 + er() * 0.014, m: R0 * (0.02 + er() * 0.07), ph: er() * TAU });
  }
  const ENV_DRAW = [null,
  ...
  // A motif grows along its local -y. At rim angle a the inward direction is (-cos a, -sin a), and
  // rotating by (a - PI/2) maps local -y onto exactly that — so every formation leans into the bowl.
  const rimRot = a => a - Math.PI / 2;
  // THE SURFACE (v18.27) — what the ground itself is made of on this world. Drawn very faint, under
  // everything, so the field you fight on reads as a PLACE without ever competing with the fight.
  const GROUND_DRAW = {
    craters: (c, s) => { const r = 26 * s;
      c.beginPath(); c.arc(0, 0, r, 0, TAU); c.stroke();
      c.globalAlpha *= 0.6; c.beginPath(); c.arc(r * 0.2, r * 0.16, r * 0.55, 0, TAU); c.stroke(); },
    cracks: (c, s, v) => { const L = 52 * s;
      c.beginPath(); c.moveTo(-L, 0);
      for (let i = 1; i <= 5; i++) c.lineTo(-L + L * 0.4 * i, (i % 2 ? 1 : -1) * 6 * s * (0.5 + v));
      c.stroke();
      c.beginPath(); c.moveTo(-L * 0.2, 0); c.lineTo(-L * 0.05, -20 * s); c.stroke(); },
    pools: (c, s, v) => { const rx = 32 * s, ry = 16 * s, rot = v * 1.4;
      c.save(); c.globalAlpha *= 1.5; c.fillStyle = "#000"; c.beginPath(); c.ellipse(0, 0, rx, ry, rot, 0, TAU); c.fill(); c.restore();
      c.beginPath(); c.ellipse(0, 0, rx, ry, rot, 0, TAU); c.stroke();
      c.globalAlpha *= 0.5; c.beginPath(); c.ellipse(0, 0, rx * 0.6, ry * 0.6, rot, 0, TAU); c.stroke(); },
    dunes: (c, s) => { for (let i = 0; i < 3; i++) { c.beginPath(); c.arc(0, 34 * s * i, 44 * s, -2.45, -0.75); c.stroke(); } },
    plates: (c, s, v) => { const r = 32 * s;
      c.beginPath();
      for (let i = 0; i < 6; i++) { const a = i / 6 * TAU + v * TAU, rr = r * (0.7 + ((i * 7 + Math.floor(v * 13)) % 3) * 0.15);
        const x = Math.cos(a) * rr, y = Math.sin(a) * rr; i ? c.lineTo(x, y) : c.moveTo(x, y); }
      c.closePath(); c.stroke(); },
    ash: (c, s, v) => { for (let i = 0; i < 7; i++) { const a = v * TAU + i * 0.9, rr = (7 + i * 5) * s;
        c.beginPath(); c.arc(Math.cos(a) * rr, Math.sin(a) * rr * 0.5, 1.3 * s, 0, TAU); c.fill(); } },
    rifts: (c, s) => { const L = 42 * s, w = 8 * s;
      const lens = () => { c.beginPath(); c.moveTo(-L, 0); c.quadraticCurveTo(0, -w, L, 0); c.quadraticCurveTo(0, w, -L, 0); };
      c.save(); c.globalAlpha *= 1.4; c.fillStyle = "#000"; lens(); c.fill(); c.restore();
      lens(); c.stroke(); },
  };
  // the closed terrain silhouette, at `mul` times its relief (1 = the ridge itself)
  function ridgePath(cx, cy, R0, mul, rot) {
    ctx.beginPath();
    for (let i = 0; i < envRidge.length; i++) { const q = envRidge[i], rr = R0 - q.d * mul, a = q.a + (rot || 0);
      const x = cx + Math.cos(a) * rr, y = cy + Math.sin(a) * rr; i ? ctx.lineTo(x, y) : ctx.moveTo(x, y); }
    ctx.closePath();
  }
  // OUTER WASTES (v18.27, owner: "the edges should be the environment/terrain of the planet not random
  // shit") — beyond the limb the SAME landscape keeps going: this world's ridge echoes outward as
  // receding contours, its own formations stand out there in the half-light, and the whole thing dims
  // into the planet's shadow. No orbit rings, no bearing ticks, no starfield — it is all ground.
  // Drawn with an even-odd fill so it can never paint over the playfield.
  function drawOuter(t) {
    const cx = discCX(), cy = discCY(), R0 = discR(), M = Math.max(W, H) * 1.8;
    ctx.save();
    ctx.beginPath(); ctx.rect(cx - M, cy - M, M * 2, M * 2); ctx.arc(cx, cy, R0, 0, TAU, true);
    ctx.fillStyle = "#000"; ctx.fill("evenodd");
    ctx.clip("evenodd");                       // everything below is confined to the wastes
    { const g = ctx.createRadialGradient(cx, cy, R0, cx, cy, R0 * 1.66);   // land beyond the limb, falling away into the planet's shadow
      g.addColorStop(0, "rgba(255,255,255,0.014)"); g.addColorStop(0.42, "rgba(255,255,255,0.006)"); g.addColorStop(1, "rgba(255,255,255,0)");
      ctx.fillStyle = g; ctx.fillRect(cx - M, cy - M, M * 2, M * 2); }
    ctx.strokeStyle = "#fff"; ctx.fillStyle = "#fff";
    ctx.lineWidth = 1.4;
    for (let ring = 0; ring < 4; ring++) {     // the ridge, receding outward as fainter contours of the same terrain
      const k = 1 + (ring + 1) * 0.118;
      ctx.globalAlpha = 0.14 * Math.pow(0.6, ring);
      ridgePath(cx, cy, R0 * k, (1 - ring * 0.15) * k, ring * 0.10); ctx.stroke();
    }
    ctx.lineWidth = 1.6;
    const fnOf = i => ENV_DRAW[clamp(i | 0, 1, 18)] || ENV_DRAW[1];
    for (const o of envOuter) {                // the same formations, standing out in the wastes
      const p2 = discPt(o.a, o.rad);
      ctx.save(); ctx.translate(p2.x, p2.y); ctx.rotate(rimRot(o.a));
      ctx.globalAlpha = o.al; fnOf(o.f)(ctx, o.s, o.v, o.p, t); ctx.restore();
    }
    ctx.restore();
  }
  ...
  let bandChk = 0, bandLast = -1;
  function bandWatch() {
    const n = performance.now(); if (n - bandChk < 240) return; bandChk = n;
    const dk = $("dock"); if (!dk) return;
    const top = dk.getBoundingClientRect().top;
    if (Math.abs(top - bandLast) > 2) { bandLast = top; fitDisc(); envFor = 0; }
  }
  function drawEnv() {
    bandWatch();
    ensureEnv();
    const B = biomeOf(S.galaxy), t = performance.now() / 1000;
    const cx = discCX(), cy = discCY(), R0 = discR();
    const fnOf = i => ENV_DRAW[clamp(i | 0, 1, 18)] || ENV_DRAW[1];
    drawOuter(t);
    ctx.save();
    // THE GROUND ITSELF — the backdrop starfield lives in screen space and would otherwise shine up
    // through the planet's surface. Lay solid ground inside the limb, with a soft vignette so the
    // horizon curves away from you instead of ending at a hard line.
    ctx.beginPath(); ctx.arc(cx, cy, R0, 0, TAU); ctx.fillStyle = "#050505"; ctx.fill();
    { const g = ctx.createRadialGradient(cx, cy, R0 * 0.25, cx, cy, R0);
      g.addColorStop(0, "rgba(255,255,255,0.022)"); g.addColorStop(0.88, "rgba(255,255,255,0.012)"); g.addColorStop(1, "rgba(0,0,0,0.18)");
      ctx.fillStyle = g; ctx.beginPath(); ctx.arc(cx, cy, R0, 0, TAU); ctx.fill(); }
    ctx.beginPath(); ctx.arc(cx, cy, R0, 0, TAU); ctx.clip();   // v18.27: NOTHING the landscape draws may leak past the horizon
    ctx.strokeStyle = "#fff"; ctx.fillStyle = "#fff";
    // THE SURFACE — this world's own ground, under everything
    { const gd = GROUND_DRAW[B.gnd] || GROUND_DRAW.plates;
      ctx.lineWidth = 1.2;
      for (const q of envGround) { const p2 = discPt(q.a, q.rad);
        ctx.save(); ctx.translate(p2.x, p2.y); ctx.rotate(rimRot(q.a));
        ctx.globalAlpha = 0.13; gd(ctx, q.s, q.v, q.p, t); ctx.restore(); } }
    // THE RIDGE — the terrain silhouette, with a softer echo further in for depth
    ctx.globalAlpha = 0.30; ctx.lineWidth = 1.7; ridgePath(cx, cy, R0, 1); ctx.stroke();
    ctx.globalAlpha = 0.11; ctx.lineWidth = 1.2; ridgePath(cx, cy, R0, 2.1, 0.06); ctx.stroke();
    // dust & scratches through the terrain belt
    ctx.globalAlpha = 0.14; ctx.lineWidth = 1;
    for (const d of envDust) {
      const x = cx + Math.cos(d.a) * d.rad, y = cy + Math.sin(d.a) * d.rad;
      if (d.ln) { ctx.beginPath(); ctx.moveTo(x - Math.cos(d.an) * d.ln, y - Math.sin(d.an) * d.ln); ctx.lineTo(x + Math.cos(d.an) * d.ln, y + Math.sin(d.an) * d.ln); ctx.stroke(); }
      else { ctx.beginPath(); ctx.arc(x, y, d.r, 0, TAU); ctx.fill(); }
    }
    // BACK ROW — the fainter rank behind, for depth
    ctx.lineWidth = 1.5;
    for (const a of envBack) {
      const p2 = discPt(a.a, a.rad);
      ctx.save(); ctx.translate(p2.x, p2.y); ctx.rotate(rimRot(a.a));
      ctx.globalAlpha = 0.15; fnOf(a.f)(ctx, a.s, a.v, a.p, t); ctx.restore();
    }
    // THE LANDSCAPE — this world's mix of formations, with its signatures standing over them
    for (const a of envAnchors) {
      const p2 = discPt(a.a, a.rad);
      ctx.save(); ctx.translate(p2.x, p2.y); ctx.rotate(rimRot(a.a));
      if (a.sig) {   // a signature sits on its own shelf of ground rather than a ring of survey furniture
        ctx.globalAlpha = 0.13; ctx.lineWidth = 1.2;
        const sw = 20 * a.s;
        ctx.beginPath(); ctx.moveTo(-sw, 2); ctx.lineTo(-sw * 0.62, -4 * a.s); ctx.lineTo(sw * 0.62, -4 * a.s); ctx.lineTo(sw, 2); ctx.stroke();
      }
      ctx.globalAlpha = a.sig ? 0.72 : a.lm ? 0.56 : 0.40;
      ctx.lineWidth = a.sig ? 2.4 : 1.9;
      fnOf(a.f)(ctx, a.s, a.v, a.p, t);
      ctx.restore();
    }
    // drifting motes riding the belt
    for (const m of envMotes) {
      const a = (m.off + t * m.spd) * TAU, rad = R0 - m.m;
      ctx.globalAlpha = 0.05 + 0.09 * (0.5 + 0.5 * Math.sin(t * 1.7 + m.ph));
      ctx.beginPath(); ctx.arc(cx + Math.cos(a) * rad, cy + Math.sin(a) * rad, 1.5, 0, TAU); ctx.fill();
    }
    // THE HORIZON — the world's own boundary, drawn last so it reads as the edge of the ground
    ctx.globalAlpha = 0.20; ctx.lineWidth = 2.4;
    ctx.beginPath(); ctx.arc(cx, cy, R0 - 1.2, 0, TAU); ctx.stroke();
    if (mineBuilt(S.galaxy) && !wardenOn) drawMineRig(t);
    ctx.restore();
  }
  function drawMineRig(t) {
    ...
    const R = Math.max(70, playR() * 0.80), sc = R / 100;   // v18.27: sized to the ground you can see
    ...
    // ── the seam, glowing up through the ground ──
    { const g = ctx.createRadialGradient(0, 0, 2, 0, 0, 62), pa = 0.11 + 0.05 * Math.sin(t * 1.3);
    ...
    ctx.globalAlpha = 0.18; ctx.lineWidth = 1; ctx.setLineDash([9, 7]);
    ctx.beginPath(); ctx.arc(0, 0, 100, 0, TAU); ctx.stroke(); ctx.setLineDash([]);
    ring(93, 0.10, 1);
    for (let i = 0; i < 8; i++) { const a = i * TAU / 8 + 0.2, lx = Math.cos(a) * 100, ly = Math.sin(a) * 100;
      const lit = 0.3 + 0.35 * (0.5 + 0.5 * Math.sin(t * 1.6 + i));
    ...
    { const sa = t * 0.55;   // slow radar-style survey sweep across the pit
      const sg = ctx.createRadialGradient(0, 0, 10, 0, 0, 96);
      sg.addColorStop(0, "rgba(255,255,255,0.055)"); sg.addColorStop(1, "rgba(255,255,255,0)");
    ...
    for (let i = 0; i < SPOKES; i++) {
      ...
      for (const off of [-1.8, 1.8]) { ctx.beginPath();
        ctx.moveTo(ca * 30 + nx * off, sa2 * 30 + ny * off); ctx.lineTo(ca * 88 + nx * off, sa2 * 88 + ny * off); ctx.stroke(); }
      ctx.globalAlpha = 0.16;
      for (let s = 0; s < 10; s++) { const rr = 30 + s * 6.4;
        ctx.beginPath(); ctx.moveTo(ca * rr + nx * 3, sa2 * rr + ny * 3); ctx.lineTo(ca * rr - nx * 3, sa2 * rr - ny * 3); ctx.stroke(); }
      // a cart shuttling the line: out loaded, back empty (loaded carts read solid)
    ...
    const shed = (a, rr, w, h, hatch) => { const x = Math.cos(a) * rr, y = Math.sin(a) * rr;
      ...
      ctx.globalAlpha = 0.62; ctx.lineWidth = 1.5; ctx.strokeRect(-w / 2, -h / 2, w, h);
      ...
      if (hatch) { ctx.globalAlpha = 0.5; ctx.beginPath(); ctx.moveTo(-w / 2, 0); ctx.lineTo(w / 2, 0); ctx.stroke(); }
    ...
    const silo = (a, rr, r2) => { const x = Math.cos(a) * rr, y = Math.sin(a) * rr;
      ctx.globalAlpha = 0.6; ctx.lineWidth = 1.4; ctx.beginPath(); ctx.arc(x, y, r2, 0, TAU); ctx.stroke();
      ctx.globalAlpha = 0.28; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(x - r2, y); ctx.lineTo(x + r2, y); ctx.moveTo(x, y - r2); ctx.lineTo(x, y + r2); ctx.stroke();
      return { x, y }; };
    ...
    { ctx.globalAlpha = 0.9; ctx.lineWidth = 2.4;
      ...
      ctx.globalAlpha = 0.16; ctx.lineWidth = 2.4; ctx.beginPath(); ctx.arc(hop.x, hop.y, 12, 0, TAU); ctx.stroke();
      ctx.globalAlpha = 0.5 + 0.3 * Math.sin(t * 2.2); ctx.font = "7px ui-monospace,monospace"; ctx.textAlign = "center";
      ctx.fillText("◈", hop.x, hop.y + 2.6); }
    // steam venting from the engine house, drifting off the pad
    for (let i = 0; i < 3; i++) { const kk = (t * 0.4 + i / 3) % 1;
      ctx.globalAlpha = 0.22 * (1 - kk); ctx.lineWidth = 1;
    ...
    { const a = 0.35 + TAU / SPOKES * 3, ca = Math.cos(a), sa2 = Math.sin(a), nx = -sa2, ny = ca;
      ctx.globalAlpha = 0.5; ctx.lineWidth = 1.2;
      for (const off of [-3, 3]) { ctx.beginPath(); ctx.moveTo(ca * 34 + nx * off, sa2 * 34 + ny * off); ctx.lineTo(ca * 84 + nx * off, sa2 * 84 + ny * off); ctx.stroke(); }
      for (let i = 0; i < 5; i++) { const u = (t * 0.3 + i / 5) % 1, rr = 34 + u * 50;
        ctx.globalAlpha = 0.75; ctx.beginPath(); ctx.arc(ca * rr, sa2 * rr, 1.5, 0, TAU); ctx.fill(); }
      const hx = ca * 95, hy = sa2 * 95;   // the heap it feeds — concentric contours, tallest inside
      for (let k = 0; k < 3; k++) { ctx.globalAlpha = 0.34 - k * 0.07; ctx.lineWidth = 1.2;
        ctx.beginPath(); ctx.arc(hx, hy, 13 - k * 4, 0, TAU); ctx.stroke(); } }

    // ── the shaft: apron, collar, the dark mouth, and the winding gear turning above it ──
    ring(30, 0.3, 1.4);                                   // concrete apron edge
    ctx.globalAlpha = 0.5; ctx.lineWidth = 1;
    for (let i = 0; i < 12; i++) { const a = i * TAU / 12; ctx.beginPath();
      ctx.moveTo(Math.cos(a) * 22, Math.sin(a) * 22); ctx.lineTo(Math.cos(a) * 30, Math.sin(a) * 30); ctx.stroke(); }   // apron slabs
    ctx.globalAlpha = 1; ctx.fillStyle = "#070707"; ctx.beginPath(); ctx.arc(0, 0, 18, 0, TAU); ctx.fill(); ctx.fillStyle = "#fff";
    ring(18, 0.8, 2); ring(13, 0.3, 1); ring(8.5, 0.18, 1);          // collar + shaft walls receding into the dark
    { const rot = t * 1.5;                                           // winding gear seen from above
      ctx.globalAlpha = 0.75; ctx.lineWidth = 1.6;
      for (let i = 0; i < 4; i++) { const a = rot + i * TAU / 4;
        ctx.beginPath(); ctx.moveTo(Math.cos(a) * 5, Math.sin(a) * 5); ctx.lineTo(Math.cos(a) * 21, Math.sin(a) * 21); ctx.stroke(); }
      ring(21, 0.45, 1.2);
      // the cage: a square in the mouth that shrinks as it descends, then rides back up
      const ph = (t * 0.16) % 1, depth = ph < 0.5 ? ph * 2 : (1 - ph) * 2, cs = 7 * (1 - depth * 0.72);
    ...
    // ── ◈ glints rising out of the mouth — the seam paying out ──
    for (let i = 0; i < 3; i++) { const kk = (t * 0.3 + i / 3) % 1, a = i * 2.1 + t * 0.4;
      const gr = kk * 26, gx = Math.cos(a) * gr, gy = Math.sin(a) * gr, gs = 2.2 + 2.4 * (1 - kk);
      ctx.globalAlpha = 0.75 * (1 - kk); ctx.lineWidth = 1.3;
  ...
  function render() {
    ...
    if (state === "play" && !S.travel && planetMeta(S.galaxy).conquered && !wardenOn) {
      const v = S.vault[S.galaxy] || {}, cy = (VIEW_T || SH * 0.22) + 26, breathe = 0.85 + 0.15 * Math.sin(performance.now() / 900);
      ...
      const body = [
        ...
        v.mine ? "⛏ core mine digging " + fmtMineRate(mineRate(S.galaxy)) + " — progress in the panel below" : "⛏ mine site ready — ▲ summon " + wardenOf(S.galaxy).n + " (win the mine) or buy it, panel below",
  ...
  function renderSettlePanel() {
    ...
    const cost = mineCost(g), can = S.cash >= cost;
    let secs = -1;
    if (built && rate > 0) secs = Math.max(0, (1 - buf) * 86400 / rate);
    const key = [g, built ? 1 : 0, can ? 1 : 0, Math.floor(secs / 30), Math.floor(buf * 200)].join("|");
    ...
    if (built) {
      ...
      // route to the mine — its hoard pays the exact build price. Buying outright stays available.
      html += '<button id="sp-warden" class="sp-build">▲ SUMMON ' + wardenOf(g).n + ' — win the ◈ mine</button>'
           + '<div class="sp-note">' + wardenOf(g).tell + ' — a 60s duel over the ◈ seam; beat it and its hoard (' + curSym(g) + ' ' + fmt(cost) + ') founds the mine on the spot. Or buy it outright:</div>'
           + '<button id="sp-build" class="sp-build"' + (can ? "" : " disabled") + '>⛏ BUILD ◈ CORE MINE — ' + curSym(g) + ' ' + fmt(cost) + '</button>'
           + '<div class="sp-note">digs ' + fmtMineRate(rate) + ' for the rest of this run — even while you’re away</div>';
    }
    ...
    html += '<div class="sp-inc">' + (spl > 0
         ...
         + (empireIdleRate() > 0 ? ' · empire tribute +' + fmt(empireIdleRate()) + '/s' : '') + '</div>';
    sp.innerHTML = html;
    const bb = $("sp-build"); if (bb) bb.onclick = () => { if (buildMine(S.galaxy)) { settleKey = ""; renderSettlePanel(); } };
    const wb = $("sp-warden"); if (wb) wb.onclick = () => { summonWarden(); };   // dock swaps to combat on the next syncHUD
  }
  function syncHUD() {
  ...
  function showGalaxyInfo(g) {
    ...
    const prog = current ? (planetMeta(g).conquered ? "✓ conquered  ·  settlement pays +" + curSym(S.galaxy) + " " + fmt(settleIncomeRate()) + "/s while you\u2019re parked here" : Math.floor(clamp(curEarned / conquerTarget(g), 0, 1) * 100) + "% to conquer  ·  unlocks Travel + idle income")
    ...
    const stats = "<div class='gi-unlock'>" + curSym(g) + " <b>" + curName(g) + "</b>" +
      (pv && pv.conquered ? " · <b>+" + fmt(pv.bgRate || 0) + "/s</b> idle" + (pv.mine ? " · <b>⛏ ◈ " + fmtMineRate(mineRate(g)) + "</b>" : "") : "") +
      (pv && pv.conquered && !pv.mine
        ? "<br>" + (S.cash >= mineCost(g)
            ? "<button id='gi-mine'>⛏ BUILD ◈ CORE MINE · " + curSym(S.galaxy) + " " + fmt(mineCost(g)) + "  (" + fmtMineRate(mineRate(g)) + ")</button>"
            : "<button id='gi-mine' class='gi-lock' disabled>⛏ ◈ mine · need " + curSym(S.galaxy) + " " + fmt(mineCost(g)) + "</button>")   // v18.9 ONE WALLET: every price reads in the symbol your wallet shows right now — same money everywhere
        : "") +
      "<br>" + prog + "</div>";
    ...
    const mb = $("gi-mine"); if (mb) mb.onclick = () => { if (buildMine(g)) showGalaxyInfo(g); };   // v18.5: pay to found the ◈ farm, card refreshes to show it digging
  ...
  function renderAscend() {
    ...
    // v18.7 (owner ask: ascending must ALWAYS visibly beat sitting on your farms): the head-to-head, live
    { const mr2 = mineRateTotal();
      if (!locked && mr2 > 0 && pend > 0) coach += '<p class="asc-coach">\u26cf Parked, your mines dig ' + fmtMineRate(mr2) + '. Ascending banks <b>+' + pend + '\u25c8 right now</b> \u2014 \u2248 ' + Math.max(1, Math.round(pend / mr2)) + ' days of parked mining in one hop \u2014 and the Engine makes the next climb faster still.</p>'; }
    // v17.16 (owner call: "when and how exactly you get the ascension currency"): the core math,
    // itemized live \u2014 every source of the pending bank, planet by planet, summing to the button.
    let srcChips = "";
    for (let g = 1; g <= TOTAL_PLANETS; g++) { const v = S.vault && S.vault[g]; if (v && v.conquered) srcChips += '<span class="asc-chip">P' + g + ' \u2713 +' + coreVal(g) + '</span>'; }
    { const mr = mineRateTotal(); if (mr > 0) srcChips += '<span class="asc-chip">\u25c8 mines ' + fmtMineRate(mr) + '</span>'; }
    if (!(S.vault && S.vault[S.galaxy] && S.vault[S.galaxy].conquered)) { const tg = conquerTarget(S.galaxy);
      if (tg > 0) { const frac = Math.min(1, curEarned / tg), part = Math.floor(coreVal(S.galaxy) * frac * 0.5);
        srcChips += '<span class="asc-chip dim">P' + S.galaxy + ' bar ' + Math.round(frac * 100) + '% \u2192 +' + part + '</span>'; } }
    const srcHtml = '<div class="asc-src"><div class="asc-src-h">WHERE THE \u25c8 COME FROM</div>'
      + '<p class="muted asc-how">The big \u25c8 payday lands <b>at the moment you ascend</b>: each planet <b>conquered this run</b> banks a fixed bounty that grows ~\u00d71.3 per planet deeper (P1 +' + coreVal(1) + ' \u00b7 P5 +' + coreVal(5) + ' \u00b7 P10 +' + coreVal(10) + ' \u00b7 P18 +' + coreVal(18) + '). The planet you\'re still fighting adds up to <b>half</b> its bounty, scaled by its conquer bar. On top, every conquered world can <b>\u26cf BUILD a \u25c8 core mine</b> from the star map (costs 10% of that planet\'s conquer target) \u2014 a real-time dig (P1 pays 1\u25c8 every 2 days, deeper mines \u00d71.3 faster each planet \u2014 about a quarter of the \u25c8 a run banks) that keeps digging while you\'re away and stops when you ascend (re-conquer and re-build to re-open).</p>'
      + '<div class="asc-chips">' + (srcChips || '<span class="asc-chip dim">nothing conquered yet \u2014 the bank is empty</span>') + '</div></div>';
    let html = '<button class="big asc-go" id="ascend-go"' + ((pend < 1 || locked) ? " disabled" : "") + '>' + (locked ? "\u25c8 CONQUER 3 PLANETS TO ASCEND (" + conqN2 + "/3)" : "\u25c8 ASCEND NOW \u2014 BANK +" + fmt(pend)) + '</button>'
      + coach
      + srcHtml
      + '<p class="muted asc-note"><b>Resets:</b> planets \u00b7 empire \u00b7 units \u00b7 trees \u00b7 cash. <b>Keeps:</b> \u25c8 cores and your Engine levels. <b>THE LADDER:</b> hop at ~3 worlds on run 1, then ride every ascension a few worlds deeper \u2014 grinding a wall is never faster than hopping it.</p>'
      + '<div class="perk-grid">';
    ...
    render(dt) {
      ...
      for (const it of pts) {
        ...
        if (pv2 && pv2.conquered) {
          ...
          else { const pulse = 0.5 + 0.35 * Math.sin(this.t * 3.2);
            c.fillStyle = "rgba(255,255,255," + (S.cash >= mineCost(g) ? pulse.toFixed(2) : "0.45") + ")";
            c.fillText(S.cash >= mineCost(g) ? "⛏ BUILD ◈ MINE — tap" : "⛏ mine site · " + curSym(S.galaxy) + " " + fmt(mineCost(g)), p.x, p.y + r + 12); }
        }
        c.globalAlpha = 1;
      }
  ...
  function setScreen(s) {
    ...
    if (s === "play") { syncAutoBtn(); fitDisc(); envFor = 0; }   // v18.26: the dock/HUD only have real geometry once play is on screen — fit the world disc to the band they leave
  ...
  $("btn-travel").onclick = () => { if (S.travel) return; travel(); };   // v17.29: in warp the button just waits — pay-to-skip removed (it's a 7s cinematic)
  ...
  function resize() {
    ...
    fitDisc(); envFor = 0;                       // re-fit the planet to the new play band and rebuild its terrain (also sets camMin)
    camZoom = camZoom ? clamp(camZoom, camMin, 1.15) : camFit;                       // normal zoom sits inside the planet; pull back to camMin to see all of it
    for (const dr of drones) discClamp(dr, 12);
  ...
  // the play band the world disc was fitted to, so watch the dock and re-fit whenever it moves.
  // (Measured on a landscape phone: the band shrank ~90px after the rows filled in, which pushed the
  // limb straight through the dock until this was wired up.)
  if (typeof ResizeObserver !== "undefined") {
    let rfT = 0;
    const ro = new ResizeObserver(() => { const n = performance.now(); if (n - rfT < 120) return; rfT = n; fitDisc(); envFor = 0; });
    const dk = $("dock"); if (dk) ro.observe(dk);
    const tp = $("top"); if (tp) ro.observe(tp);
  }
  ...
  if (typeof window !== "undefined") window.__SIM = {
    ...
    valueMul: lv => 1 + 0.13 * lv,   // v18.14 chunkier global levels
    ...
    mineRate, mineCost, buildMine, mineBuilt, mineRateTotal,   // v18.5: the ⛏ ◈ farm is a paid build — probes exercise the real purchase path
