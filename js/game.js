...
(() => {
  ...
  function showConquest(g, cores, spoils) {
    ...
    const rows = [
      ...
      ["⛏ ◈ SEAM", wardenOf(g).n + " holds it — " + fmtMineRate(mineRate(g))],
    ...
    if (nt) nt.textContent = "The world is yours and at peace. Duel " + wardenOf(g).n
      + " for the ◈ seam when you are ready — the settlement panel holds the offer for as long as you like. Nothing starts until you say so.";
  ...
  function render() {
    ...
    if (state === "play" && !S.travel && planetMeta(S.galaxy).conquered && !wardenOn && conqCardT <= 0) {
      ...
      const body = [
        ...
        v.mine ? "⛏ core mine digging " + fmtMineRate(mineRate(S.galaxy)) + " — progress in the panel below" : "⛏ ◈ seam held by " + wardenOf(S.galaxy).n + " — ▲ summon it from the panel below to win the mine",
  ...
  function renderSettlePanel() {
    ...
    const cost = mineCost(g);   // v18.39: the price is now only the WARDEN HOARD's size — nothing in the UI can pay it
    let secs = -1;
    ...
    if (built && bp < 1) {
      ...
      // v18.39 (owner: "shouldn't be able to get round fighting the boss by buying the mine"): the ⛏ BUILD
      // button is GONE, here and on the star map. The seam has a keeper and the only way past it is through
      // it. There is no clock on the offer — take a week if you like — but there is no cheque either.
      html += '<button id="sp-warden" class="sp-build">▲ SUMMON ' + wardenOf(g).n + ' — win the ◈ mine</button>'
           + '<div class="sp-note">' + wardenOf(g).tell + ' — a 60s duel over the ◈ seam. Beat it and its hoard ('
           + curSym(g) + ' ' + fmt(cost) + ') founds the mine on the spot; lose and it is offered again, as often as you like.</div>'
           + '<div class="sp-note"><b>The seam is won, not bought.</b> A mine digs ' + fmtMineRate(rate)
           + ' for the rest of this run — even while you’re away — and ' + wardenOf(g).n + ' is the only way to one.</div>';
    }
  ...
  function showGalaxyInfo(g) {
    ...
    const stats = "<div class='gi-unlock'>" + curSym(g) + " <b>" + curName(g) + "</b>" +
      ...
      // v18.39: no BUILD button here either — a seam is taken off its keeper, on the ground, in person.
      // The card says where the duel is; it cannot be settled from the map with a wallet.
      (pv && pv.conquered && !pv.mine
        ? "<br><span class='gi-lock'>⛏ ◈ seam held by " + wardenOf(g).n + " — land here and win the duel (" + fmtMineRate(mineRate(g)) + ")</span>"
        : "") +
      "<br>" + prog + "</div>";
    ...
    render(dt) {
      ...
      for (const it of pts) {
        ...
        if (pv2 && pv2.conquered) {
          ...
          else { const pulse = 0.5 + 0.35 * Math.sin(this.t * 3.2);   // v18.39: an unclaimed seam is a duel waiting, not a price tag
            c.fillStyle = "rgba(255,255,255," + (g === S.galaxy ? pulse.toFixed(2) : "0.45") + ")";
            c.fillText(g === S.galaxy ? "▲ ◈ SEAM — summon the keeper" : "▲ ◈ seam held", p.x, p.y + r + 12); }
        }
        c.globalAlpha = 1;
      }
