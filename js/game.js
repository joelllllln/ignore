...
(() => {
  ...
  // TOP-DOWN MINING COMPLEX (v18.23 — owner: "the mine should be a top down full mine set up"). The
  // whole field is viewed from ABOVE, so the v18.19 side-elevation derrick was drawn in the wrong
  // projection. This is the plan view of a working pit head: the shaft mouth with its winding gear
  // turning over it, ore-cart rails spoking out to processing sheds and silos, carts running the
  // loop, a conveyor to the spoil heap, a lit perimeter road with a sweeping beacon, steam venting,
  // and the seam glowing up through it all. Only ever drawn on a settled calm world.
  ...
  function renderSettlePanel() {
    ...
    if (built) {
      ...
      html += '<button id="sp-warden" class="sp-build">▲ SUMMON THE MINE WARDEN — win the ◈ mine</button>'
           + '<div class="sp-note">a 60s boss duel over the ◈ seam — beat it and its hoard (' + curSym(g) + ' ' + fmt(cost) + ') founds the mine on the spot. Or buy it outright:</div>'
